...
$rsempsaleaccess=mysql_query($sqlempsaleaccess);
$rowempsaleaccess=mysql_fetch_array($rsempsaleaccess);
...
if(CRM_app=='yes' && !in_array('CRM_app',$menu_access_array))
{
	...
	$url = APICALLLOGURL."/datadownloaddictionary-7.0.4.php?nick_name=$nick_name&emp_code=$emp_code&device_id=$device_id&last_update_time=$last_update_time&incremental_download=$incremental_download";
	...
	fwrite($file, $newline);
	fclose($file);*/
	if (strpos($contents, 'destination_master') !== false) {
	...
	if(intval(($versionCodecurrent-$user_db_version_code)*10) > '1' && $is_update==1 && $incremental_download=='yes')
		{
			...
			$rsdbaccessdetails=mysql_query($sqldbaccessdetails,$linksetupdatabase);
			...
			if($remote_db_access=='yes')
			{
				...
				mysql_select_db(DBREMOTE,$link) or die("could not connect the remote database for invalid nick names");
			}
			else
			{
		...
		if(in_array('emp_master',$need_download_table_array))
			...
			else
			{
				...
				$rsqueryemp=mysql_query($sqlqueryemp);
				$rowqueryemp=mysql_fetch_array($rsqueryemp);
		...
		if(survey=='yes')
		{
			...
			else
			{
				...
				$rsquerybranch=mysql_query($sqlquerybranch);
				$rowquerybranch=mysql_fetch_array($rsquerybranch);
		...
		if(sauda_allocation=='yes')
		{
			if(!in_array('sauda',$menu_access_array))
			{
				...
				else
				{
					$sqlquerybranchfreight="SELECT branch_code FROM branch_route_freight WHERE 
											UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."') ";
					$resultbranchfreight = mysql_query($sqlquerybranchfreight);
					$countbranchfreight=mysql_num_rows($resultbranchfreight);
				...
				else
				{
					...
					$resultqueryload = mysql_query($sqlqueryload);
					$countqueryload =mysql_num_rows($resultqueryload);
					if($countqueryload >0)
					{
						$contents  .= 'load_distribution'."\n";
					}
			...
			$rowvertical=mysql_fetch_array($rsvertical);
		  ...
		  if(in_array('survey_category_master',$need_download_table_array))
			{
			...
			else
			 {
				$sqlquerysurveytableview="SELECT COUNT(row_id) AS total_survey_table_view FROM table_view WHERE 
									UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
				$rsquerysurveytableview=mysql_query($sqlquerysurveytableview);
				$rowquerysurveytableview=mysql_fetch_array($rsquerysurveytableview);
			...
			if(strtoupper($nick_name)=='EMAMI' ||  strtoupper($nick_name)=='EMAMIT')
			{
				...
				else
				{
					...
					$resultstatedistrict = mysql_query($sqlquerystatedistrict);
					$counttstatedistrict=mysql_num_rows($resultstatedistrict);
		...
		if(destination_price_list=='yes' || destination_ordertype_price_list=='yes' || destination=='yes')
		{
			...
			else
			{
				...
				$rsquerydestination=mysql_query($sqlquerydestination);
				$rowquerydestination=mysql_fetch_array($rsquerydestination);
		...
		if(distributor_route_planning=='yes' || distributor_route_emp_relation=='yes')
		{
			...
			else
			{
				...
				AND UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')";			
				$rsquerydistributorroute=mysql_query($sqlquerydistributorroute);
				$rowquerydistributorroute=mysql_fetch_array($rsquerydistributorroute);
		...
		if(input_screen_planwise=='yes')
		{
			...
			$rsqueryorderplan=mysql_query($sqlqueryorderplan);
		...
		if(product_qty_wise_TD=='yes')
		{
			...
			else
			{
				...
				$sqlquerydiscount="SELECT prod_code FROM prodqty_custclass_wise_TD WHERE 1 ".$condition_branch." AND 
								UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
				$resultquerydiscount = mysql_query($sqlquerydiscount);
				$countdiscount=mysql_num_rows($resultquerydiscount);
				if($countdiscount >0)
				{
					$contents  .= 'prodqty_custclass_wise_TD'."\n";
				}
		...
		if(TD_allocation_app=='yes')
		  ...
		  if(schemes=='yes'){
			...
			if($total_scheme_cnt >0)
			{
				...
				$rsqueryfreebies=mysql_query($sqlqueryfreebies);
				$rowqueryfreebies=mysql_fetch_array($rsqueryfreebies);
			...
			$result = mysql_query($sqlquery);
			$countordersummary=mysql_num_rows($result);
		  ...
		  if(retailer_app=='yes')
		  {
			...
			if($retailer_app=='yes' && $rds_tag!='' && $cust_type=='R')
				{
					...
					$rsempcust=mysql_query($sqlempcust); 
					while($rowempcust=mysql_fetch_array($rsempcust))
					{
				...
				$sqlquery="SELECT * FROM customer_product_billing WHERE customer_code IN (".$customer_code.") AND 
								UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
				$result = mysql_query($sqlquery);
				$countbilling=mysql_num_rows($result);
				if($countbilling >0)
				{
					$contents  .= 'billing_information'."\n";
				}
			...
			$resultstockbalancedetails = mysql_query($sqlstockbalancedetails);
			$countstockbalance=mysql_num_rows($resultstockbalancedetails);	
		   ...
		   if(!in_array('stock_mis_details',$need_download_table_array))
			...
			$resultstockmisdetails = mysql_query($sqlstockmisdetails);
			$countstockmisdetails=mysql_num_rows($resultstockmisdetails);	
		  ...
		  if(strtoupper($nick_name)=='START' || strtoupper($nick_name)=='DNV' || strtoupper($nick_name)=='DNVT')
		  {
		 	 $contents  .= 'branch_master'."\n";
		  }
		  if($reporting_level >0)
		  {
			...
			$resattendancecheckoutdetails = mysql_query($sqlattendancecheckoutdetails);
			$countattendancecheckout=mysql_num_rows($resattendancecheckoutdetails);	
...
if($successval=="1"){
	if($incremental_download=='no'){
		...
		if(business_prospect=='yes'){
			...
			$res_business_prospect=mysql_query($sql_business_prospect);
			if(mysql_num_rows($res_business_prospect)>0){
	...
	else
	{
		...
		/*if(freight_cost=='yes')
		{
			...
			$resqueryfreight = mysql_query($sqlqueryfreight);
			$countqueryfreight=mysql_num_rows($resqueryfreight);
		...
		if(catalogue=='yes')
		{
			...
			$rscatalogue_info=mysql_query($sqlcatalogue_info);
			$rowcatalogue_info=mysql_fetch_array($rscatalogue_info);
		...
		if(golden_rules=='yes')
		{
			...
			$sqlquery="SELECT gr_file_name,branch_code,acedns FROM branch_schemes_PDF WHERE 
			FIND_IN_SET(branch_code,'".$branch_code."') AND UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')
			ORDER BY download_time DESC";
			$result = mysql_query($sqlquery);
			$count=mysql_num_rows($result);
		 ...
		 if(strtoupper($nick_name)=='AJANTA')
		  ...
		  if(strtoupper($nick_name)=='DURO')
		  {
			  ...
			  $sqlqueryfacilitator="SELECT f_code FROM facilitator_master WHERE 1 ".$emp_val_rds." AND 
						UNIX_TIMESTAMP(download_time) > UNIX_TIMESTAMP('".$last_update_time."')";
			  $resultfacilitator = mysql_query($sqlqueryfacilitator);
			  $countfacilitator=mysql_num_rows($resultfacilitator);
	...
	if (strpos($contents, 'destination_master') !== false) {
		if (strpos($contents, 'branch_dump') !== false) {
