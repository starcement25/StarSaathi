...
$pgsql = "select $t_apperpdo.* from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS`!='Order canceled' and $customer_master.`branch_code` in('".$theactbcstr."') $new_whr_str ";
}else{
$pgsql = "select $t_apperpdo.* from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS`!='Order canceled' $new_whr_str ";
}
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                          <h2>Order List (<?php echo $total_pgres;?>) &nbsp;&nbsp;&nbsp;<span class="rpt_loader"></span> &nbsp;&nbsp;<a href="export_order2.php?get_type=all<?php echo $export_filtered_str;?>" class="btn bg-red waves-effe">Export&nbsp;all&nbsp;orders</a> &nbsp; <a href="export_order2.php?get_type=pending<?php echo $export_filtered_str;?>" class="btn bg-red waves-effe">Export&nbsp;pending&nbsp;orders</a> &nbsp; <a href="export_dispatched_order_with_challan.php?exp=yes<?php echo $export_filtered_str;?>" class="btn bg-red waves-effe">Export&nbsp;dispatched&nbsp;order&nbsp;with&nbsp;challan</a>
...
<?php
if($_SESSION["start_user_type"]=="MANAGER" && $order_show_branch!=""){
$sql1 = "select $t_apperpdo.*,$customer_master.`branch_code` from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS`!='Order canceled' and $customer_master.`branch_code` in('".$theactbcstr."') $new_whr_str order by $t_apperpdo.`order_date` desc limit $start_from,$limit";
}else{
$sql1 = "select $t_apperpdo.*,$customer_master.`branch_code` from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS`!='Order canceled' $new_whr_str order by `order_date` desc limit $start_from,$limit";
}
...
<td>
<?php if($order_status=="Dispatched" || $order_status=="Delivered"){ ?>
