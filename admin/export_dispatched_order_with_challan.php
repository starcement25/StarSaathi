...
$whr_str = "";
$export_filtered_str = "";
$search_array = array("trn_branch_id"=>$trn_branch_id,"ds_code"=>$ds_code,"daywise"=>array("sl_day_wise"=>$sl_day_wise,"from_dt"=>$from_dt,"to_dt"=>$to_dt));
foreach($search_array as $search_array_key=>$search_array_val){
	if($search_array_key=="trn_branch_id"){
		if($search_array_val!=''){
			if(trim($whr_str)!=""){
				$aand = " and";
			}else{
...
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_name` from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $customer_master.`branch_code` in('".$theactbcstr."') and $t_apperpdo.`STATUS` in('Dispatched','Delivered')  $new_whr_str order by $t_apperpdo.`order_date` asc";
}else{
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_name` from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS` in('Dispatched','Delivered') $new_whr_str order by `order_date` asc ";
}
$sql = mysql_query($qry);
$columns_total = mysql_num_fields($sql);
// Get The Field Name
$slno_cnt = 1;
$output .= '"SL_No","APPORDERNO","ERPORDERNO","DATE","Branch_Name","Cust_Code","Customer_Name","Order_For_Type","Consignee_Name","Consignee_Address","Freight","Destination","Product_Name","qty(MT)","Unbooked_qty(MT)","Phone_No","Dump_Status","Dump_Name","Dealer_Truck","STATUS","Order_From","ERP_Date","Challan_No","Challan_Date","Product_Name","QTY(MT)","Challan_QTY(MT)","Transporter_Name","Quantity_Checking","No_of_bags_short","Quality_Checking","No_of_damaged_bags","Challan_Status","Submitted_On"';
$output .="\n";
...
$sql_chalna = "select * from $t_dochallan where `dns_customer_code`='$dns_customer_code' and  `APPORDERNO`='".addslashes($apporder_no)."'";
