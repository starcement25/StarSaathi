...
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_name` from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS`!='Order canceled' and $customer_master.`branch_code` in('".$theactbcstr."') $new_whr_str order by $t_apperpdo.`order_date` asc";
}else{
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_name` from $t_apperpdo left join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code` where $t_apperpdo.`APPORDERNO`!='' and $t_apperpdo.`STATUS`!='Order canceled' $new_whr_str order by `order_date` asc";
}
$sql = mysql_query($qry);
$columns_total = mysql_num_fields($sql);
// Get The Field Name
$slno_cnt = 1;
$output .= '"SL_No","APPORDERNO","ERPORDERNO","DATE","Branch_Name","Cust_Code","Customer_Name","Order_For_Type","Consignee_Name","Consignee_Address","Freight","Destination","Product_Name","qty(MT)","Unbooked_qty(MT)","Phone_No","Dump_Status","Dump_Name","Dealer_Truck","STATUS","Order_From","Ref_SubDealer_Order","Authorized_By","Authorization_Date"';
$output .="\n";
...
$output .= '"'.$sl_no.'","'.$apporder_no.'","'.$erporder_no.'","'.$order_date.'","'.$customer_branch_name.'","'.$dns_customer_code.'","'.$customer_name.'","'.$order_for_type.'","'.$consignee_name.'","'.$consignee_address.'","'.$freight.'","'.$destination_name.'","'.$prod_display_name.'","'.$prod_qty.'","'.$the_unbooked_qty.'","'.$phone_no.'","'.$dump_status.'","'.$dump_name.'","'.$dealer_truck.'","'.$order_status.'","'.$order_from.'
