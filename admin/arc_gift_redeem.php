...
$sl_gift = $_GET["sl_gift"] ? addslashes(trim($_GET["sl_gift"])) : "";
$whr_str = "";
$search_array = array("srch_dtls"=>$srch_dtls,"sl_acr_status"=>$sl_acr_status,"sl_gift"=>$sl_gift);
...
$whr_str .= "$aand ($arc_gift_redeem_table.`name` like '%$search_array_val%' or $arc_gift_redeem_table.`mobile` like '%$search_array_val%' or $customer_master.`dns_customer_code` like '%$search_array_val%' or $customer_master.`customer_name` like '%$search_array_val%') ";
	...
	else if($search_array_key=="sl_gift"){
		if($search_array_val!=''){
			if(trim($whr_str)!=""){
				$aand = " and";
			}else{
			...
			$whr_str .= "$aand ($arc_gift_redeem_table.`gift_name`='$search_array_val') ";
			$new_qry_string_filtered .= "&sl_gift=".$search_array_val;
		}
	}
}
if($whr_str!=""){
...
$cnt = 0;
$countrow = 1;
$sqlgiftlist="SELECT DISTINCT `gift_name` FROM $arc_gift_redeem_table ORDER BY `gift_name` ASC ";
$resgiftlist = mysql_query($sqlgiftlist);
$total_gift = mysql_num_rows($resgiftlist);
...
<input type="text" class="form-control" id="srch_dtls" value="<?php echo $srch_dtls;?>" placeholder="Search Details">
    ...
    <select class="form-control" id="sl_gift">
    <option value="">Select Gift</option>
    <?php
    if($total_gift >0){
		while($rowgift=mysql_fetch_assoc($resgiftlist)){ 
			$gift_name=$rowgift['gift_name'];
		?>
		<option value="<?php echo $gift_name;?>" <?php if($gift_name==$sl_gift){?> selected="selected" <?php } ?>><?php echo $gift_name;?></option>	
	<?php	}
	}
	?>
    </select>
...
jQuery(".upd_arc_reg_sts_btn").click(function(){
	...
	jQuery(".srch_btn").click(function(){
		...
		var sl_gift = jQuery("#sl_gift").val();
		var qstring ="";
		var amp = "";
		if(srch_dtls!="" || sl_acr_status!="" || sl_gift!=""){
		if(srch_dtls!=""){
			if(qstring!=""){
		...
		if(sl_gift!=""){
			if(qstring!=""){
				qstring = qstring+"&sl_gift="+encodeURIComponent(sl_gift);
			}else{
				qstring = qstring+"sl_gift="+encodeURIComponent(sl_gift);
			}
		}
