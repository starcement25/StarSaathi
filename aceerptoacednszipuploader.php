				...
				//print_r($data);
					...
					if(no_of_filter > 1){
						...
						if($countprodgroupnamechk<1){
							...
							$sqlbrand .= "  product_group_code='".$product_group_code_name."'";
					...
					if(no_of_filter > 2){
						...
						if($countprodsubgroupnamechk<1){
							...
							$sqlbrandform .= "  product_sub_group_code='".mysql_real_escape_string($product_sub_group_code_name)."'";
					...
					if(no_of_filter > 3){
						...
						if($countprodbrandnamechk<1){
							...
							$sqlbrandsubform .= "  product_brand_code='".mysql_real_escape_string($product_brand_code_name)."'";
			...
			foreach($lines as $line)
			{
				...
				{ 
					...
					if($countempnamechk<1)
					{
						...
						$sql .= " , state='".$state."'";
						$sql .= " , zone='".$zone."'";
						mysql_query($sql) or die(mysql_error().".mysql_error().Duplicate key @row $csv_row_count on Employee code column in Employee Master.csv.Please check.");
						...
						$sqlcp .= " , status='true'";
						$sqlcp .= " , is_licensed='1'"; 
						mysql_query($sqlcp) or die(mysql_error().".mysql_error().Internal DATA execution problem on password table.PLease contact aceDNS admin.");
					}
					else
					{
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				mysql_query($sqlvendor) or die(mysql_error().".Internal error in Vendor master.csv.Please check.");
				}
			}
			 $rec_count++;
		}		
			...
			foreach($lines as $line)
			{
				...
				//print_r($data);
					...
					if($countmrpchk<1){
						...
						$sql .= " , download_time=CURRENT_TIMESTAMP()";
						mysql_query($sql)  or die(mysql_error().".Duplicate key @row $csv_row_count on Mrp code  columns in Mrp.csv.Please check.");
					}
					else
					{
