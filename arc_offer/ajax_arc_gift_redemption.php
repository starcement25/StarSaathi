...
$gift_idval = $_POST["gift_idval"] ? addslashes(trim($_POST["gift_idval"])) : "";
$redeem_content = "";
...
$res_data = array("process_sts"=>"NO","process_msg"=>"Please choose a gift.");	
}else{
...
$sql_gft = "select * from $arc_gift_catalogue where `id`='$gift_idval'";
$res_gft = mysql_query($sql_gft);
$totres_gft = mysql_num_rows($res_gft);
if($totres_gft>0){
$row_gft = mysql_fetch_assoc($res_gft);
...
if($tot_bag_count>=$eligible_gift_val){
...
$res_data = array("process_sts"=>"NO","process_msg"=>"The gift point is greater than available point.");	
}
...
$res_data = array("process_sts"=>"NO","process_msg"=>"Gift data not found.");		
}
