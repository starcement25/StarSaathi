...
<?php
$sql1 = "select * from $arc_gift_redeem_table where `customer_code`='$login_user_id' order by `entry_datetime` desc limit $start_from,$limit";
...
$the_reg_date = "";
$sql_reg = "select `entry_datetime` from $arc_consumer_reg where `mobile`='$consumer_mobile' order by `entry_datetime` asc limit 0,1";
$res_reg = mysql_query($sql_reg);
$totres_reg = mysql_num_rows($res_reg);
if($totres_reg>0){
$row_reg = mysql_fetch_assoc($res_reg);
$the_reg_date = $row_reg["entry_datetime"];
if($the_reg_date!=""){
$the_reg_date = date("jS M'y",strtotime($the_reg_date));	
}
}
?>
...
<span><?php echo $consumer_mobile;?></span><br/>
<span>Reg Date: <?php echo $the_reg_date;?></span>
