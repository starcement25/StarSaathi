...
$all_email_arr = array();
$email_hierarchy='';
$sqlcustcode="SELECT customer_code FROM customer_master WHERE dns_customer_code='".$dns_customer_code."'";
$rscustcode=mysql_query($sqlcustcode);
$rowcustcode=mysql_fetch_array($rscustcode);
$the_customer_code=$rowcustcode['customer_code'];
$sqldealeremp="SELECT emp_code FROM customer_route_emp_relation WHERE acedns='Y' AND customer_code='".$the_customer_code."'";
...
$remarks = $_REQUEST["remarks"] ? addslashes(trim($_REQUEST["remarks"])) : "";
		...
		$message= '<br><b>App Order No: </b> '.$apporderno.'<br>
			...
			<b>No. of bags short: </b> '.$ch_quantity_no_of_bags.'<br>
