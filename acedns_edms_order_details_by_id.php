...
if($the_id!=""){
$sqlall = "select * from $T_APPERPDO where `customer_code`='$the_id' order by `order_date` desc";
...
$order_data[] = array("apporderno"=>$apporderno,"erporderno"=>$erporderno,"erporderdt"=>$order_date,"isdoimp"=>$isdoimp,"order_for"=>$order_for,"customer_code"=>$customer_code,"dns_customer_code"=>$dns_customer_code,"status"=>$status,"prod_code"=>$prod_code,"dns_prod_code"=>$dns_prod_code,"prod_display_name"=>$prod_display_name,"qty"=>$qty,"order_full_date_time"=>$order_full_date_time,"order_challan_data"=>$order_challan_data);
