...
//for testing
$sqlin = "SELECT allocation_id, dns_prod_code, prod_desc, allocation_qty, date_and_time, order_id,sub_dealer_id,challan_no FROM $allocation_details WHERE $user_type_clause $year_month_condition order by date_and_time desc";
...
// ORDER BY date_and_time DESC;
    ...
    if (!$resin) {
		...
		$sqlchallandate="SELECT `CHALLANDT` FROM `T_DOCHALLAN` WHERE FIND_IN_SET(`CHALLANNO`,'".$challan_no."')";
				$rschallandate=mysql_query($sqlchallandate);
				$challan_date='';
				while($rowchallandate=mysql_fetch_array($rschallandate))
				{
					$challan_date=$challan_date.$rowchallandate['CHALLANDT'].',';
				}
				
				$challan_date=substr($challan_date,0,-1);
				
				
              
                $allocation_data[] = array("allocation_id" => $allocation_id, "dns_prod_code" => $dns_prod_code, "prod_desc" => $prod_desc, "allocation_qty" => $allocation_qty, "date_and_time" => $date_and_time, "order_id" => $order_id, "challan_no" => $challan_no, "challan_date" => $challan_date, "counter_name" => $customer_name);
            }
