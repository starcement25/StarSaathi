...
if ($the_customer_code == "") {
	...
	where $t_apperpdo.dns_customer_code='".$the_customer_code."' and (`status`='Success' or `status`='Shipped' )  $date_qry order by $t_apperpdo_pop.`order_date` desc limit $start_from,$limit";*/
		echo $sqlall2 = "select $t_apperpdo.APPORDERNO,$t_apperpdo.order_date ,$t_apperpdo.QTY,$t_apperpdo.dns_prod_code,$t_apperpdo.prod_display_name,$t_apperpdo.STATUS ,$t_apperpdo.freight,$t_apperpdo.destination_name,$customer_master.`customer_name`,$customer_master.`customer_id` from $t_apperpdo inner join $customer_master on $t_apperpdo.`dns_customer_code`=$customer_master.`dns_customer_code`
