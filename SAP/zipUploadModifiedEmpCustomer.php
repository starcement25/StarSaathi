					...
					{ 
					  ...
					  //print_r($data);
						...
						if($countbranchnamechk<1)
						{
							...
							$sqlbranch .= " , branch_accounts_email_id='".mysql_real_escape_string($branch_accounts_email_id)."'";
						...
						else
						{
							...
							$sqlbranch .= " , branch_accounts_email_id='".mysql_real_escape_string($branch_accounts_email_id)."'";
							$sqlbranch .= " , download_time=CURRENT_TIMESTAMP()";
							$sqlbranch .= " , alternative_email_id='".mysql_real_escape_string($alternative_email_id)."' WHERE branch_code='".addslashes($branch_code_db)."'";
						}
						mysql_query($sqlbranch) or die(mysql_error().".Duplicate key @row $csv_row_count in Branch master.csv.Please check.");
					}
					 $rec_count++;
				}		
				$successval=1;
					...
					foreach($lines as $line)
					{
						...
						//print_r($data);
							...
							if(no_of_filter > 1){
								...
								else
								{
									...
									if($vertical_value_db!=$vertical_value)
									{
										...
										mysql_query($sqlupdatebrand) or die(mysql_error().".Internal error occurrs in product_group_name column @row $csv_row_count in sku master.csv.Please check.");
									}
							...
							if($countskunamechk<1)
							{
								...
								mysql_query($sql) or die(mysql_error().".Duplicate key @row $csv_row_count on Sku code column in Sku master.csv.Please check.");								$insertflag=1;
							}
							else
							{
									...
									$conversion_factor_two_db!=$conversion_factor || $UOM3_db!=$UOM3 || $pack_size_db!=$pack_size || $TD_db!=$TD))
								{
									...
									mysql_query($sql) or die(mysql_error().".Duplicate key @row $csv_row_count on Sku code column in sku master.csv.Please check.");
									$updateflag=1;
								}
								else if($cl_stk_db!=$cl_stk)
								{
									...
									mysql_query($sql) or die(mysql_error().".Internal error @row $csv_row_count on Sku code column in sku master.csv.Please check.");
									$updateflag=1;
								}
								//For TT
					...
					foreach($lines as $line)
					{
						...
						{ 
							...
							if($countempnamechk<1)
							{
								...
								$sql .= " , zone='".$zone."'";
								$sql .= " , download_time=CURRENT_TIMESTAMP()";
								mysql_query($sql) or die(mysql_error().".Duplicate key @row $csv_row_count on Employee code column in Employee Master.csv.Please check.");
							...
							else
							{
								...
								mysql_query($sqlupdate) or die(mysql_error().".Internel error  @row $csv_row_count on in Employee Master.csv.Please check.");
						...
						{ 
						  ...
						  //print_r($data);
							...
							if($countcustomernamechk<1)
							{
								...
								$sql .= " , download_time=CURRENT_TIMESTAMP()";
								//exit();
								mysql_query($sql) or die(mysql_error().".Duplicate key @row $csv_row_count on Customer name and Employee columns in customer master.csv.Please check.");
								modifyempdatadownloadlog($emp_code,strtoupper($folderName));
					...
					foreach($lines as $line)
					{
						...
						{ 
						  ...
						  //print_r($data);
							...
							mysql_query($sql) or die(mysql_error().".Internel error occurrs @row $csv_row_count on Outstanding.csv.Please check.");
							if($emp_code!='')
							 {
								modifyempdatadownloadlog($emp_code,strtoupper($folderName));
							 }
					...
					foreach($lines as $line)
					{
						...
						//print_r($data);
							...
							if($countmrpchk<1){
								...
								$sql .= " , acedns='".$acedns."'";
								$sql .= " , download_time=CURRENT_TIMESTAMP()";
								mysql_query($sql)  or die(mysql_error().".mysql_error().Duplicate key @row $csv_row_count on Mrp code  columns in Mrp.csv.Please check.");
								$insertflag=1;
							}
							else
							{
							...
							/*if($updateflag==1 || $insertflag==1)
							{
								...
								$rsupdatedbversion=mysql_query($sqlupdatedbversion);
							}*/
					 ...
					 //print_r($data);
						...
						if($emp_code!='' && $route_code!='')
						{
							$sqlemproute="SELECT emp_code,acedns,route_code FROM emp_route_relation WHERE emp_code='".$emp_code."' 
										AND route_code='".$route_code."'";
							$rsemproute=mysql_query($sqlemproute);
							$countemproute=mysql_num_rows($rsemproute);
							if($countemproute <1)
							{
								...
								mysql_query($sqlinsertemproute)  or die(mysql_error().".Internal error occurrs @row $csv_row_count in Employee route.csv.Please check.");
							}
							else
							{
								$rowemproute=mysql_fetch_array($rsemproute);
								...
								if($acednsdb!=$acedns)
								{
									...
									$rsupdateemproute=mysql_query($sqlupdateemproute)  or die(mysql_error().".Internal error occurrs @row $csv_row_count in Employee route.csv.Please check.");
								}
							}
						}
					}
