...
function return_employee_upper_hierarchy($emp_code,$link) {
    $emphierarchy = array();
    employee_upper_hierarchy_details($emp_code, $emphierarchy,$link);
	foreach($emphierarchy as $hierarchyval)
	{
		$emphierarchystring.=$hierarchyval.',';
	}
...
function employee_upper_hierarchy_details($emp_code,&$emphierarchy,$link){
		...
		while($rowemphierarchy=mysqli_fetch_array($rsemphierarchy))
		{
			...
			if(strpos($reporting_to,',')!=false){
				...
				for($cn=0;$cn<count($reporting_to_Arr);$cn++)
				{
					...
					employee_upper_hierarchy_details($reporting_to_Arr[$cn],$emphierarchy,$link);
				}
			}
			else
			{
				$emphierarchy[] = "'".$reporting_to."'";
				employee_upper_hierarchy_details($reporting_to,$emphierarchy,$link);
			}
		}
	}
	else
