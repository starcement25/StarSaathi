        ...
        if(isset($json["d"]["results"]))
        {
            foreach($json["d"]["results"] as $row)
            {
                ...
                if($vbeln != "" && $abgru != "")
                {
                    ...
                    */
                    // Check if ERPORDERNO already exists
                    $sql_check_log = "SELECT id FROM $log_table 
                                    WHERE ERPORDERNO='$vbeln' 
                                    LIMIT 1";

                    $res_check_log = mysql_query($sql_check_log);

                    if(mysql_num_rows($res_check_log) > 0)
                    {
                       ...
                       echo $sql_update_log = "UPDATE $log_table SET
                                            apperpdo_id='$apperpdo_id',
                                            allocation_id='$allocation_id',
                                            log_datetime='$curr_datetime',
                                            api_date='$date_val'
                                        WHERE ERPORDERNO='$vbeln'";

                        mysql_query($sql_update_log);
                    }
                    else
                    {
                        // ➕ INSERT new record
                        $sql_insert_log = "INSERT INTO $log_table
                            (ERPORDERNO, apperpdo_id, allocation_id, log_datetime, api_date)
                            VALUES
                            ('$vbeln', '$apperpdo_id', '$allocation_id', '$curr_datetime', '$date_val')";
