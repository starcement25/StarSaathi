	...
	where $t_apperpdo.dns_customer_code='" . $dns_customer_code . "' and `status`='Dispatched'  order by $t_apperpdo.`order_date` desc";
		...
		if ($totall2 > 0) {
			while ($row112 = mysql_fetch_assoc($resall2)) {
				...
				$sqlall3 = "select * from $T_DOINVOICE where `APPORDERNO`='$order_id'".$date_condition;
				...
				if ($totall3 > 0) {
					...
					$dispatched_order_data[] = array("order_id" => $order_id, "order_date" => $order_full_date_time, "customer_name" => $customer_name, "destination_name" => $destination_name, "dns_prod_code" => $dns_prod_code, "prod_display_name" => $prod_display_name, "qty" => $qty, "freight" => $freight, "STATUS" => $STATUS,  "dispatched_invoice_data" => $dispatched_invoice_data);
				}
					//$ch_challandt
