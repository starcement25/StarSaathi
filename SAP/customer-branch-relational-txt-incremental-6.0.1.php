...
$countbranches=mysql_num_rows($rsbranches);
 ...
 if($countbranches>1 && vertical_fields=='yes' && $emp_code!='C0007'){
	$sqlquery="SELECT  DISTINCT CBR.customer_code,CBR.branch_code,CBR.acedns FROM customer_master CM,emp_route_relation ERM,customer_branch_relation CBR 
				WHERE ".$emp_hierarchy_condition." AND CBR.customer_code=CM.customer_code AND 
				CM.route_code=ERM.route_code AND ERM.acends!='N' ".$login_condition."";			
 }
 ...
 {
	$sqlquery="SELECT  DISTINCT CBR.customer_code,CBR.branch_code,CBR.acedns FROM customer_master CM,employee_master EM,customer_branch_relation CBR 
				WHERE ".$emp_hierarchy_condition." AND CBR.customer_code=CM.customer_code AND 
				CM.route_code=ERM.route_code AND ERM.acends!='N' ".$login_condition.""; 
}
...
$count=mysql_num_rows($result);
	...
	$url = "http://www.acedns.in/acednsproduct/customer-branch-relational-txt-incremental-6.0.1.php?nick_name=$nick_name&emp_code=$emp_code&last_update_time=$last_update_time&data_download_time=$data_download_time&incremental_download=$incremental_download";
