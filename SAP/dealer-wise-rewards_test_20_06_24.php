...
if ($rsdealertour === false) {
    ...
    if ($rowdealertour) {
        ...
        if ($result === false) {
            ...
            if ($rowtour) {
                $rewardstatus = $rowtour['dealer_status'];

                // Determine the reward link based on user_type and rewardstatus
                if ($rewardstatus == 'ACTIVE') {
                    if ($user_type == 'dealer') {
                        $reward_link = 'https://dealer.starsaathirewards.com/dealer_code=' . $dns_customer_code . '/';
                    } elseif ($user_type == 'sub dealer') {
                        $reward_link = 'https://dealer.starsaathirewards.com/dealer_code=' . $dns_customer_code . '/'; 
                    } else {
                        $reward_link = ''; // Handle other user types if needed
                    }
