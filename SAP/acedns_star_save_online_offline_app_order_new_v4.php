...
$user_type = $_REQUEST["user_type"] ? strtoupper($_REQUEST["user_type"]) : ""; //BROKER
...
//echo"<pre>";print_r($is_allow_order);die;
...
//echo"<pre>";print_r($sql_ck_reford);die;
...
//echo"<pre>";print_r($SAP_sub_dealer_code);die;
...
if($sub_dealer_code!=""){
		...
		$resadditionaldata = mysql_query($sqladditionaldata);
...
//echo"<pre>";print_r($sqlin_temp);die;
...
//echo"<pre>";print_r($GLOBALS['send_email']);die;
...
if($email_hierarchy!=''){
	...
	$curr_date_format = date("jS M, y",strtotime($order_date));
		...
		//$send_mail = send_the_mail($final_email,$subject,$message);
			//$email_arr = explode(",", $final_email);
			$email= $final_email;
			//foreach ($email_arr as $email) {
				$email = trim($email);
				//if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
					$email_safe   = mysql_real_escape_string($email);
					$subject_safe = mysql_real_escape_string($subject);
					$message_safe = mysql_real_escape_string($message);
					date_default_timezone_set("Asia/Kolkata");
					$created_at = date("Y-m-d H:i:s");
					$sqlin_temp = "
						INSERT IGNORE  INTO email_queue (APPORDERNO, recipient, subject, body, created_at) 
						VALUES ('$apporderno', '".$email_safe."', '$subject_safe', '$message_safe', '$created_at')
					";
					mysql_query($sqlin_temp);
				//}
			//}
		//echo"<pre>";print_r('99');
