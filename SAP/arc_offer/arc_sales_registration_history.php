...
$add_page_name = "arc_sales_registration_history.php";
$page_name = "arc_sales_registration_history.php";
$is_show = "NO";
$show_message = "";
...
/*---------PAGINATION RELATED CODE START----------*/
$pgsql = "select $arc_consumer_reg.`ac_id` from $arc_consumer_reg where `customer_code`='$login_user_id'";
...
<title>SALE REGISTRATION HISTORY</title>
<style>
...
<h5 style="text-align:center; margin-bottom:30px; margin-top:10px">SALE REGISTRATION HISTORY</h5>
...
<?php
$sql1 = "select * from $arc_consumer_reg where `customer_code`='$login_user_id' order by `entry_datetime` desc limit $start_from,$limit";
...
if($totres1>0){
	while($row1=mysql_fetch_assoc($res1)){
		...
		if($each_status=="REDEEMED"){
			$bag_cnt = $redeemed_bags;
		}else{
			$bag_cnt = $no_of_bags;
		}
...
<div class="card">
<div class="card-body">
<h5 class="card-title"><?php echo $consumer_name;?></h5>
<p class="card-text"><?php echo $consumer_mobile;?><br/>
<span class="right_text"><?php echo $entry_datetime;?></span><br/>
<span><strong>Bags:</strong> <?php echo $bag_cnt;?></span><span class="right_text <?php echo $sts_cls;?>"><strong><?php echo $each_status;?></strong></span></p>
