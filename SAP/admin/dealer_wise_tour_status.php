...
$whr_str = "";
$search_array = array("srch_dlr_dtls"=>$srch_dlr_dtls,"sl_status"=>$sl_status );
...
$whr_str .= "$aand ($customer_master.`dns_customer_code` like '%$search_array_val%' or $customer_master.`customer_name` like '%$search_array_val%' or $customer_master.`customer_id` like '%$search_array_val%' ) ";
	...
	if($search_array_key=="sl_status"){
		if($search_array_val!=''){
			...
			{
				$whr_str .= " $aand $dealer_tour_status.`tour_status`='$search_array_val'";
			}
			if($search_array_val=='INACTIVE')
			{
				$whr_str .= " $aand ($dealer_tour_status.`tour_status`='$search_array_val'  OR $customer_master.`dns_customer_code` NOT IN(SELECT customer_code FROM $dealer_tour_status ))";
			}
...
$add_page_name = "dealer_wise_tour_status.php";
$page_name = "dealer_wise_tour_status.php";
$cnt = 0;
$countrow = 1;
...
$pgsql = "SELECT $customer_master.`customer_code` FROM $customer_master left join $dealer_tour_status on $customer_master.`dns_customer_code`=$dealer_tour_status.`customer_code` WHERE $customer_master.cust_type='Dealer' $con $new_whr_str ";
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                          <h2>Dealer Wise Tour Status (<?php echo $total_pgres;?>)&nbsp;&nbsp;<!--<a href="export_sp_destination.php?get_type=all" class="btn bg-red waves-effe">Export&nbsp;all</a>-->
...
$sql1 = "SELECT $customer_master.`customer_code`,$customer_master.`dns_customer_code`,$customer_master.`customer_name`,$dealer_tour_status.`tour_status` FROM $customer_master left join $dealer_tour_status on $customer_master.`dns_customer_code`=$dealer_tour_status.`customer_code` WHERE $customer_master.cust_type='Dealer'  $con $new_whr_str order by $customer_master.`customer_name` asc limit $start_from,$limit";
...
if($totres1>0){
	while($row1=mysql_fetch_assoc($res1)){
		...
		$tour_status = $row1["tour_status"];
		if($tour_status=='') $tour_status='INACTIVE';
...
if(count($pg_sts_arr)>0){
	foreach($pg_sts_arr as $pg_sts_arr_val){
        ...
        <option value="<?php echo $the_key_val;?>" <?php if($the_key_val==$tour_status){?> selected="selected" <?php } ?>><?php echo $the_title_val;?></option>
        <?php
	}
...
jQuery(function(){
	...
	jQuery(".cwts_sel").change(function(){
		...
		if(the_customer_code!=""){
			...
			jQuery.ajax({
			url: 'ajax_tour_status_update.php',
			type: 'post',
			dataType: "JSON",
