<?php
session_start();
//echo"<pre>";print_r($_SESSION["start_user_type"]);die;
include "web_check.php";
include "star_connection.php";

//delete record sk 300425
if(isset($_GET['allocation_id'])){
	$user_id=$_SESSION["start_report_admin"];
	//echo"<pre>";print_r($user_id);die;
	$allocation_id=trim($_GET['allocation_id']);
	$log_sql = "INSERT INTO allocation_details_invoicewise_log (
				allocation_id,
				date_and_time,
				inv_date,
				order_id,
	...
	mysql_query($log_sql);
	//$sql="DELETE FROM `allocation_details_invoicewise` WHERE `allocation_id`='".$allocation_id."'";
	$sql="UPDATE `allocation_details_invoicewise` SET `delete_at`='1' WHERE `allocation_id`='".$allocation_id."'";
...
if ($res) {
	...
	// window.location.href='allocation_details_report_invoicewise.php';
	// </script>";
	?>
	<div id="popup-alert" class="alert alert-success alert-dismissible" role="alert">
...
$totres1 = 0;
$page_name = "allocation_details_report_invoicewise.php";
$cnt = 0;
$countrow = 1;
...
if($_SESSION["start_user_type"]=="MANAGER" && $order_show_branch!=""){
	$sql1 = "select $allocation_details.*,$allocation_details.allocation_qty AS total_allocation_qty,$customer_master.dns_customer_code,$customer_master.customer_name,$branch_master.branch_name from $allocation_details,$customer_master,$branch_master Where $allocation_details.customer_id=$customer_master.customer_id AND $customer_master.branch_code=$branch_master.branch_code and $customer_master.`branch_code` in('".$theactbcstr."')  $new_whr_str  order by $allocation_details.`customer_id` asc,$allocation_details.`APPORDERNO` asc,$allocation_details.`prod_desc` asc,$allocation_details.`inv_date` asc,$allocation_details.date_and_time asc limit $start_from,$limit";
}
else{
$sql1 = "select $allocation_details.*,$allocation_details.allocation_qty AS total_allocation_qty,$customer_master.dns_customer_code,$customer_master.customer_name,$branch_master.branch_name from $allocation_details,$customer_master,$branch_master Where $allocation_details.customer_id=$customer_master.customer_id AND $customer_master.branch_code=$branch_master.branch_code $new_whr_str  order by $allocation_details.`customer_id` asc,$allocation_details.`APPORDERNO` asc,$allocation_details.`prod_desc` asc,$allocation_details.`inv_date` asc,$allocation_details.date_and_time asc limit $start_from,$limit";
}
				...
				<div class="card">
					<div class="header">
						<h2>Allocation Report (<?php echo $total_pgres; ?>)&nbsp;&nbsp;
							<a href="export_allocation_report_invoicewise.php?get_type=all<?php echo $export_filtered_str; ?>" class="btn bg-red waves-effe">Export&nbsp;Allocation&nbsp;Report</a> &nbsp;
							<a href="export_deleted_allocation_report_invoicewise.php?get_type=all<?php echo $export_filtered_str; ?>" class="btn bg-red waves-effe">Export&nbsp;Deleted&nbsp;Allocation&nbsp;Report</a> &nbsp;
							<a href="javascript:void(0);" data-toggle="modal" data-target="#importModal" 
								class="btn bg-red waves-effe">
								Bulk RSSD Allocation Delete
								</a>
						</h2>
						<br><br>
						<span style="clear:both;display:block;"></span>
						<!-- <form  method="post" action="allocation_invoice_import.php" enctype="multipart/form-data" style="display:inline;">
								<input type="file" name="import_file" accept=".csv" required><br>
								<button type="submit" class="btn bg-red waves-effe">Import Deleted RSSD Allocation CSV</button>
							</form> -->
					...
					<!-- ================== IMPORT POPUP ================== -->
						...
						<div class="modal-dialog modal-md" role="document">
							...
							<div class="modal-header">
								<h4 class="modal-title">Import Deleted RSSD Allocation CSV</h4>
							</div>

							<div class="modal-body">

								<p><b>Download Sample CSV:</b>  
								<a href="../csv/sample_deleted_rssd_allocation.csv" target="_blank" style="color:blue;">
									Click here to download sample file
								</a>
								</p>

								<form method="post" action="allocation_invoice_import.php" enctype="multipart/form-data" id="importForm">
							...
							<div class="modal-footer">
								...
								<button type="submit" form="importForm" class="btn bg-red waves-effe">
								Upload & Import
								</button>
							</div>
							...
							<tbody>
								...
								if ($totres1 > 0) {
									while ($row1 = mysql_fetch_assoc($res1)) {
										...
										$delete_at = $row1["delete_at"] ? trim($row1["delete_at"]) : "";
								...
								?>
										<tr>
											...
											<td><?php echo $total_inv_qty; ?></td>
											<td><?php echo $row1["total_allocation_qty"] ; ?></td>
											<td><?php echo $remaining_allocation_qty; ?></td>
											<td><?php echo $inv_no; ?></td>
											<td><?php echo $inv_cancl; ?></td>
											<td style="padding:5px;width:120px;" class="td_action">
												<?php if($_SESSION["start_user_type"]=="ADMIN" || $_SESSION["start_user_type"]=="MANAGER"){ if($delete_at==0){ ?>
											<a onclick="return confirm('Are you sure you want to delete this allocation?');" href="allocation_details_report_invoicewise.php?allocation_id=<?php echo $row1["allocation_id"]; ?>" class="btn bg-red waves-effect dlt_slider_cls" dlt_slider_id="66" style="padding:5px;display:inline-block;">DELETE</a>
