...
$pg_sts_arr = array();
$pg_sts_arr[] = array("key_val"=>"","title_val"=>"Not Set");
...
$add_page_name = "branch_wise_pg_status.php";
$page_name = "branch_wise_pg_status.php";
$cnt = 0;
$countrow = 1;
...
$pgsql = "SELECT $branch_master.`branch_code` FROM $branch_master left join $branch_PGstatus on $branch_master.`branch_code`=$branch_PGstatus.`branch_code` $new_whr_str";
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                          <h2>Branch Wise PG Status (<?php echo $total_pgres;?>)&nbsp;&nbsp;<!--<a href="export_sp_destination.php?get_type=all" class="btn bg-red waves-effe">Export&nbsp;all</a>-->
...
$sql1 = "SELECT $branch_master.`branch_code`,$branch_master.`dns_branch_code`,$branch_master.`branch_name`,$branch_PGstatus.`pg_status` FROM $branch_master left join $branch_PGstatus on $branch_master.`branch_code`=$branch_PGstatus.`branch_code` $new_whr_str order by $branch_master.`branch_name` asc limit $start_from,$limit";
...
if($totres1>0){
	while($row1=mysql_fetch_assoc($res1)){
		...
		$the_pg_status = $row1["pg_status"] ? trim($row1["pg_status"]) : "";
...
if(count($pg_sts_arr)>0){
	foreach($pg_sts_arr as $pg_sts_arr_val){
        ...
        <option value="<?php echo $the_key_val;?>" <?php if($the_key_val==$the_pg_status){?> selected="selected" <?php } ?>><?php echo $the_title_val;?></option>
        <?php
	}
...
jQuery(function(){
	...
	jQuery(".bwps_sel").change(function(){
		...
		if(the_brnch_code!=""){
			...
			jQuery.ajax({
			url: 'ajax_bwps_status_update.php',
			type: 'post',
			dataType: "JSON",
