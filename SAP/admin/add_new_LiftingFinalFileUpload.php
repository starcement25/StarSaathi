                        ...
                        if (mysql_query($sql_up) === TRUE) {
                            ...
                            //echo"<pre>";print_r('ss');die;
                                ...
                                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                                    // validate data
                                        // if (empty($data) || count($data) < 17) {
                                        //     continue;
                                        // }
                                    
                                        $allocationDateTime = date('Y-m-d', strtotime(trim($data[0])));
                                        $appOrderNo = mysql_real_escape_string($data[1]);
                                        $invDate = date('Y-m-d', strtotime(trim($data[2])));
                                        $linkedDealerCode = mysql_real_escape_string($data[3]);
                                        $linkedDealerSAPCode = mysql_real_escape_string($data[4]);
                                        $linkedDealerName = mysql_real_escape_string($data[5]);
                                        $subDealerCode = mysql_real_escape_string($data[6]);
                                        $subDealerSAPCode = mysql_real_escape_string($data[7]);
                                        $subDealerName = mysql_real_escape_string($data[8]);
                                        $branch = mysql_real_escape_string($data[9]);
                                        $month = mysql_real_escape_string($data[10]);
                                        $productName = mysql_real_escape_string($data[11]);
                                        $totalInvQty = is_numeric($data[12]) ? $data[12] : 0;
                                        $allocatedQty = is_numeric($data[13]) ? $data[13] : 0;
                                        $remainingQty = is_numeric($data[14]) ? $data[14] : 0;
                                        $invNo = mysql_real_escape_string($data[15]);
                                        $is_offline = mysql_real_escape_string($data[16]);
                                    ...
                                    $sql_insert = "INSERT INTO lifting_final_allocation_data (
                                        ...
                                        $totalInvQty, $allocatedQty, $remainingQty, '$invNo', '$last_id', '$is_offline'
                                    )";
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="form-group">
                                        <div class="form-group n_pdf_upload_section">
                                            <label class="Select PDF">Select CSV Files<?php if($required!=''){ ?><span class="estarix_cls">*</span><?php } ?>&nbsp;(file size: less than 10MB )&nbsp;<span class="pdf_err_cls"></span></label>
