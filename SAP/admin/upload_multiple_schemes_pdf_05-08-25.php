	...
	if(count($_FILES['npdf_file']['name'])>0){
		...
		if(count($uploaded_pdf_file_names_arr>0)){
			$curr_date_time = date("Y-m-d H:i:s");
				//echo"<pre>";print_r($branch_code_arr);die;

			foreach($branch_code_arr as $branch_code_arr_val){
				$the_branch_code = $branch_code_arr_val;
				//echo"<pre>";print_r($uploaded_pdf_file_names_arr);die;
				
				foreach($uploaded_pdf_file_names_arr as $uploaded_pdf_file_names_vals){
				$sql_in = "insert into $branch_schemes_pdf (`scheme_name`,`branch_code`,`PDF_file_name`,`start_date`,`end_date`,`download_time`) values('$scheme_name','$the_branch_code','$uploaded_pdf_file_names_vals','$start_dt','$end_dt','$curr_date_time')";
				echo"<pre>";print_r($sql_in);die;
				$res_msg = array("process_sts"=>"YES","process_msg"=>"Scheme PDF successfully added. ".$sql_in);	
