...
$output = "";
$header = "";
$header .= '"SL_No","APPORDERNO","Sale Order No","DATE","Branch_Name","Dealer SAP Code","Cust_Code","Customer_Name","Order_For_Type","Consignee SAP Code","Consignee_Name","Consignee_Address","Freight","Destination Code","Destination","Product_Name","qty(MT)","Unbooked_qty(MT)","Phone_No","Dump_Status","Dump_Code","Dump_Name","Dealer_Truck","STATUS","Order_From","Ref_SubDealer_Order","Authorized_By","Authorization_Date","Delivery point","Remarks text/truck no","Additional_data_1"';
$header .="\n";
...
$limit  = 5000;   // number of rows per chunk
$offset = 0;

while (true) {

if($_SESSION["start_user_type"]=="MANAGER" && $order_show_branch!=""){
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$customer_master.`customer_name` from $t_apperpdo left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code` where $t_apperpdo.`APPORDERNO`!=''  and $customer_master.`branch_code` in('".$theactbcstr."') $new_whr_str order by $t_apperpdo.`order_date` desc LIMIT $limit OFFSET $offset";
}else{
...
(SELECT $customer_master.customer_id FROM $customer_master WHERE  $customer_master.dns_customer_code=IF($t_apperpdo.`dns_sub_dealer_code`!='',$t_apperpdo.`dns_sub_dealer_code`,$t_apperpdo.`dns_customer_code`) AND $t_apperpdo.`dns_customer_code`!='') AS `the_consignee_code`,
(select `VWERK` from $SAP_customer_master where KUNNR=(select customer_id from $customer_master where `dns_customer_code`=$t_apperpdo.`dns_customer_code` AND $t_apperpdo.`dns_customer_code`!='') ORDER BY AEDAT DESC,ADDITIONAL_DATA1 DESC LIMIT 0,1) AS  `VWERK`
from $t_apperpdo INNER join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code` where $t_apperpdo.`APPORDERNO`!=''  and  SUBSTRING( T_APPERPDO.order_date ,1,10) > '2025-04-01' $new_whr_str order by `order_date` desc LIMIT $limit OFFSET $offset";
}
//exit();
...
'.$broker_name.'","'.$ref_sub_dealer_order_id.'","'.$authorized_by.'","'.$authorization_date.'","'.$Delivery_point.'","'.$Remarks_text.'","'.$Additional_data_1.'"';
$output .="\n";
$slno_cnt++;
}
$offset += $limit; // next chunk
}
// Download the file
