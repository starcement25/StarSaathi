...
$page_name = "customer_app_usage_log.php";
$cnt = 0;
$countrow = 1;
...
/*---------PAGINATION RELATED CODE START----------*/
$pgsql = "select $app_service_track_log.`customer_code`,$app_service_track_log.`appservice_name`,$customer_master.`customer_name`,$customer_master.region,$branch_master.branch_name,$app_service_track_log.datetime from 
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                    <div class="header">
                          <h2>Usage Log (<?php echo $total_pgres;?>)&nbsp;&nbsp;<a href="export_usage_log_list.php?paged=<?php echo $page.$new_qry_string_filtered;?>" class="btn bg-red waves-effe">Export&nbsp;all</a></h2>
...
<?php $sqlapiname="SELECT DISTINCT $app_service_track_log.appservice_name FROM $app_service_track_log ORDER BY $app_service_track_log.appservice_name ASC"; 
	  $rsapiname=mysql_query($sqlapiname);
	  while($rowapiname=mysql_fetch_assoc($rsapiname))
	  {
?>
<option value="<?php echo $rowapiname['appservice_name'];?>" <?php if($srch_api_name==$rowapiname['appservice_name']){ ?> selected="selected" <?php }?> ><?php echo $rowapiname['appservice_name'];?></option>
<?php }?>
</select>
    </div>
...
<span style="display:block; clear:both;"></span>
 <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover">
                                    ...
                                    <tbody>
<?php
/*$sql1 = "select $app_service_track_log.`customer_code`,$app_service_track_log.`appservice_name`,count($app_service_track_log.`appservice_name`) as `tot_count`,$customer_master.`dns_customer_code`,$customer_master.`customer_name`,$customer_master.sales_office,$customer_master.sales_office_desc,SUBSTRING($app_service_track_log.datetime,1,10) AS hit_date from 
		$app_service_track_log left join $customer_master on $app_service_track_log.`customer_code`=$customer_master.`customer_code` $new_whr_str 
		group by SUBSTRING($app_service_track_log.datetime,1,10),$app_service_track_log.`customer_code`,$app_service_track_log.`appservice_name` order by SUBSTRING($app_service_track_log.datetime,1,10) DESC,$app_service_track_log.`customer_code` asc limit $start_from,$limit";*/
$sql1 = "select $app_service_track_log.`customer_code`,$app_service_track_log.`appservice_name`,$customer_master.`customer_name`,$customer_master.region,$branch_master.branch_name,$app_service_track_log.datetime from 
		$app_service_track_log inner join $customer_master on $app_service_track_log.`customer_code`=$customer_master.`customer_id` inner join $branch_master ON $customer_master.`branch_code`=$branch_master.branch_code $new_whr_str 
		 order by $app_service_track_log.datetime DESC,$app_service_track_log.`customer_code` asc limit $start_from,$limit";										
...
<td><?php echo $appservice_name;?></td>	
