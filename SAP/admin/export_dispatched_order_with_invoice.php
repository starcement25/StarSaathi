...
$the_file_name = "dispatched_orders_with_invoice_".$curr_date.".csv";
...
// Get The Field Name
$slno_cnt = 1;
$output .= '"SL_No","APPORDERNO","Sale Order No","ORDER DATE","Branch_Name","Cust_Code","Customer_Name","Order_For_Type","Consignee_Name","Consignee_Address","Freight","Destination","Product_Name","qty(MT)","Unbooked_qty(MT)","Phone_No","Dump_Status","Dump_Name","Dealer_Truck","STATUS","Order_From","Sale Order Date","Invoice_No","Invoice_Date","Product_Name","Invoice_QTY(MT)","Truck No"';
$output .="\n";
...
$sql_invoice = "select * from $T_DOINVOICE where `APPORDERNO`='".addslashes($apporder_no)."'";
$res_invoice = mysql_query($sql_invoice);
$totres_invoice = mysql_num_rows($res_invoice);
if($totres_invoice>0){

	while($row_invoice=mysql_fetch_assoc($res_invoice)){
		$apporder_no_chtl = $row_invoice["APPORDERNO"];
		$erporder_no_chtl = $row_invoice["ERPORDERNO"];
		$challanno_chtl = $row_invoice["CHALLANNO"];
		$prod_display_name_chtl= $row_invoice["prod_display_name"];
		$INVNO= $row_invoice["INVNO"];
		$INVDT= $row_invoice["INVDT"];
		$INVDT=date('d-m-Y',strtotime($INVDT));
		$inv_qty = $row_invoice["INVQTY"];
		
$TRUCKNO = $row_invoice["TRUCKNO"] ? trim($row_invoice["TRUCKNO"]) : "";
		

$output .= '"'.$sl_no.'","'.$apporder_no.'","'.$erporder_no.'","'.$order_date.'","'.$customer_branch_name.'","'.$dns_customer_code.'","'.$customer_name.'","'.$order_for_type.'","'.$consignee_name.'","'.$consignee_address.'","'.$freight.'","'.$destination_name.'","'.$prod_display_name.'","'.$prod_qty.'","'.$the_unbooked_qty.'","'.$phone_no.'","'.$dump_status.'","'.$dump_name.'","'.$dealer_truck.'","'.$order_status.'","'.$order_from.'","'.$erporder_date.'","'.$INVNO.'","'.$INVDT.'","'.$prod_display_name_chtl.'","'.$inv_qty.'","'.$TRUCKNO.'"';
$output .="\n";
