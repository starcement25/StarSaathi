...
startCreatNoticeListCsvfile();

function startCreatNoticeListCsvfile(){
$notice_2023 = "notice_2023";
$changepassword = "changepassword";
$get_fetch_type = $get_fetch_type ? strtolower(trim($get_fetch_type)) : "";
$curr_date = date("jS_M_Y_h_m_s_A");
$the_file_name = "noticeList_".$curr_date.".csv";
$server_url = "http://".$_SERVER['SERVER_NAME']."/SAP/";
$home_url = $server_url."admin/";
$output ="";
$output .= '"Dealer_ID","Dealer_SAP_Code","Dealer_Name","Phone","Device_Type","App_Version","Branch_Name","Branch_Code","Branch_DNS_Code","is_confirmed_declaration","Submitted_On"';
$output .="\n";
$qry = "select $notice_2023.*,$changepassword.`device_type`,$changepassword.`app_version` from $notice_2023 left join $changepassword on $notice_2023.`sf_cust_code`=$changepassword.`customer_code` order by $notice_2023.`sf_submitted_datetime` desc";
$sql = mysql_query($qry);
...
$output .= '"'.$sf_dealer_id.'","'.$sf_sf_dealer_sap_code.'","'.$sf_dealer_name.'","'.$sf_dealer_mobile.'","'.$sf_device_type.'","'.$sf_app_version.'","'.$sf_branch_name.'","'.$sf_branch_code.'","'.$sf_dns_branch_code.'","'.$sf_is_checked_declaration.'","'.$sf_submitted_datetime.'"';
