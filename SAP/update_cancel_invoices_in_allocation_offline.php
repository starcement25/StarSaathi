...
echo $prev_date_3days='2025-06-11';
...
//$sql2 = "SELECT inv_no,customer_id,APPORDERNO  FROM $allocation_details_invoicewise WHERE SUBSTRING(date_and_time,1,10)='".$prev_date_3days."' AND customer_id='1000000646' ORDER BY `date_and_time` ASC";
  $sql2 = "SELECT inv_no,customer_id,APPORDERNO  FROM $allocation_details_invoicewise WHERE SUBSTRING(date_and_time,1,10)='".$prev_date_3days."' AND is_offline='Offline'  ORDER BY `date_and_time` ASC";	
...
$inv_no_list=substr($inv_no_list,0,-1);
  ...
  echo"<pre>2";print_r($selinvdate);	

	//die;
    //echo"<pre>";print_r($selinvdate);die;
...
/*
    ...
    if (!in_array($normalized_pair, $sap_invoice_pairs)) {
        ...
        $sqlinactivate = "UPDATE allocation_details_invoicewise 
                          ...
                          AND APPORDERNO='" . mysql_real_escape_string($apporderno) . "' LEFT(INVDT, 4) ='$year1'
                          AND is_offline='Offline'";
       // echo"<pre>4";print_r($sqlinactivate);                 
