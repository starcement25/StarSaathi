...
$order_status_arr = array("DO approved","Dispatched","Credit Check Failed");
...
$sel_order_status = $_GET["sel_order_status"] ? addslashes(trim($_GET["sel_order_status"])) : "";
...
$whr_str = "";
$search_array = array("srch_dealer_dtls"=>$srch_branch_dtls,"sel_message_type"=>$sel_message_type,"sel_sent_status"=>$sel_sent_status,"sel_order_status"=>$sel_order_status,"daywise"=>array("sl_day_wise"=>$sl_day_wise,"from_dt"=>$from_dt,"to_dt"=>$to_dt));
...
$whr_str .= "$aand (`cust_dns_code` like '%$search_array_val%' or `cust_name` like '%$search_array_val%') ";
	...
	else if($search_array_key=="sel_order_status"){
		if($search_array_val!=''){
			if(trim($whr_str)!=""){
				$aand = " and";
			}else{
				...
				$whr_str .= "$aand `message_text` LIKE '%".$search_array_val."%' ";
			$new_qry_string_filtered .= "&sel_order_status=".$search_array_val;		
		}		
	}
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="header">
                          <h2>Auto Notification Log (<?php echo $total_pgres;?>)&nbsp;&nbsp;
                           <a href="export_auto_notification.php" class="btn bg-red waves-effe">Export</a>
...
<input type="text" class="form-control" id="srch_dealer_dtls" value="<?php echo $srch_dealer_dtls;?>" placeholder="Search Dealer Details">
	...
	?>
     ...
     <div class="col-lg-1 col-md-1 col-sm-12 col-xs-12 add_top_bottom_padding" style="padding:5px 2px;">
    <select class="form-control sel_order_status" id="sel_order_status">
    <option value="">Order Status</option>
    <?php
    if(count($order_status_arr)>0){
		foreach($order_status_arr as $order_status_val){ ?>
			<option value="<?php echo $order_status_val;?>" <?php if($order_status_val==$sel_order_status){ ?> selected="selected" <?php } ?> ><?php echo $order_status_val;?></option>
		<?php }
	}
	?>
    </select>
...
changeYear:true
	...
	jQuery(".srch_btn").click(function(){
		...
		var sel_order_status = jQuery("#sel_order_status").val();
		...
		var amp = "";
			...
			if(sel_order_status!=""){
			if(qstring!=""){
				qstring = qstring+"&sel_order_status="+encodeURIComponent(sel_order_status);
			}else{
				qstring = qstring+"sel_order_status="+encodeURIComponent(sel_order_status);
			}
			}
