...
$add_page_name = "pop_order_report.php";
$page_name = "pop_order_report.php";
$cnt = 0;
$countrow = 1;
...
/*---------PAGINATION RELATED CODE START----------*/
$the_status_arr = array("Selected","Cancelled & Refund Initiated","Refund Completed");
?>
<style>
    .adminActivityField3{
	display:block;
...
if($totres1>0){
    ...
    $admin_remarks = htmlspecialchars($row1["admin_remark"]);
	...
	$sqlpaymentstatus="select payment_gateway from $pop_product_master where `dns_prod_code`='$dns_prod_code'";
    // echo $sql2;
    $respaymentstatus=mysql_query($sqlpaymentstatus);
    $rowpaymentgateway=mysql_fetch_array($respaymentstatus);
    $payment_gateway=$rowpaymentgateway['payment_gateway'];
	if($payment_gateway=='N'){ $status='Payment Pending';}
	else {	$status=$status;}	
...
<td>
    <div>
        <select class="form-control the_status" id="the_status_<?php echo $order_id; ?>" e_order_id="<?php echo $order_id; ?>">
            <option value="" <?php if ($order_status == "") echo ""; ?>>Selected</option>
            <option value="Cancelled & Refund Initiated" <?php if ($order_status == "Cancelled & Refund Initiated") echo "selected"; ?>>Cancelled & Refund Initiated</option>
            <option value="Refund Completed" <?php if ($order_status == "Refund Completed") echo "selected"; ?>>Refund Completed</option>
        </select>
        <span class="adminActivityField3" id="sl_eng_sts_ldr_<?php echo $order_id; ?>"></span>
    </div>
</td>

<td>
<input type="text" class="remark-input" id="remark-input_<?php echo $order_id; ?>" value="<?php echo $admin_remarks; ?>">
<button class="update-btn btn bg-red waves-effect" data-order-id="<?php echo $order_id; ?>">Update</button>
<span class="adminActivityField3" id="sl_eng_sts_ldr_<?php echo $order_id; ?>"></span>
</td>
...
<script>
    function reset_func(){
        window.location.href = "https://starsaathi.com/SAP/admin/pop_order_report.php";
    }
</script>

<script>
    $(document).ready(function() {
        $('.the_status').change(function() {
            var order_id = $(this).attr('e_order_id');
            ...
            $.ajax({
                ...
                success: function(response) {
                    var messageElement = $('#sl_eng_sts_ldr_' + order_id);
                    
                    // Clear any previous message and make it visible
                    messageElement.stop(true, true).html(response).show();
                ...
                error: function(xhr, status, error) {
                    // Handle any errors
                    console.error(xhr);
                    $('#sl_eng_sts_ldr_' + order_id).html('Error updating status');
                }
            });
        });
    });
...
<script>
$(document).ready(function() {
        // Function for updating admin remarks
        $('.update-btn').click(function() {
            var order_id = $(this).data('order-id');
            var newRemark = $('#remark-input_' + order_id).val();

            $.ajax({
                url: 'ajax_update_remarks.php',
                type: 'POST',
                data: {
                    admin_remarks: newRemark,
                    order_id: order_id
                },
                success: function(response) {
                    var messageElement = $('#sl_eng_sts_ldr_' + order_id);
                ...
                error: function(xhr, status, error) {
                    // Handle any errors
                    console.error(xhr);
                    $('#sl_eng_sts_ldr_' + order_id).html('Error updating remark');
                }
            });
        });
    });
