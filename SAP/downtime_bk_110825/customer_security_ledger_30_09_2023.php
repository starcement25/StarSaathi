...
if ($the_customer_id != "") {
    ...
    $astn_fy = isset($_POST['astn_fy']) ? $_POST['astn_fy'] : array();

    if (empty($astn_fy)) {
        // Handle the case when no financial years are selected
        if ($from_dt == '' && $to_dt == '') {
            $curr_date_time = date("Y-m-d H:i:s");
            if ($curr_month > 3) {
                $monthyearstart = date("Y", strtotime($curr_date_time)) - 1;
            ...
            $the_start_date_ac = $monthyearstart . "-04-01";
            $the_end_date_ac = $monthyearend . "-03-31";
        } else {
            $the_start_date_ac = date('Y-m-d', strtotime($from_dt));
            $the_end_date_ac = date('Y-m-d', strtotime($to_dt));
        }
    } else {
        // Handle the case when financial years are selected
        // Sort the selected financial years
        sort($astn_fy);

        // Get the start and end years from the selected financial years
        $start_year = reset($astn_fy);
        $end_year = end($astn_fy);

        // Calculate the start and end dates based on the selected range
        $the_start_date_ac = $start_year . "-04-01";
        $the_end_date_ac = ($end_year + 1) . "-03-31";
    }
    ...
    // Update $from_dt and $to_dt if needed
...
?>
                    ...
                    <div class="card">
                        <div class="header">
                            ...
                            <div class="col-lg-1 col-md-1 col-sm-12 col-xs-12 add_top_bottom_padding">
                                ...
                                <!-- <h5 style="text-align:center">Select FY</h5> -->
                                <div class="dropdown">
                                <button class="btn bg-red waves-effect" id="selectFyButton">Select FY</button>

                                <div class="dropdown-content" id="fyDropdown">
                                    <label>
                                        <input type="checkbox" name="astn_fy[]" value="2022" <?php if (in_array('2022', $astn_fy)) { ?> checked <?php } ?>>
                                        FY22-23
                                    </label>
                                    <label>
                                        <input type="checkbox" name="astn_fy[]" value="2023" <?php if (in_array('2023', $astn_fy)) { ?> checked <?php } ?>>
                                        FY23-24
                                    </label>
                                </div>
                            </div>
                                ...
                                <tbody>
                                    ...
                                    if (empty($astn_fy)) {
                                        ...
                                        if ($from_dt == '' && $to_dt == '') {
                                            ...
                                            $the_start_date_ac = $monthyearstart . "-07-31T00:00:00";
                                            $the_end_date_ac = $monthyearend . "-03-31T00:00:00";
                                        } else {
                                            $the_start_date_ac = date('Y-m-d', strtotime($from_dt));
                                            $the_end_date_ac = date('Y-m-d', strtotime($to_dt));
                                        }
                                        ...
                                        // Calculate the start and end dates based on the selected range
                                        $the_start_date_ac = $start_year . "-04-01T00:00:00";
                                        $the_end_date_ac = ($end_year + 1) . "-03-31T00:00:00";
                                    }

                                    // Convert the start and end dates to the desired format
                                    $the_start_date_time = date('Y-m-d', strtotime($the_start_date_ac)) . "T00:00:00";
                                    $the_end_date_time = date('Y-m-d', strtotime($the_end_date_ac)) . "T00:00:00";
                                    ...
                                    // Use $the_start_date_time and $the_end_date_time in your code as needed
    ...
    <script>
    document.getElementById("selectFyButton").addEventListener("click", function(event) {
        // Prevent the default form submission behavior
        event.preventDefault();

        // Toggle the dropdown
        var fyDropdown = document.getElementById("fyDropdown");
        fyDropdown.classList.toggle("show");
    });

    // Close the dropdown if the user clicks outside of it
    window.addEventListener("click", function(event) {
        var fyDropdown = document.getElementById("fyDropdown");
        if (!event.target.matches("#selectFyButton") && !event.target.matches(".dropdown-content label")) {
            if (fyDropdown.classList.contains("show")) {
                fyDropdown.classList.remove("show");
            }
        }
    });
</script>
