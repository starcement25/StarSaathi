...
if($_SESSION["start_user_type"]=="MANAGER" && $order_show_branch!=""){
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$customer_master.`customer_name`,$plant_list.plant_name  from $t_apperpdo
left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code`
left join $plant_list on $t_apperpdo.`plant_code`=$plant_list.`plant_code`
where $t_apperpdo.`APPORDERNO`!=''  and $customer_master.`branch_code` in('".$theactbcstr."') $new_whr_str order by $t_apperpdo.`order_date` desc";
}else{
	//(select `VWERK` from $SAP_customer_master where KUNNR=(select customer_id from $customer_master where `dns_customer_code`=$t_apperpdo.dns_customer_code) ORDER BY AEDAT DESC,ADDITIONAL_DATA1 DESC LIMIT 0,1) AS  `VWERK`

/*$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$customer_master.`customer_name`,
(SELECT `cust_type` FROM $customer_master where $customer_master.customer_code=$t_apperpdo.sub_dealer_code) AS `cust_type`,
(SELECT $branch_master.branch_name FROM $branch_master WHERE  $branch_master.branch_code=$t_apperpdo.branch_code) AS `branch_name`,
(SELECT $destination_master.`dns_destination_code` FROM $destination_master WHERE  $destination_master.destination_code=$t_apperpdo.destination_code) AS `the_destination_code`,
(SELECT $customer_master.customer_id FROM $customer_master WHERE  $customer_master.dns_customer_code=$t_apperpdo.`dns_customer_code` AND $t_apperpdo.`dns_customer_code`!='') AS `the_consignee_code`,
(select `VWERK` from $SAP_customer_master where KUNNR=(select customer_id from $customer_master where `dns_customer_code`=$t_apperpdo.dns_customer_code AND $t_apperpdo.`dns_customer_code`!='') ORDER BY AEDAT DESC,ADDITIONAL_DATA1 DESC LIMIT 0,1) AS  `VWERK`
from $t_apperpdo INNER join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code` where $t_apperpdo.`APPORDERNO`!='' $new_whr_str order by `order_date` desc";*/
/*$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$customer_master.`customer_name`,
(SELECT `cust_type` FROM $customer_master where $customer_master.customer_code=$t_apperpdo.sub_dealer_code) AS `cust_type`,
(SELECT $branch_master.branch_name FROM $branch_master WHERE  $branch_master.branch_code=$t_apperpdo.branch_code) AS `branch_name`,
(SELECT $destination_master.`dns_destination_code` FROM $destination_master WHERE  $destination_master.destination_code=$t_apperpdo.destination_code) AS `the_destination_code`,
(SELECT $customer_master.customer_id FROM $customer_master WHERE  $customer_master.dns_customer_code=IF($t_apperpdo.`dns_sub_dealer_code`!='',$t_apperpdo.`dns_sub_dealer_code`,$t_apperpdo.`dns_customer_code`) AND $t_apperpdo.`dns_customer_code`!='') AS `the_consignee_code`,
(select `VWERK` from $SAP_customer_master where KUNNR=(select customer_id from $customer_master where `dns_customer_code`=IF($t_apperpdo.`dns_sub_dealer_code`!='',$t_apperpdo.`dns_sub_dealer_code`,$t_apperpdo.`dns_customer_code`) AND $t_apperpdo.`dns_customer_code`!='') ORDER BY AEDAT DESC,ADDITIONAL_DATA1 DESC LIMIT 0,1) AS  `VWERK`
from $t_apperpdo INNER join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code` where $t_apperpdo.`APPORDERNO`!='' $new_whr_str order by `order_date` desc";
(SELECT zorderdetailsp .Consignee_Code FROM zorderdetailsp WHERE  zorderdetailsp.APPORDERNO=$t_apperpdo.`APPORDERNO`) AS `the_consignee_code`

*/

/*echo $qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$customer_master.`customer_name`,
(SELECT `cust_type` FROM $customer_master where $customer_master.customer_code=$t_apperpdo.sub_dealer_code) AS `cust_type`,
(SELECT $branch_master.branch_name FROM $branch_master WHERE  $branch_master.branch_code=$t_apperpdo.branch_code) AS `branch_name`,
(SELECT $destination_master.`dns_destination_code` FROM $destination_master WHERE  $destination_master.destination_code=$t_apperpdo.destination_code) AS `the_destination_code`,
(SELECT $customer_master.customer_id FROM $customer_master WHERE  $customer_master.dns_customer_code=IF($t_apperpdo.`dns_sub_dealer_code`!='',$t_apperpdo.`dns_sub_dealer_code`,$t_apperpdo.`dns_customer_code`) AND $t_apperpdo.`dns_customer_code`!='') AS `the_consignee_code`,
(SELECT customer_master .customer_id FROM customer_master WHERE  customer_master.dns_customer_code=$t_apperpdo.`dns_customer_code`) AS `the_consignee_code`,
(select `VWERK` from $SAP_customer_master where KUNNR=(select customer_id from $customer_master where `dns_customer_code`=$t_apperpdo.`dns_customer_code` AND $t_apperpdo.`dns_customer_code`!='') ORDER BY AEDAT DESC,ADDITIONAL_DATA1 DESC LIMIT 0,1) AS  `VWERK`
from $t_apperpdo INNER join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code` where $t_apperpdo.`APPORDERNO`!=''  and  SUBSTRING( T_APPERPDO.order_date ,1,10) > '2023-04-01' $new_whr_str order by `order_date` desc";*/
$qry = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$customer_master.`customer_name`,
...
(SELECT $customer_master.customer_id FROM $customer_master WHERE  $customer_master.dns_customer_code=IF($t_apperpdo.`dns_sub_dealer_code`!='',$t_apperpdo.`dns_sub_dealer_code`,$t_apperpdo.`dns_customer_code`) AND $t_apperpdo.`dns_customer_code`!='') AS `the_consignee_code`,
(select `VWERK` from $SAP_customer_master where KUNNR=(select customer_id from $customer_master where `dns_customer_code`=$t_apperpdo.`dns_customer_code` AND $t_apperpdo.`dns_customer_code`!='') ORDER BY AEDAT DESC,ADDITIONAL_DATA1 DESC LIMIT 0,1) AS  `VWERK`
from $t_apperpdo INNER join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code`
left join $plant_list on $t_apperpdo.`plant_code`=$plant_list.`plant_code`
where $t_apperpdo.`APPORDERNO`!=''  and  SUBSTRING( $t_apperpdo.order_date ,1,10) > '2023-04-01' $new_whr_str order by `order_date` desc";
}
//exit();
$sql = mysql_query($qry);
//echo mysql_errno() . ": " . mysql_error();
//$colum//ns_total = mysql_num_fields($sql);
// Get The Field Name
$slno_cnt = 1;
$output .= '"SL_No","APPORDERNO","Sale Order No","DATE","Branch_Name","Dealer SAP Code","Cust_Code","Customer_Name","Order_For_Type","Consignee SAP Code","Consignee_Name","Consignee_Address","Freight","Destination Code","Destination","Product_Code","Product_Name","qty(MT)","Unbooked_qty(MT)","Phone_No","Dump_Status","Dump_Code","Dump_Name","Dealer_Truck","STATUS","Order_From","Ref_SubDealer_Order","Authorized_By","Authorization_Date","Delivery point","Remarks text/truck no","Additional_data_1","Plant"';
$output .="\n";
...
$sl_no = $row["id"];
$apporder_no = normalizeUtf8($row["APPORDERNO"]);
//exit();
$erporder_no = normalizeUtf8($row["ERPORDERNO"]);
$order_date = $row["order_date"];
$customer_branch_code = normalizeUtf8($row["branch_code"]);
//$customer_branch_name = get_branch_name_from_id($customer_branch_code);
$customer_branch_name = normalizeUtf8($row["branch_name"]);
$dns_customer_code = normalizeUtf8($row["dns_customer_code"]);
$customer_code = normalizeUtf8($row["customer_code"]);
//$customer_name = get_customer_name_from_dealer_id($dns_customer_code);
$customer_name = normalizeUtf8($row["customer_name"]);
$consignee_name = "";
$consignee_address = "";
...
$order_for = $row["order_for"] ? trim($row["order_for"]) : "";
	...
	$prod_code = normalizeUtf8($row["dns_prod_code"]);
	$prod_display_name = normalizeUtf8($row["prod_display_name"]);
	$prod_qty = normalizeUtf8($row["QTY"]);
	$freight = normalizeUtf8($row["freight"]);
	...
	$prod_qty = normalizeUtf8($row["QTY"]);
	$phone_no = normalizeUtf8($row["phone_no"]);
	$phone_no = $phone_no ? str_replace('"', '""',$phone_no) : "";
	$dump_status = normalizeUtf8($row["dump_status"]);
	$dump_code = normalizeUtf8($row["dump_code"]);
	$dump_name = normalizeUtf8($row["dump_name"]);
	...
	$order_for_cust_type=$row['cust_type']? trim($row["cust_type"]) : "";
		...
		$totconsigneedet = mysql_num_rows($resconsigneedet);
			...
			//$the_SAP_plant='';
			$the_destination_code = $row["the_destination_code"] ? addslashes(trim($row["the_destination_code"])) : "";
			$the_consignee_code = $row["the_consignee_code"] ? addslashes(trim($row["the_consignee_code"])) : "";
			$the_SAP_plant = $row["VWERK"] ? addslashes(trim($row["VWERK"])) : "";
			//$the_consignee_code=$the_SAP_code;
			/*if($order_for_type =="Self" && $the_consignee_code==''){
				$the_consignee_code=$the_SAP_code;
			}
	...
	$Delivery_point = $row["Delivery_point"];
		...
		$Additional_data_1 = $row["Additional_data_1"];
		$plant_name = $row["plant_name"];




$output .= '"'.$sl_no.'","'.$apporder_no.'","'.$erporder_no.'","'.$order_date.'","'.$customer_branch_name.'","'.$the_SAP_code.'","'.$dns_customer_code.'","'.$customer_name.'","'.$order_for_type.'","'.$the_consignee_code.'","'.$consignee_name.'","'.$consignee_address.'","'.$freight.'","'.$the_destination_code.'","'.$destination_name.'","'.$prod_code.'","'.$prod_display_name.'","'.$prod_qty.'","'.$the_unbooked_qty.'","'.$phone_no.'","'.$dump_status.'","'.$the_SAP_plant.'","'.$dump_name.'","'.$dealer_truck.'","'.$order_status.'","'.$order_from.'
'.$broker_name.'","'.$ref_sub_dealer_order_id.'","'.$authorized_by.'","'.$authorization_date.'","'.$Delivery_point.'","'.$Remarks_text.'","'.$Additional_data_1.'","'.$plant_name.'"';
$output .="\n";
$slno_cnt++;
}
//echo $output;die;
