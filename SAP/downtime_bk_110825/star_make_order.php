...
$sl_self_id = $_POST["sl_self_id"] ? $_POST["sl_self_id"] : "";
...
$sl_delivery_id=$_POST["sl_delivery_id"] ? $_POST["sl_delivery_id"] : "";
$delivery_remarks=$_POST["delivery_remarks"] ? $_POST["delivery_remarks"] : "";
$plant_list=$_POST["plant_list"] ? $_POST["plant_list"] : "";

if($sl_self_id!=='') $sl_sd_id=$sl_self_id;
if($order_for_type==""){
...
$the_dest_code =$sl_des_ex_id;
if($the_freight=='EX') $the_freight='EXW';
...
foreach($prod_qty_arr as $prod_qty_arr_key2=>$prod_qty_arr_val2){
	...
	if($the_prod_qty_arr_val!=""){
		if(is_numeric($the_prod_qty_arr_val)){
			$gen_order_data_arr[] = array("apporderno"=>"","erporderno"=>"","erporderdt"=>"","order_for"=>$order_for,"order_for_type"=>$order_for_type,"customer_code"=>$sswa_selected_customer_code,"sub_dealer_code"=>$sl_sd_id,"prod_code"=>$the_prod_qty_arr_key,"qty"=>$the_prod_qty_arr_val,"freight"=>$the_freight,"destination_code"=>$the_dest_code,"destination_name"=>$destination_name,"destination_address"=>$destination_name,"phone_no"=>$phone_no,"dump_status"=>$dump_status,"dump_code"=>$sl_dump_id,"dump_name"=>$dump_name,"Delivery_point"=>$sl_delivery_id,"Remarks_text"=>$delivery_remarks,"plant_list"=>$plant_list);
		}
	}
}


//$url_ck = "http://starsaathi.com/SAP/acedns_star_save_online_offline_app_order_new_v4.php";
$url_ck =BASE_URL . "downtime/acedns_star_save_online_offline_app_order_new_v4.php";
...
// Debug: Output raw API response and stop
/*echo "<pre>";
print_r($body_for_mcode); // Or use var_dump($body_for_mcode);
echo "</pre>";
die();*/
...
$sql_plant="SELECT * FROM `plant_list` WHERE 1";
$res_plant1 = mysql_query($sql_plant);
$totres_plant=mysql_num_rows($res_plant1);
//echo"<pre>";print_r(mysql_num_rows($res_plant1));die;

//$url_ck1 = "http://starsaathi.com/SAP/ship-to-party-master-txt-V1.php?emp_code=$sswa_selected_customer_code&user_type=dealer";
$url_ck1 =BASE_URL . "ship-to-party-master-txt-V1.php?emp_code=$sswa_selected_customer_code&user_type=dealer";
	 $body_for_mcode1 = get_data_from_cserver($url_ck1);
	if(isJsonCk($body_for_mcode1)){
	$json_decoded = json_decode($body_for_mcode1,true);
	$dealer_data=$json_decoded['dealer_data'];
	}

	//$url_ck2 = "http://starsaathi.com/SAP/ship-to-party-master-txt-V1.php?emp_code=$sswa_selected_customer_code&user_type=sub-dealer";
	$url_ck2 =BASE_URL . "ship-to-party-master-txt-V1.php?emp_code=$sswa_selected_customer_code&user_type=sub-dealer";
	$body_for_mcode2 = get_data_from_cserver($url_ck2);
	if(isJsonCk($body_for_mcode2)){
	$json_decoded_sub_dealer = json_decode($body_for_mcode2,true);
	$sub_dealer_data=$json_decoded_sub_dealer['sub_dealer_data'];
	}
...
.sd_div,.destination_for_div,.dump_div,.self_div,.delivery_div{
	display:none;
}
.mand_msg{
	color:#FF0000;
...
<div class="form-group v_element self_div">
<label for="Select Ship to Party Dealer">SELECT SHIP TO PARTY DEALER <span class="mand_msg">*</span> <span class="sd_self_loader"></span> </label>
<div class="form-line">
<select name="sl_self_id" id="sl_self_id" class="form-control sl_self_id" data-placeholder="Choose SHIP TO PARTY DEALER...">
<option value="">Choose SHIP TO PARTY DEALER ...</option>
<?php
if(count($dealer_data) >0){
	foreach($dealer_data as $dealerdataval)
	{
		$ship_to_dealer_code=$dealerdataval['customer_code'];
		$ship_to_dealer_name=$dealerdataval['customer_name'];
	?>
<option value="<?php echo $ship_to_dealer_code; ?>"><?php echo $ship_to_dealer_name; ?></option>
	<?php
	}
}
?>
...
<span class="err_msg" id="sl_self_id_error"></span>
</div>
...
if(count($sub_dealer_data) >0){
	foreach($sub_dealer_data as $subdealerdataval)
	{
		$ship_to_sub_dealer_code=$subdealerdataval['customer_code'];
		$ship_to_sub_dealer_name=$subdealerdataval['customer_name'];
	?>
<option value="<?php echo $ship_to_sub_dealer_code; ?>"><?php echo $ship_to_sub_dealer_name; ?></option>
	<?php
	}
}
?>
...
<div class="form-line destination_ex_div"  id="destination_ex_div">
...
<div style=" text-align:left;padding-top:10px;">
<input name="the_freight" id="the_freight_ex" value="EX" type="radio" >
...
<div class="form-group v_element delivery_div">
	<label for="SELECT DELIVERY">SELECT POINIT OF DELIVERY <span class="mand_msg">*</span></label>
		<div class="form-line">
			<select name="sl_delivery_id" id="sl_delivery_id" class="form-control sl_delivery_id" data-placeholder="Choose Point Of Delivery...">
			<option value="">Choose Point Of Delivery...</option>
			<option value="Single">Single</option>
			<option value="Multiple">Multiple</option>
			</select>
		</div>
		<label style="display: none;" for="SELECT ">SELECT PLANT LIST<span class="mand_msg">*</span></label>
		<div class="form-line" style="display: none;">
			<select name="plant_list" id="plant_list" class="form-control " data-placeholder="Choose Plant List...">
			<option value="">Choose Plant List...</option>
			<?php

				if ($res_plant1 && mysql_num_rows($res_plant1) > 0) {
					while ($row_plant = mysql_fetch_assoc($res_plant1)) {
						$the_plant_code = $row_plant["plant_code"];
						$the_plant_name = $row_plant["plant_name"];
						?>
						<option value="<?php echo $the_plant_code; ?>"><?php echo $the_plant_name; ?></option>
						<?php
					}
				} else {
					echo "<option>No plants found</option>";
				}
				?>
			</select>
		</div>
	<span class="err_msg" id="sl_delivery_id_error"></span>
</div>

<div class="form-group v_element">
<label for="DELIVERY REMARKS">DELIVERY REMARKS </label>
<input type="text" class="form-control delivery_remarks" id="delivery_remarks" name="delivery_remarks"  autocomplete="off">

<span class="err_msg" id="delivery_remarks_error"></span>
</div>
...
var xhrself,xhrsubdealer,xhrsselfshiptoparty;
...
jQuery('#sl_self_id').chosen({width:"100%",no_results_text:'Oops, no Ship to party dealer found!'});
...
//jQuery('#plant_list').chosen({width:"100%",no_results_text:'Oops, no Destination found!'});
...
var sl_self_id = jQuery.trim(jQuery("#sl_self_id").val());
...
var sl_delivery_id = jQuery.trim(jQuery("#sl_delivery_id").val());
//var plant_list = jQuery.trim(jQuery("#plant_list").val());
var delivery_remarks = jQuery.trim(jQuery("#delivery_remarks").val());
//alert(plant_list);
...
else if(order_for_type=="self")
{
	var selfCount="<?php echo count($dealer_data);?>";
	if(selfCount > 1 && sl_self_id==""){
		jQuery(".sd_self_loader").html("Please select Ship to party.");
	jQuery("#sl_self_id").focus();
	setTimeout(function(){
	jQuery(".sd_self_loader").html("");
	},5000);
	return false;
		}
}
...
return false;
}else if(the_freight=="EX" && sl_dump_id==""){
...
return false;
}else if(the_freight=="FOR" && sl_delivery_id==""){
	jQuery("#sl_delivery_id_error").html("Please select point of delivery.");
	jQuery("#sl_delivery_id").focus();
	setTimeout(function(){
		jQuery("#sl_delivery_id_error").html("");
		},5000);
		return false;
	}
	/*else if(the_freight=="FOR" && plant_list==""){
	jQuery("#sl_delivery_id_error").html("Please select Plant List.");
	jQuery("#sl_delivery_id").focus();
	setTimeout(function(){
		jQuery("#sl_delivery_id_error").html("");
		},5000);
...
if(selfCount >1)
{
	jQuery(".self_div").show();
	ship_to_loader_elmnt.html("");
}
else
{
	jQuery(".self_div").hide();
	...
	var user_destination = response.user_destination;
	jQuery("#consignee_name").val(user_name);
	jQuery("#consignee_address").val(user_address);
	jQuery("#destination_ex_div").html(user_destination);
...
timeout : 0
		...
		jQuery("#consignee_address").val("");
		jQuery("#phone_no").val("");
		jQuery("#destination_ex_div").html("");
		ship_to_loader_elmnt.html("");
		sd_loader_elmnt.html("");
	}
});

jQuery("#sl_self_id").change(function(){
	var sl_self_id = jQuery.trim(jQuery(this).val());
	if(sl_self_id!=""){
sd_loader_elmnt.html(img1);
if(xhrsselfshiptoparty && xhrsselfshiptoparty.readystate != 4){
xhrsselfshiptoparty.abort();
}
xhrsselfshiptoparty = jQuery.ajax({
url: 'ajax_get_user_details.php',
type: 'post',
dataType: 'json',
data: "dealer_id="+sl_self_id+"&user_type=Ship-to-Party-dealer",
success: function(response){
if(response.process_sts=="YES"){
var user_name = response.user_name;
var user_address = response.user_address;
