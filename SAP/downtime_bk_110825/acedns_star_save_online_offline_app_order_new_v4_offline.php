...
$conn2 = mysql_connect($remote_host, $remote_user, $remote_pass, true); // 'true' for new link
if (!$conn2) {
...
//SB_T_APPERPDO,SB_zorderdetailsp,SB_T_APPERPDO_TEMP
$live_t_dochallan = "SB_T_APPERPDO";
$live_t_apperpdo_temp = "SB_T_APPERPDO_TEMP";
...
//$t_apperpdo = "downtime_T_APPERPDO";
...
//echo"<pre>";print_r($sql_ckredius);die;
$res_ckredius = mysql_query($sql_ckredius, $conn);
...
//echo"<pre>";print_r($carry_forward_process_st);die;
	...
	if($cust_id!=''){
		...
		if($totress>0){
			...
			if($the_branch_code!=""){
				...
				$ress_bd = mysql_query($sqls_bd, $conn);
...
if($delivery_point != "" && $remarks_text!=""){

	$res_data = array("process_status"=>"YES","process_message"=>"Two column datas sent","data"=>$array_data);
...
//sk
//echo"<pre>";print_r($sql_bclck);die;
$res_bclck = mysql_query($sql_bclck, $conn);

$totres_bclck = mysql_num_rows($res_bclck);
//echo"<pre>";print_r($totres_bclck);die;
if($totres_bclck>0){
...
$resall2 = mysql_query($sqlall2, $conn);
...
//echo"<pre>";print_r($sql_dwp1);die;
$res_dwp1 = mysql_query($sql_dwp1, $conn);
...
$res_ck_reford = mysql_query($sql_ck_reford, $conn);
...
$delivery_point=$order_data_val["Delivery_point"] ? addslashes(trim($order_data_val["Delivery_point"])) : "";

$remarks_text=$order_data_val["Remarks_text"] ? addslashes(trim($order_data_val["Remarks_text"])) : "";

$plant_list=$order_data_val["plant_list"] ? addslashes(trim($order_data_val["plant_list"])) : "";
...
if($sub_dealer_code!=""){
		...
		$sqladditionaldata = "select `cust_type`,rds_tag from $customer_master where `customer_code`='$sub_dealer_code'";

		$resadditionaldata = mysql_query($sqladditionaldata, $conn);

		$totresadditionaldata = mysql_num_rows($resadditionaldata);

			if($totresadditionaldata>0){

				$rowadditionaldata = mysql_fetch_assoc($resadditionaldata);

				$customer_type = $rowadditionaldata["cust_type"] ? addslashes(trim($rowadditionaldata["cust_type"])) : "";

				$rds_tag = $rowadditionaldata["rds_tag"] ? addslashes(trim($rowadditionaldata["rds_tag"])) : "";



				if(strtolower($customer_type)=='ship to party-dealer' || strtolower($customer_type)=='shiptoparty-subdeale')

				{

					$additional_data_1 = show_SAP_code_from_customer_code_code($rds_tag);

				}

				else{

					$additional_data_1 = show_SAP_code_from_customer_code_code($sub_dealer_code);
...

	{
		...
		$additional_data_1=$SAP_customer_code;
...
$res_subord_utd = mysql_query($sql_subord_utd, $conn);

}
if(lIVE_INSERT=='TRUE'){
	//	live table data insert
	$sqlin = "insert into $live_t_dochallan (`ref_order_id_for_duplicate_ck`,`ERPORDERNO`,`ERPORDERDT`,`order_date`,`order_for`,`order_for_type`,`consignee_name`,`consignee_address`,`sub_dealer_code`,`dns_sub_dealer_code`,`customer_code`,`dns_customer_code`,`prod_code`,`dns_prod_code`,`prod_display_name`,`QTY`,`freight`,`branch_code`,`dns_branch_code`,`destination_code`,`destination_name`,`destination_address`,`phone_no`,`dump_status`,`dump_code`,`dump_name`,`order_from`,`order_by`,`dealer_truck`,`ref_sub_dealer_order_id`,`Delivery_point`,`Remarks_text`,`Additional_data_1`)

values('$ref_order_id_for_duplicate_ck','$erporderno','$erporderdt','$order_date','$order_for_new_in','$order_for_type','$consignee_name_of','$consignee_address_of','$sub_dealer_code','$dns_sub_dealer_code','$customer_code','$dns_customer_code','$prod_code','$dns_prod_code','$prod_dtld_desc','$qty','$freight','$branch_code','$dns_branch_code','$destination_code','$destination_name','$destination_address','$phone_no','$dump_status','$dump_code','$dump_name','$user_type','$login_user_id','$dealer_truck','$ref_sub_dealer_order_id','$delivery_point','$remarks_text','$additional_data_1')";
//echo"<pre>";print_r($sqlin);die;
$resin = mysql_query($sqlin, $conn2);
//echo"<pre>";print_r($resin);die;
	$created_last_id = mysql_insert_id($conn2);

	$new_order_id_val = str_pad($created_last_id, 7, "0", STR_PAD_LEFT);

	$live_apporderno = "SB".$new_order_id_val;

	//${'apporderno'.$cntslno}=$live_apporderno;

	$sqlupd1 = "update $live_t_dochallan set `APPORDERNO`='$live_apporderno' where `id`='$created_last_id'";
	//echo"<pre>";print_r($sqlupd1);die;
	$resupd1 = mysql_query($sqlupd1, $conn2);	
...
// Edited by Soumik Hazra - 03/01/2025 //
if (strtolower($delivery_point) == 'dot') {
	$delivery_point = 'DOT';
}
//////////////////////////////////////////

	$sqlinSAP="insert into $zorderdetails (`APPORDERNO`,`DATE`,`time`,`Cust_Code`,`Consignee_Code`,`Freight`,`DestinationCode`,`ProductCode`,`Qty`,`Unit`,`PLANT`,`OrderNo`,`Remarks`,`Delivery_point`,`Remarks_text`,`Additional_data_1`)

values('$apporderno','$SAP_order_date','$SAP_order_time','$SAP_customer_code','$SAP_sub_dealer_code','$freight','$the_SAP_destination','$dns_prod_code','$qty','TO','$the_SAP_plant','','$remarks','$delivery_point','$remarks_text','$additional_data_1')";
echo"<pre>";print_r($sqlinSAP);die;

	$resinSAP = mysql_query($sqlinSAP, $conn);
...
$resin_temp = mysql_query($sqlin_temp, $conn);

if(lIVE_INSERT=='TRUE'){
	$sqlinSAP="insert into $live_zorderdetails (`APPORDERNO`,`DATE`,`time`,`Cust_Code`,`Consignee_Code`,`Freight`,`DestinationCode`,`ProductCode`,`Qty`,`Unit`,`PLANT`,`OrderNo`,`Remarks`,`Delivery_point`,`Remarks_text`,`Additional_data_1`)

	values('$live_apporderno','$SAP_order_date','$SAP_order_time','$SAP_customer_code','$SAP_sub_dealer_code','$freight','$the_SAP_destination','$dns_prod_code','$qty','TO','$the_SAP_plant','','$remarks','$delivery_point','$remarks_text','$additional_data_1')";

		$resinSAP = mysql_query($sqlinSAP, $conn2);

	$sqlin_temp = "insert into $live_t_apperpdo_temp (`ref_order_id_for_duplicate_ck`,`APPORDERNO`,`ERPORDERNO`,`ERPORDERDT`,`order_date`,`order_for`,`order_for_type`,`consignee_name`,`consignee_address`,`sub_dealer_code`,`dns_sub_dealer_code`,`customer_code`,`dns_customer_code`,`prod_code`,`dns_prod_code`,`prod_display_name`,`QTY`,`freight`,`branch_code`,`dns_branch_code`,`destination_code`,`destination_name`,`destination_address`,`phone_no`,`dump_status`,`dump_code`,`dump_name`,`order_from`,`order_by`,`dealer_truck`,`ref_sub_dealer_order_id`) values('$ref_order_id_for_duplicate_ck','$live_apporderno','$erporderno','$erporderdt','$order_date','$order_for_new_in','$order_for_type','$consignee_name_of','$consignee_address_of','$sub_dealer_code','$dns_sub_dealer_code','$customer_code','$dns_customer_code','$prod_code','$dns_prod_code','$prod_dtld_desc','$qty','$freight','$branch_code','$dns_branch_code','$destination_code','$destination_name','$destination_address','$phone_no','$dump_status','$dump_code','$dump_name','$user_type','$login_user_id','$dealer_truck','$ref_sub_dealer_order_id')";

$resin_temp = mysql_query($sqlin_temp, $conn2);
}
...
if(count($the_fetched_array)>0){
	...
	if(array_key_exists("email",$the_fetched_array)){
		...
		$message_old = '<br><b>App Order No: </b> '.$apporderno.'<br>
		...
		$rsdealeremp=mysql_query($sqldealeremp, $conn);
		$email_hierarchy='';
		...
		//$emp_hierarchy_condition='c1.emp_code IN('.$employee_hierarchy.')';
			...
			$rsemailhierarchy=mysql_query($sqlemailhierarchy, $conn);
		...
		$rsbroker=mysql_query($sqlbroker, $conn);
		while($rowbroker=mysql_fetch_array($rsbroker))
		{
			...
			$rsemailbroker=mysql_query($sqlemailbroker, $conn);
...
/*$final_email=substr($email_hierarchy,0,-1).','.substr($email_broker,0,-1).','.'emovesfa@starcement.co.in'.','.'antarabanerjee@starcement.co.in';*/
	...
	else 	$final_email='samirdas@starcement.co.in';
	$curr_date_format = date("jS M, y",strtotime($order_date));


		$subject = "Order for branch ".strtoupper($the_fetched_branch_name)." ";
		...
		$message="Order received from $fetched_customer_name @ $hour:$minute\n";

		$message.= '<br><b>App Order No: </b> '.$apporderno.'<br>

					<b>DATE: </b> '.$curr_date_format.'<br>

					<b>Branch Name: </b> '.strtoupper($the_fetched_branch_name).'<br>
...
/*$final_email=substr($email_hierarchy,0,-1).','.substr($email_broker,0,-1).','.'emovesfa@starcement.co.in'.','.'antarabanerjee@starcement.co.in';*/
	$final_email=substr($email_hierarchy,0,-1).','.substr($email_broker,0,-1).','.'samirdas@starcement.co.in,abhishekd@coral.in';
