...
$customer_id=$_SESSION["sswa_user_id"];
$pgsql = "select $t_apperpdo.*,$customer_master.`branch_code` from $t_apperpdo left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code` where $t_apperpdo.`APPORDERNO`!=''  AND $customer_master.`customer_code`='".$customer_id."'   $new_whr_str ";
}else{
...
if($_SESSION["sswa_user_type"]=="DEALER"){
$customer_id=$_SESSION["sswa_user_id"];
$pgsql = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id` from $t_apperpdo
left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code`

where $t_apperpdo.`APPORDERNO`!='' AND $customer_master.`customer_code`='".$customer_id."'  $new_whr_str order by $t_apperpdo.`id` desc limit $start_from,$limit";
}else{
$pgsql = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id` from $t_apperpdo
left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code`
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                         <h2>Order List (<?php echo $total_pgres;?>) &nbsp;&nbsp;&nbsp;<span class="rpt_loader"></span> &nbsp;&nbsp;<?php if($_SESSION["sswa_user_type"]=="SP"){?><a href="export_order2.php?get_type=all<?php echo $export_filtered_str;?>" class="btn bg-red waves-effe">Export&nbsp;all&nbsp;orders</a> <?php } ?> &nbsp;  <!-- <a href="export_order2.php?get_type=pending<?php echo $export_filtered_str;?>" class="btn bg-red waves-effe">Export&nbsp;pending&nbsp;orders</a> &nbsp; <a href="export_dispatched_order_with_invoice.php?exp=yes<?php echo $export_filtered_str;?>" class="btn bg-red waves-effe">Export&nbsp;dispatched&nbsp;order&nbsp;with&nbsp;invoice</a>
...
<?php
if($_SESSION["start_user_type"]=="MANAGER" && $order_show_branch!=""){
	$sql1 = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id`,$plant_list.plant_name from $t_apperpdo
	left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code`
...
//echo"<pre>";print_r($_SESSION["sswa_user_type"]);die;

if($_SESSION["sswa_user_type"]=="DEALER"){
$customer_id=$_SESSION["sswa_user_id"];
$sql1 = "select $t_apperpdo.*,$customer_master.`branch_code`,$customer_master.`customer_id` from $t_apperpdo
left join $customer_master on $t_apperpdo.`customer_code`=$customer_master.`customer_code`
...
?>
	...
	<!-- Table Cell with Edit and Update Functionality -->
	 <td><?php echo $erporder_no; ?></td>
	<!-- <td id="erporder_cell_<?php echo $sl_no ; ?>">
		<span class="display-text"><?php echo $erporder_no; ?></span>
		<input type="text" class="edit-input" value="<?php echo $erporder_no; ?>" style="display:none;" />
		<button class="edit-btn btn bg-red">Edit</button>
		<button class="update-btn btn bg-red" style="display:none;">Update</button>
	</td> -->
...
jQuery('#ds_code').chosen({width:"100%",no_results_text:'Oops, no Dump Name found!'});
jQuery('#trn_branch_id').chosen({width:"100%",no_results_text:'Oops, no Dump Name found!'});
</script>
...
$(document).ready(function() {
    $('.edit-btn').click(function() {
        ...
        cell.find('.update-btn').show();
    });

    $('.update-btn').click(function() {
        let cell = $(this).closest('td');
        let newVal = cell.find('.edit-input').val();
        let allocationId = cell.attr('id').split('_')[2]; // get allocation_id

        $.ajax({
            url: 'update_erporder.php',
            type: 'POST',
            data: {
            ...
            success: function(response) {
                ...
                if (response === 'success') {
                    cell.find('.display-text').text(newVal).show();
                    cell.find('.edit-input').hide();
                    cell.find('.edit-btn').show();
                    cell.find('.update-btn').hide();
					alert('Sale Order No Update Successfully!');
                } else {
                    alert('Update failed!');
                }
            }
