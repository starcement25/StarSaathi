...
$sql_sho = "select `branch_code` from $branch_consumer_scheme_status where `branch_code`='$the_brnch_code'";
$res_sho=mysql_query($sql_sho);
$tot_res_sho = mysql_num_rows($res_sho);
if($tot_res_sho>0){
$sql_upd="update $branch_consumer_scheme_status set `consumer_scheme_status`='$the_status',`download_time`='$download_time' where `branch_code`='$the_brnch_code'";
$res_upd=mysql_query($sql_upd);
}else{
$sql_in="insert into $branch_consumer_scheme_status (`branch_code`,`consumer_scheme_status`,`download_time`) values ('$the_brnch_code','$the_status','$download_time')";
$res_in=mysql_query($sql_in);	
}
$res_data = array("process_status"=>"YES","process_message"=>"Status updated successfully.");
}else{
	$sql_dlt="delete from $$branch_consumer_scheme_status where `branch_code`='$the_brnch_code'";
