...
<section class="content">
        <div class="container-fluid">
            ...
            <div class="header">
                ...
                <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 add_top_bottom_padding">
					<span style="text-align:center;display: block;">&nbsp;</span>
                <a href="javascript:void(0);" class="btn bg-red waves-effect dai_btn">Download All Invoice</a>
                </div>
				<div class="col-lg-1 col-md-1 col-sm-12 col-xs-12 add_top_bottom_padding">
					 <span style="text-align:center;display: block;">&nbsp;</span>
	...
	<input type="hidden" name="all_invoice_no" id="all_invoice_no"  value="" />
...
$url_ck1 = 'https://starfiori.starcement.co.in:44300/sap/opu/odata/sap/ZSD_CUSTOMER_BULK_INVOICE_SRV/ZSD_CUSTOMER_INVOICESet?$format=json'.str_replace(" ","%20",$the_filter);
// echo $url_ck1;
...
<div class=\"container\" style=\"width:100%;\">
  ...
  <tr>
   ...
   <td style=\"width:60%;text-align:center;\"> <p style=\"text-align:center; font-size:10px;margin: 0px; padding-top:2px;\"><strong>$CompName1</strong></p>
   	...
   	<td style=\"width:20%;text-align:right;\" ><img src=\"https://starsaathi.com/SAP/dashboard/images/qr.jpg\" alt=\"Star\" width=\"50px\" style=\"position: absolute; top:10px; right:10px;\"/></td>
   </tr>
   </table>
...
if($_REQUEST['mode']=='allinvoiceprint'  && $_REQUEST['all_invoice_no']!='')
	{
set_time_limit(0);
ini_set('memory_limit', '-1');
$output = "";
$ain_arr = explode(",",$_REQUEST['all_invoice_no']);
$tot_inv = count($ain_arr);
$cnt_inv = 1;
		if($tot_inv>0){
			$output .= "<html><body style=\"border:1px solid #333;\">";
		foreach($ain_arr as $the_inv_no){
		foreach(${'invoice_print_array_'.$the_inv_no} as $invoice_details_val)
		{
...
<div class=\"container\" style=\"width:100%;\">
  ...
  <td style=\"width:50%;font-size:8px; border-left:1px solid #333;padding-left:10px;\">Invoice No.: <strong>".$the_inv_no."</strong><br/>
  Invoice Dt: <strong>$InvoiceDt_format</strong><br/>
  Cust PO No.: <strong>$CustPo</strong> <span style=\"padding-left:70px;\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Cust PO Dt: <strong>$CustPoDt_format</strong></span>
  </td>
  </tr>
...
$curr_date = date("jS_M_Y_h_m_i_s_A");
$the_file_name = "invoice_".$curr_date.".pdf";
//define ('PDF_MARGIN_RIGHT', 4);
$pdf=new LongTableTCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
//$pdf->SetMargins(PDF_MARGIN_LEFT, PDF_MARGIN_TOP, PDF_MARGIN_RIGHT);
...
function invoice_all_print(allinvoiceno)
{
//alert(invoiceno);
document.getElementById('modeval').value='allinvoiceprint';
document.getElementById('all_invoice_no').value=allinvoiceno;
document.invoiceform.submit();
}
...
<?php
$all_inv_str = "";
if(count($all_inv_arr)>0){
$all_inv_str = implode(",",$all_inv_arr);	
}
?>

var all_inv_ids = "<?php echo $all_inv_str;?>";

jQuery(".dai_btn").click(function(){
if(all_inv_ids==""){
alert("No invoice record found.");
}else{
invoice_all_print(all_inv_ids);
}
