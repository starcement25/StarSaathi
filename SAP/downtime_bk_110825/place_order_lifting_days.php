...
$add_page_name = "place_order_lifting_days.php";
$page_name = "place_order_lifting_days.php";
$cnt = 0;
$countrow = 1;
...
$pgsql = "SELECT $branch_master.`branch_code` FROM $branch_master left join $place_order_lifting_days on $branch_master.`branch_code`=$place_order_lifting_days.`branch_code` $new_whr_str";
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                          <h2>Branch Wise Lifting Days (<?php echo $total_pgres;?>)&nbsp;&nbsp;<!--<a href="export_sp_destination.php?get_type=all" class="btn bg-red waves-effe">Export&nbsp;all</a>-->
...
<span style="display:block; clear:both;"></span>
 <div class="table-responsive">
   ...
   <input type="hidden" name="mode" value="days_update" />
<?php
	if($_REQUEST['mode']=='days_update')
	{
	...
	for($i=0;$i<count($the_branch_code_val);$i++)
	{
	  ...
	  $days_val=$_REQUEST["days_".$the_branch_code];
	 if($days_val=='')	$days_val=0;
	  //if($allocation_days_val!=''){
		  $sqlselbranch="SELECT branch_code FROM place_order_lifting_days WHERE branch_code='".$the_branch_code."'";
		  $rsselbranch=mysql_query($sqlselbranch);
		  $countselbranch=mysql_num_rows($rsselbranch);
		  if($countselbranch==0)
			 {
			  $ins_sql="INSERT INTO place_order_lifting_days SET branch_code ='$the_branch_code',days ='".$days_val."',upload_time =CURRENT_TIMESTAMP()";
			 mysql_query($ins_sql) or die(mysql_error()." Error in inser allocation days.");
		  
		  }
		  else{
			$upd_sql="UPDATE place_order_lifting_days SET days ='$days_val',upload_time =CURRENT_TIMESTAMP() WHERE branch_code = '".$the_branch_code."'";
	  ...
	  //}
		...
		?>
         <table class="table table-bordered table-striped table-hover">
				<tr> 
					<td align="center" ><b>Place Order days updated successfully</b></td>
				</tr>
				</table>
        <?php
...
$sql1 = "SELECT $branch_master.`branch_code`,$branch_master.`dns_branch_code`,$branch_master.`branch_name`,$place_order_lifting_days.`days` FROM $branch_master left join $place_order_lifting_days on $branch_master.`branch_code`=$place_order_lifting_days.`branch_code` $new_whr_str order by $branch_master.`branch_name` asc limit $start_from,$limit";
...
<input type="text" class="form-control" name="days_<?php echo $the_branch_code;?>" id="days_<?php echo $the_branch_code;?>" the_brnch_code="<?php echo $the_branch_code;?>" style="width: 50%" value="<?php echo $days;?>">
