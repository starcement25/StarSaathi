...
$sql_sho = "select `customer_code` from $dealer_security_ledger_status where `customer_code`='$the_customer_code'";
$res_sho=mysql_query($sql_sho);
$tot_res_sho = mysql_num_rows($res_sho);
if($tot_res_sho>0){
$sql_upd="update $dealer_security_ledger_status set `security_ledger_status`='$the_status',`download_time`='$download_time' where `customer_code`='$the_customer_code'";
$res_upd=mysql_query($sql_upd);
}else{
$sql_in="insert into $dealer_security_ledger_status (`customer_code`,`security_ledger_status`,`download_time`) values ('$the_customer_code','$the_status','$download_time')";
$res_in=mysql_query($sql_in);	
}
$res_data = array("process_status"=>"YES","process_message"=>"Status updated successfully.");
}else{
	$sql_dlt="delete from $dealer_security_ledger_status where `customer_code`='$the_customer_code'";
