...
$sql1 = "SELECT `id`,`APPORDERNO`,`ERPORDERNO`,`STATUS`,`QTY`,`customer_code`,`dns_customer_code`,`prod_code`,`dns_prod_code`,`freight`,`destination_code`,`destination_name` FROM $t_apperpdo where `STATUS`='DO approved'  AND SUBSTRING(`order_date`,1,10) >='2024-08-01' AND SUBSTRING(`order_date`,1,10) <='2024-09-09'  order by `id` asc";
...
if($totres1>0){
	...
	if($so_num!=""){
		...
		echo $sql_upd = "update $t_apperpdo set `ERPORDERNO`='$so_num',`ERPORDERDT`='$erdat_date_format' where `APPORDERNO`='$the_app_ord_no' ";
