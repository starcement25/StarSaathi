...
<table width="100%" align="center" border="0" cellspacing="0" cellpadding="4">
	...
	<tr>
		...
		<!--td width="15%" align="left" style="text-align:left;"><?php /*$_REQUEST[pageNo];if($_REQUEST[pageNo]!=1 && $_REQUEST[pageNo]!=''){ ?>
...
function return_employee_upper_hierarchy($emp_code) {
    ...
    // Always include the current emp_code at the end
    if (!in_array("'".$emp_code."'", $emphierarchy)) {
...
function employee_upper_hierarchy_details($emp_code, &$emphierarchy, &$visited = array()) {
    // Prevent infinite loop by checking visited
    if (in_array($emp_code, $visited)) {
        return;
    }
    $visited[] = $emp_code;

    $sqlemphierarchy = "SELECT reporting_to FROM employee_master 
    ...
    if ($cntemphierarchy > 0) {
        while ($rowemphierarchy = mysql_fetch_array($rsemphierarchy)) {
            ...
            if (strpos($reporting_to, ',') !== false) {
                $reporting_to_Arr = explode(',', $reporting_to);
                foreach ($reporting_to_Arr as $rep) {
                    $rep = trim($rep);
                    if (!in_array("'".$rep."'", $emphierarchy)) {
                        $emphierarchy[] = "'".$rep."'";
                    }
                    employee_upper_hierarchy_details($rep, $emphierarchy, $visited);
                }
            } else {
                $reporting_to = trim($reporting_to);
                if (!in_array("'".$reporting_to."'", $emphierarchy)) {
                    $emphierarchy[] = "'".$reporting_to."'";
                }
                employee_upper_hierarchy_details($reporting_to, $emphierarchy, $visited);
            }
        }
    }
}
