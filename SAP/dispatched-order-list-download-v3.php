...
if ($the_customer_code == "") {
	$res_data = array("process_status" => "NO", "process_message" => "Something went wrong.", "rssd_allocation_days" => $rssd_allocation_days, "dispatched_order_data" => $dispatched_order_data);
} else {
	...
	AND dns_prod_code='$dns_prod_code'";
				...
				if ($totall3 > 0) {
					...
					while ($row113 = mysql_fetch_assoc($resall3)) {
						...
						$sqlall4 = "select SUM(CHALLANQTY) As tot_challan_qty from $T_DOCHALLAN where `dns_customer_code`='$dns_customer_code' and `APPORDERNO`='$order_id'";
				$resall4 = mysql_query($sqlall4);
				$totall34= mysql_num_rows($resall4);
						$rowll34=mysql_fetch_array($resall4);
						
						$tot_challan_qty=$rowll34['tot_challan_qty'];
						
						$sqlallocationqty = "select SUM(allocation_qty) tot_allocation_qty from $allocation_details where `customer_id`='$the_customer_code' and `dns_prod_code`='$dns_prod_code' AND  	dispatch_date='$ch_challandt_part' AND APPORDERNO='$order_id' GROUP BY APPORDERNO,dns_prod_code,dispatch_date";
						...
						$available_allocation_qty=$tot_challan_qty-$totresallocationqty;

						$dispatched_challan_data[] = array("challanno" => $ch_challanno, "dispatch_date" => $ch_challandt_part, "dispatch_qty" => $ch_challanqty,"available_allocation_qty" => $available_allocation_qty);
			...
			$res_data = array("process_status" => "NO", "process_message" => "No Dispatched order found.", "rssd_allocation_days" => $rssd_allocation_days);
		}
	} else {
		$res_data = array("process_status" => "NO", "process_message" => "Dealer record not found.",  "rssd_allocation_days" => $rssd_allocation_days, "dispatched_order_data" => $dispatched_order_data);
	}
}
