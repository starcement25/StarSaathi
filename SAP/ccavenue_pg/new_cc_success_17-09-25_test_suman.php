...
$retry= "";
	...
	//echo"<pre>";print_r($encResponse);die;
	...
	$order_status = $response['order_status']; // Success, Aborted, Failure
...
$cust_email = "";	
$sql_em = "select `customer_code` from $t_main_order_pop where `order_id`='$order_id'";
...
$totres_em = mysql_num_rows($res_em);
	if($totres_em>0){
 	...
 	$sql_sel = "SELECT `tran_datetime`,`payment_mode`,`mobile`,`email`,`amount`,`bank_ref_no` FROM $t_main_order_pop where `order_id`='$order_id'";
