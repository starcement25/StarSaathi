...
$T_DOINVOICE = "T_DOINVOICE";
	$sqls = "select `appointment_date`,`acedns` from $customer_master where `customer_id`='$customer_id_check'";
		...
		if($totress>0){
			...
			$acedns = $rows["acedns"] ? trim($rows["acedns"]) : "";
		}
		//add condition sk 10-09-25
		if($acedns=='N'){
			$res_data = array("process_status"=>"NO","process_message"=>"Customer Inactive");
			echo json_encode($res_data);
			die;
		} 
		//add end condition sk 10-09-25

		//echo "$customer_id Raw appointment_date from DB: '$appointment_date'<br>";die;
		...
		//echo $INVDT.'=='.strtotime($dateObj->format('Ymd'));die;
		if ($dateObj !== false && isset($INVDT) && $INVDT !='') {
			$formattedDate = $dateObj->format('Ymd'); // e.g., "20220625"

			if (strtotime($formattedDate) <= strtotime($INVDT)) {
...
if ($row['total'] > 0) {
	...
	// $sqlchk = "select  order_id from $allocation_details where order_id='$order_id' and delete_at='0' and prod_desc='$prod_desc' "; order_id     = '$order_id' AND
			$sqlchk = "
			SELECT 1
			...
			AND inv_qty      = '$inv_qty'
			AND delete_at    = '0'
			AND date_and_time >= DATE_SUB(NOW(), INTERVAL 1 MINUTE)
			LIMIT 1
		";
