...
// $allo = "SELECT * FROM `allocation_details_invoicewise` WHERE `customer_id` LIKE '%c/0%' ORDER BY `allocation_id` DESC";
// $allo_q = mysql_query($allo);
// $totallo_q = mysql_num_rows($allo_q);
// if ($totallo_q > 0) {
// 		while ($allo_result = mysql_fetch_assoc($allo_q)) {
// 			$order_id = $allo_result["APPORDERNO"] ? trim($allo_result["APPORDERNO"]) : "";
// 			$allocation_id = $allo_result["allocation_id"] ? trim($allo_result["allocation_id"]) : "";
// 			$customer_code = $allo_result["customer_id"] ? trim($allo_result["customer_id"]) : "";
// 			$sub_dealer_id = $allo_result["sub_dealer_id"] ? trim($allo_result["sub_dealer_id"]) : "";

// 			$sql_cust = "SELECT `customer_id` FROM $customer_master where `customer_code`='" . $customer_code . "'";
...
// 			$sql_subdealer_cust = "SELECT `customer_id` FROM $customer_master where `customer_code`='" . $sub_dealer_id . "'";
// 			//echo"<pre>";print_r($sql_cust);die;
// 			$res_subdealer_cust = mysql_query($sql_subdealer_cust);
// 			$tot_res_subdealer_cust = mysql_num_rows($res_subdealer_cust);
// 			if ($tot_res_subdealer_cust > 0) {

// 				$row_subcust = mysql_fetch_array($res_subdealer_cust);
// 				$sub_customer_id = $row_subcust['customer_id'];
// 			}

// 			if ($tot_res_cust > 0 && $tot_res_subdealer_cust > 0){
// 				 $sqlinactivate = "UPDATE allocation_details_invoicewise 
//                           SET `customer_id`='".$customer_id."', `sub_dealer_id`='".$sub_customer_id."'
//                           WHERE allocation_id='" . mysql_real_escape_string($allocation_id) . "'";
// 				//echo $sqlinactivate.'####';
	...
	where $t_apperpdo.dns_customer_code='" . $dns_customer_code . "' and `status`='Dispatched'  order by $t_apperpdo.`order_date` desc";
		...
		if ($totall2 > 0) {
			while ($row112 = mysql_fetch_assoc($resall2)) {
				...
				$logArr[] = array('APPORDERNO'=>$order_id, 'invoice_count'=>$totall3,'query'=>$sqlall3);
			...
			$t_apperpdo_offline="T_APPERPDO_OFFLINE";
				...
				while ($row = mysql_fetch_array($result)) {
					...
					$order_id = $row["ERPORDERNO"] ? trim($row["ERPORDERNO"]) : "";
					if($month_year_val!=''){
						$date_condition=" AND SUBSTRING($T_DOINVOICE.INVDT,1,6)='".$month_year_val."'";
					}
				...
				if ($order_date_off != "") {
					...
					$sqlall3 = "select * from $T_DOINVOICE where `ERPORDERNO`='$order_id' AND INVNO NOT IN(SELECT DISTINCT inv_no FROM $allocation_details WHERE `inv_cancl`='yes'  )
				".$date_condition;
				...
				$totall3 = mysql_num_rows($resall3);
				 if ($totall3 > 0) {
				 	...
				 	while ($row1 = mysql_fetch_assoc($resall3)) {
						...
						$INVDT_formatted=date('Y-m-d',strtotime($INVDT));
						$sqlallocationqty = "select SUM(allocation_qty) tot_allocation_qty from $allocation_details where `customer_id`='$the_customer_code'  and (inv_no='$INVNO' OR inv_no='$apporder_no_chtl') and delete_at='0' ";
			...
			//end  offline data  sk190525
				...
				$res_data = array("process_status" => "YES", "process_message" => "Success.", "rssd_allocation_days" => $rssd_allocation_days, "dispatched_order_data" => $dispatched_order_data,'log'=>$logArr);
