...
$whr_str .= "$aand ($customer_master.`dns_customer_code` like '%$search_array_val%' or $customer_master.`customer_name` like '%$search_array_val%' or $customer_master.`customer_id` like '%$search_array_val%' ) ";
	...
	if($search_array_key=="sl_status"){
		if($search_array_val!=''){
			...
			{
				$whr_str .= " $aand ($dealer_reward_status.`dealer_status`='$search_array_val' OR $customer_master.`dns_customer_code` NOT IN(SELECT emp_code FROM $dealer_reward_status ))";
			}
...
if ($cust_type == 'Dealer') {
$pgsql = "SELECT $customer_master.`customer_code` FROM $customer_master left join $dealer_reward_status on $customer_master.`dns_customer_code`=$dealer_reward_status.`emp_code` WHERE $customer_master.cust_type='Dealer' $new_whr_str";
} else {
	$pgsql = "SELECT $customer_master.`customer_code` FROM $customer_master left join $dealer_reward_status on $customer_master.`dns_customer_code`=$dealer_reward_status.`emp_code` WHERE $customer_master.cust_type='Sub Dealer' $new_whr_str";
}
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                          <h2>Star Saathi Rewards Status&nbsp;&nbsp;<!--<a href="export_sp_destination.php?get_type=all" class="btn bg-red waves-effe">Export&nbsp;all</a>-->
...
$sql1 = "SELECT $customer_master.`customer_code`,$customer_master.`dns_customer_code`,$customer_master.`customer_name`,$dealer_reward_status.`dealer_status` FROM $customer_master left join $dealer_reward_status on $customer_master.`dns_customer_code`=$dealer_reward_status.`emp_code` WHERE $customer_master.cust_type='Dealer' $new_whr_str order by $customer_master.`customer_name` asc limit $start_from,$limit";
...
if($totres1>0){
	while($row1=mysql_fetch_assoc($res1)){
		...
		$dealer_status = $row1["dealer_status"];
		// if($dealer_status=='') $dealer_status='INACTIVE';
		if($dealer_status=='') $dealer_status='ACTIVE';
...
if(count($pg_sts_arr)>0){
	foreach($pg_sts_arr as $pg_sts_arr_val){
        ...
        <option value="<?php echo $the_key_val;?>" <?php if($the_key_val==$dealer_status){?> selected="selected" <?php } ?>><?php echo $the_title_val;?></option>
        <?php
	}
...
$sql2 = "SELECT $customer_master.`customer_code`,$customer_master.`dns_customer_code`,$customer_master.`customer_name`,$dealer_reward_status.`dealer_status` FROM $customer_master left join $dealer_reward_status on $customer_master.`dns_customer_code`=$dealer_reward_status.`emp_code` WHERE $customer_master.cust_type='Dealer' $new_whr_str order by $customer_master.`customer_name` asc";
...
jQuery(function(){
	...
	jQuery(".cwts_sel").change(function(){
		...
		// console.log("the_customer_code: "+the_customer_code);
