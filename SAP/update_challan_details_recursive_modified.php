...
for($i=1;$i<=10;$i++)
{
	 $prev_date_time_val = date('Y-m-d', strtotime("-$i days,$curr_date"));
...
//echo"<pre>";print_r($body_for_mcode1);die;
...
if($InvNo!=""){
	...
	$res_clupd = mysql_query($sql_clupd);
		$sql="UPDATE $T_DOINVOICE SET `DRIVERNO` ='$DriverNo'  WHERE `ERPORDERNO`='$SoNo'";
		 mysql_query($sql);
	$upd_cnt++;
	}else{
