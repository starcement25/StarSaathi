	...
	while($row11=mysql_fetch_assoc($resall)){
		...
		if($the_actual_ord_id!=""){
			...
			if($totall2>0){
				...
				$ch_driverno = $row1["DRIVERNO"] ? trim($row1["DRIVERNO"]) : "";
				
				$destination = $row1["destination"];
				$customer_code = $row1["customer_code"];
				/*
				//add line sk 010725  driver no
				// Query to get DRIVERNO
						$query = "SELECT DRIVERNO FROM T_DOCHALLAN WHERE ERPORDERNO = '".$erporder_no_chtl."' LIMIT 1";
						$result = mysql_query($query);

						if ($result && mysql_num_rows($result) > 0) {
							$row = mysql_fetch_assoc($result);
							$driverno = $row['DRIVERNO'];
							//echo "DRIVERNO: " . $driverno;
						}
				//end add line sk 010725
				*/
				$order_invoice_data[] = array("ch_uid"=>$ch_uid,"apporderno"=>$apporder_no_chtl,"erporderno"=>$erporder_no_chtl,"challanno"=>$challanno_chtl,"invno"=>$INVNO,"invdt"=>$INVDT,"prod_display_name"=>$prod_display_name_chtl,"invqty"=>$INVQTY,"customer_code"=>$customer_code,"truckno"=>$TRUCKNO,"driverno"=>$ch_driverno,"destination"=>$destination);
