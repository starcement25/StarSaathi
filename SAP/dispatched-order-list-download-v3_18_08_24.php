...
$rssd_allocation_days='';
if ($the_customer_code == "") {
	$res_data = array("process_status" => "NO", "process_message" => "Something went wrong.", "rssd_allocation_days" => $rssd_allocation_days , "dispatched_order_data" => $dispatched_challan_data);
} else {
	...
	where $T_DOCHALLAN.dns_customer_code='".$dns_customer_code."' and $T_DOCHALLAN.APPORDERNO!=''  and $T_DOCHALLAN.APPORDERNO IS NOT NULL GROUP BY $T_DOCHALLAN.APPORDERNO,$T_DOCHALLAN.dns_prod_code,SUBSTRING($T_DOCHALLAN.CHALLANDT,1,10) order by $T_DOCHALLAN.`CHALLANDT` desc";
		...
		if ($totall2 > 0) {
			...
			$res_data = array("process_status" => "YES", "process_message" => "Success.", "rssd_allocation_days" => $rssd_allocation_days, "dispatched_order_data" => $dispatched_challan_data);
		} else {
			$res_data = array("process_status" => "NO", "process_message" => "No Dispatched order found." ,  "rssd_allocation_days" => $rssd_allocation_days);
		}
	} else {
		$res_data = array("process_status" => "NO", "process_message" => "Dealer record not found.",  "rssd_allocation_days" => $rssd_allocation_days, "dispatched_order_data" => $dispatched_challan_data);
	}
}
