...
//for testing
$sqlin = "SELECT allocation_id, dns_prod_code, prod_desc, allocation_qty, date_and_time, order_id,sub_dealer_id,inv_no,inv_qty,inv_date   FROM $allocation_details WHERE $user_type_clause $year_month_condition order by date_and_time desc";
    ...
    if (!$resin) {
        ...
        if (mysql_num_rows($resin) > 0) {
            ...
            while ($row = mysql_fetch_assoc($resin)) {
                ...
                $allocation_data[] = array("allocation_id" => $allocation_id, "prod_desc" => $prod_desc, "allocation_qty" => $allocation_qty, "date_and_time" => $date_and_time, "order_id" => $order_id, "inv_no" => $inv_no, "inv_date" => $inv_date, "counter_name" => $customer_name);
            }
