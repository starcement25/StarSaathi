	...
	where $T_DOCHALLAN.dns_customer_code='".$dns_customer_code."' and $T_DOCHALLAN.APPORDERNO!=''  and $T_DOCHALLAN.APPORDERNO IS NOT NULL GROUP BY $T_DOCHALLAN.dns_prod_code,SUBSTRING($T_DOCHALLAN.CHALLANDT,1,10) order by $T_DOCHALLAN.`CHALLANDT` desc";
		...
		if ($totall2 > 0) {
			while ($row112 = mysql_fetch_assoc($resall2)) {
						...
						$dispatched_challan_data[] = array("dns_prod_code" => $dns_prod_code,"prod_display_name" => $prod_display_name,"challanno" => $ch_challanno, "dispatch_date" => $ch_challandt, "dispatch_qty" => $ch_challanqty,"available_allocation_qty" => $available_allocation_qty);
					}
