...
$customer_id_custom=$the_customer_id;
}else{
$the_customer_id = "";
}
...
foreach($prod_qty_arr as $prod_qty_arr_key2=>$prod_qty_arr_val2){
	...
	if($the_prod_qty_arr_val!=""){
		if(is_numeric($the_prod_qty_arr_val)){
			$gen_order_data_arr[] = array("apporderno"=>"","erporderno"=>"","erporderdt"=>"","order_for"=>$order_for,"order_for_type"=>$order_for_type,"customer_code"=>$sswa_selected_customer_code,"sub_dealer_code"=>$sl_sd_id,"prod_code"=>$the_prod_qty_arr_key,"qty"=>$the_prod_qty_arr_val,"freight"=>$the_freight,"destination_code"=>$the_dest_code,"destination_name"=>$destination_name,"destination_address"=>$destination_name,"phone_no"=>$phone_no,"dump_status"=>$dump_status,"dump_code"=>$sl_dump_id,"dump_name"=>$dump_name,"Delivery_point"=>$sl_delivery_id,"Remarks_text"=>$delivery_remarks);
		}
	}
}
...
$url_ck = BASE_URL . "acedns_star_save_online_offline_app_order_new_v4.php";
...
$url_ck1 =BASE_URL . "ship-to-party-master-txt-V2.php?emp_code=$sswa_selected_customer_code&user_type=dealer";
	//echo"<pre>";print_r($url_ck1);
	...
	$url_ck2 =BASE_URL . "ship-to-party-master-txt-V2.php?emp_code=$sswa_selected_customer_code&user_type=sub-dealer";
	//echo"<pre>";print_r($url_ck2);
...
<?php
			$base_url = 'https://starsaathi.com/SAP/' ; // Replace with actual DEV_BASE_URL
				//$customer_code = "1000000301"; // Replace with actual customer code
				//$customer_code = trim($the_customer_id); // Replace with actual customer code

				echo $url = $base_url . "fpx-delaer-truck-list.php?customer_code=" . urlencode($customer_id_custom);

				// Option 1: Using file_get_contents (requires allow_url_fopen = On)
				$response = file_get_contents($url);

				// Option 2 (Alternative): Using cURL (uncomment if file_get_contents doesn't work)
				...
				// Check if truck_data is not empty
				if (!empty($data['truck_data'])) {
