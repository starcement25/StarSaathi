...
class ProductController extends Controller{
  ...
  public function productmasterincremental(Request $request){
      ...
      $count=count($sqlquery);
      	...
      	if($count>0){
      		...
      		while($rowproduct = mysql_fetch_array($result)){
      			if($state_branch_wise_TD=='yes'){
      				$rowproduct=$CUTDB->select("SELECT TD FROM state_branch_product_wise_TD WHERE state_code='".$state_code."' AND
      							branch_code='".$rowproduct['branch_code']."' AND prod_code='".$rowproduct['prod_code']."'
      							AND SUBSTRING(download_time,1,10)='".$date."' ORDER BY download_time DESC LIMIT 0,1")[0];
        ...
        $url = url('/api/v1/productmaster?nick_name='.$nick_name.'&emp_code='.$emp_code.'&last_update_time='.$last_update_time.'&data_download_time='.$data_download_time.'&incremental_download='.$incremental_download);
