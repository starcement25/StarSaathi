...
class EmpMasterController extends Controller{
  ...
  public function empmastertxt(Request $request){
      ...
      if($isverify==1){
        ...
        if($sale=='yes'){
          ...
          $sqlquery=$CUTDB->select("SELECT emp_code,emp_name,sale_access,reporting_to,designation,vertical_value,branch_code,state,zone,acedns FROM employee_master WHERE
          			branch_code IN($branch_code_list) ORDER BY emp_name ASC");
        }
        else if($nick_name='MAITHAN'){
        	$sqlquery=$CUTDB->select("SELECT emp_code,emp_name,sale_access,reporting_to,designation,vertical_value,branch_code,state,zone,acedns,lower_leaves
        ...
        if($count>0){
        		...
        		foreach($sqlquery as $rowemp ){
        			  ...
        			  $cntemphierarchy=count($sqlemphierarchy);
         ...
         $url = url('/api/v1/empmaster?nick_name='.$nick_name.'&emp_code='.$emp_code.'&device_id='.$device_id.'&last_update_time='.$last_update_time.'&incremental_download='.$incremental_download);
