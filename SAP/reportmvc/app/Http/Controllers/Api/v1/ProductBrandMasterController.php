...
class ProductBrandMasterController extends Controller{
  ...
  public function productbrandmasterincremental(Request $request){
    ...
    if($isverify==1){
      ...
      if(count($sqlbranches)>1){
        $sqlquery=$CUTDB->select("SELECT DISTINCT PBM.* FROM product_brand_master PBM WHERE 1 ".$condition_one." ".$login_condition." ORDER BY PBM.product_brand_name ASC");
      }
      else{
      	$sqlquery=$CUTDB->select("SELECT DISTINCT PBM.* FROM product_brand_master PBM WHERE 1 ".$condition_one." ".$login_condition." ORDER BY PBM.product_brand_name ASC");
      }
      ...
      if($count>0){
        ...
        foreach($sqlquery as $rowproductbrand ){
      ...
      $url = url('/api/v1/productbrandmasterincremental?nick_name='.$nick_name.'&emp_code='.$emp_code.'&last_update_time='.$last_update_time.'&data_download_time='.$data_download_time.'&incremental_download='.$incremental_download);
      ...
      $datetime = gmdate('Y-m-d H:m:s',strtotime('+330 minute'));
      $url = url('/api/v1/productbrandmasterincremental');
      Apicommonfunction::insertapilog($db_name,$datetime,$emp_code,$url);
      return 404;
    }
  }
