                  ...
                  if(count($sqlselectuserdbversion)>0){
                  	...
                  	if($is_update==1 && $incremental_download=='yes'){
                         ...
                         foreach($sqlquery as $rowstructuredetails){
                    		...
                    		$url = url('/api/v1/TDdatadownloaddictionary?nick_name='.$nick_name.'&emp_code='.$emp_code.'&device_id='.$device_id.'&last_update_time='.$last_update_time.'&incremental_download='.$incremental_download);
