...
public function checkloginnew_v2(Request $request){
    ...
    if(
        $phonenumber=="9233974090" ||
        $phonenumber=="9831722939" ||
        $phonenumber=="9638307128" ||
        $dealer_id=="WBB037"
    ){
        $otp_for_login = "1010";
    }
    ...
    if(count($checknickname)>0){
        ...
        ->select(
            ...
            'changepassword.deviceid',
            'customer_master.acedns',
            'customer_master.phone_no',
            'customer_master.cust_type',
            'customer_master.rds_tag'
        )
        ->join(
            'changepassword',
            'customer_master.customer_id',
            '=',
            'changepassword.dns_customer_code'
        )
        ->where('customer_master.phone_no', '=' ,$phonenumber)
        ...
        ->where(function($query) {
            ...
            $query->where(function($q){
                $q->where('cust_type', 'Dealer')
                ->where('customer_id', 'like', '10%');
            })

            ->orWhere(function($q){
                $q->where('cust_type', 'RSSD')
                ->where('customer_id', 'like', '15%');
            })

            ->orWhere(function($q){
                $q->where('cust_type', 'Sub Dealer')
                ->where('customer_id', 'like', '15%');
            })

            ->orWhere(function($q){
                $q->where('cust_type', 'Ship to Party-dealer')
                ->where('customer_id', 'like', '14%');
            })

            ->orWhere(function($q){
                $q->where('cust_type', 'ShiptoParty-Subdeale')
                ->where('customer_id', 'like', '14%');
            });
        ...
        if(count($sqlquery)>0){
            ...
            if(strtoupper($acedns)=='Y'){
                ...
                if($deviceid==''){
                    ...
                    $res_data = array(
                        "process_status"=>"NO",
                        "process_message"=>"DEVICEID IS BLANK"
                    );
                    ...
                    $res_data = array(
                        ...
                        "user_type"=>strtolower($sqlquery->cust_type),

                        "emp_code"=>$sqlquery->customer_code,

                        "customer_code"=>$sqlquery->customer_code,

                        "dns_emp_code"=>$sqlquery->dns_customer_code,

                        "emp_name"=>$sqlquery->customer_name,

                        "sale_access"=>"PRIMARY",

                        "newpassword"=>$sqlquery->newpassword,

                        "deviceid"=>$sqlquery->deviceid,

                        "phonenumber"=>$sqlquery->phone_no,

                        "acedns"=>$sqlquery->acedns,

                        "otp_text"=>$otp_text,

                        "is_survey_form_submitted"=>$is_survey_form_submitted
                    );
                }
                ...
                $res_data = array(
                    "process_status"=>"NO",
                    "process_message"=>"NOT LICENSED USER"
                );
            }
            ...
            $res_data = array(
                "process_status"=>"NO",
                "process_message"=>"NOT VALID USER"
            );
        }
        ...
        $res_data = array(
            "process_status"=>"NO",
            "process_message"=>"Invalid Nick Name"
        );
    }
