...
import static org.forcepower.starcement.constants.Constants.the_pop_cancel_url;
import static org.forcepower.starcement.constants.Constants.the_pop_cancel_url_ne;
import static org.forcepower.starcement.constants.Constants.the_pop_success_url;
import static org.forcepower.starcement.constants.Constants.the_pop_success_url_ne;
...
public final class PopPaymentWebViewActivity extends AppCompatActivity
{
    //Check git version
        ...
        if (HTTPUtils.isConnectionPossible(myActivity))
        {
            ...
            webView.setWebViewClient(new WebViewClient() {
                ...
                public void onPageFinished(WebView view, String url) {
                    ...
                    print_Log_d("star_payment_118 ", "" + url);
                    print_Log_d("star_payment_119 ", "" + the_pop_success_url.toLowerCase());
                    print_Log_d("star_payment_117 ", "" + the_pop_cancel_url.toLowerCase());
                    if(url.toLowerCase().contains(the_pop_success_url.toLowerCase()) ||
                            url.toLowerCase().contains(the_pop_success_url_ne.toLowerCase()))
                    {
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                print_Log_d("star_payment_127 ", " 127");
                                // This method will be executed once the timer is over
                                // Start your app main activity
                                msg_Dialog("Your Transaction is successful.");
                            }
                        }, 1000);
...
//                        });
                    ...
                    else if(url.toLowerCase().contains(the_pop_cancel_url.toLowerCase()) ||
                            url.toLowerCase().contains(the_pop_cancel_url_ne.toLowerCase()))
                    {
                        new Handler().postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                print_Log_d("star_payment_145 ", " 145");
                                // This method will be executed once the timer is over
                                // Start your app main activity
        ...
        try
        {
            ...
            AlertDG.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    Intent intent = new Intent(myActivity, PopProductActivity.class);
    ...
    {
        msg_back_Dialog("Do You Want To Exit");
    }
}
