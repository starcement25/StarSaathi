...
public final class DestinationAdapter_M extends ArrayAdapter<DestinationMaster> implements Filterable
{
	...
	public DestinationAdapter_M(final Context context, final int resourceId, final ArrayList<DestinationMaster> nameValue) {
	...
	public View getView(final int position, View convertView, final ViewGroup parent)
	{
		try
		{
			...
			viewHolder.txtView.setTag(finalValues.get(position).getRow_position());
			if(finalValues.get(position).get_selected())
			{
				viewHolder.imageView1.setImageResource(android.R.drawable.checkbox_on_background);
			}
			else
			{
				viewHolder.imageView1.setImageResource(android.R.drawable.checkbox_off_background);
			}
	...
	public void setFilter(final ArrayList<DestinationMaster> vals) {
		if(vals != null)
		{
			finalValues = new ArrayList<>();
			finalValues.addAll(vals);
