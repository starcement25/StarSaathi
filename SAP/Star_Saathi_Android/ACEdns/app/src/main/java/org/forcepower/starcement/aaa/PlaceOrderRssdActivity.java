...
import static org.forcepower.starcement.constants.Constants.place_order_lifting_days_download;
...
public final class PlaceOrderRssdActivity extends AceDnsParentActivity
{
	...
	private long validation_last_date = 0, current_date = 0;
...
//						nameValuePairs.add(new BasicNameValuePair("remarks", et_remarks.getText().toString())); //vola2715
	...
	public final class TRANS_GetLifting_Asynctask extends AsyncTaskCoroutine<String, String>
	{
		...
		public TRANS_GetLifting_Asynctask(final Activity mContext) {
			this.mContext = mContext;
		}
		...
		public String doInBackground(final String... params)
		{
			...
			try
			{
				if (HTTPUtils.isConnectionPossible(mContext))
				{
					try
					{
						final String url = place_order_lifting_days_download;
						...
						print_log_d("jr38et_URL ", url);
						print_log_d("jr38et_P ", nameValuePairs.toString());

						POST_result = HTTPUtils.getDataByHTTP_POST(mContext, url, nameValuePairs);
						print_log_d("jr38et_POST_Res ", POST_result);

						jo = new JSONObject(POST_result);
						final int place_order_days = jo.optInt("place_order_days")-1;
						print_log_d("jr38et_place_order_days ", place_order_days + "");
						...
						calendar.add(Calendar.DAY_OF_YEAR, place_order_days);
						validation_last_date = calendar.getTimeInMillis();
					}
					catch (Exception e)
					{
						print_log_d("jr38et_Err1 ", e.toString());
		...
		public void onPostExecute(String result)
		{
			...
			try
			{
				...
				else if(!(current_date >0 && validation_last_date >0))
				{
					continue_status = "No";
