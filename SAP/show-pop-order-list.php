...
//echo"<pre>";print_r($the_customer_code);die;
...
//echo"<pre>";print_r($tot_res_cust);die;
    ...
    if ($tot_res_cust > 0) {
        ...
        if ($totall2 > 0) {
            while ($row112 = mysql_fetch_assoc($resall2)) {
				...
				$status = isset($row112["status"]) ? trim($row112["status"]) : "";
				
				$sqlpopproduct="SELECT payment_gateway FROM $pop_product_master WHERE dns_prod_code='$dns_prod_code'";
				$rspopproduct=mysql_query($sqlpopproduct);
				$rowpopproduct=mysql_fetch_array($rspopproduct);
				$payment_gateway=$rowpopproduct['payment_gateway'];
				if($payment_gateway=='Y'){
                $pop_order_data[] = array("order_id" => $order_id, "order_date" => $order_full_date_time, "customer_name" => $customer_name, "address" => $address, "pin" => $pin, "dns_prod_code" => $dns_prod_code, "prod_display_name" => $prod_display_name, "qty" => $qty, "total_amount" => $total_amount, "image" => $prod_img_URL, "main_order_id" => $main_order_id, "order_status" => $status);
				}
				else if($payment_gateway=='N'){
