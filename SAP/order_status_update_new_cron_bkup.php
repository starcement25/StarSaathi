...
$sql1 = "SELECT `id`,`APPORDERNO`,`ERPORDERNO`,`STATUS`,`QTY`,`customer_code`,`dns_customer_code`,`prod_code`,`dns_prod_code`,`freight`,`destination_code`,`destination_name` FROM $t_apperpdo where `STATUS` in('Order authorized','Order received','CREDIT CHECK FAILED') and `APPORDERNO`!='' order by `id` asc";
...
$pn_text_msg = "";
	...
	if($so_num!=""){
		$is_sent_pn = "NO";
		if($remarks==""){
		$upd_sts_qry = ",`STATUS`='DO approved'";
		$is_sent_pn = "YES";
