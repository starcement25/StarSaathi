...
SELECT 
    ...
    COALESCE(SUM(
        CASE 
            WHEN (f.year = '$year' AND m.month_number BETWEEN 4 AND 12) 
              OR (f.year = '$year'+1 AND m.month_number BETWEEN 1 AND 3)
            THEN l.allocated_qty 
            ELSE 0 
        END
    ), 0) AS current_cycle_qty,
    -- Previous cycle: Apr-Dec last year, Jan-Mar this year
    COALESCE(SUM(
        CASE 
            WHEN (f.year = '$year'-1 AND m.month_number BETWEEN 4 AND 12)
              OR (f.year = '$year' AND m.month_number BETWEEN 1 AND 3)
            THEN l.allocated_qty
            ELSE 0
        END
    ), 0) AS prev_cycle_qty
...
while ($rowsappraisal = mysql_fetch_array($result)) {
    ...
    $current_cycle_qty = (float)$rowsappraisal['current_cycle_qty']; 
    $prev_cycle_qty = (float)$rowsappraisal['prev_cycle_qty'];

    $emp_code = strtoupper($emp_code);
    $data .= "12001^STAR ANTIRUST CEMENT  TRADE^".$emp_code."^".$month_number."^0^".$current_cycle_qty."^0^".$prev_cycle_qty."\n";
}
