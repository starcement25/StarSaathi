...
$search_sub_dealer_name = $_POST["search_sub_dealer_name"] ? addslashes(trim($_POST["search_sub_dealer_name"])) : "";

$log_date = date("Y-m-d H:i:s");

$sql4="select `customer_id` from $customer_master where `customer_code`='".$dealer_cust_code."'";
...
$sql5="insert into $app_service_track_log(`appservice_name`,`customer_code`,`datetime`) values('RETAILER LIFTING','$app_customer_code','$log_date')";

$reslon=mysql_query($sql5);

if($reslon){
	$res_data=array("process_status"=>"YES","process_message"=>"POP order submitted.");
}

$sql_app_track1="insert into $app_service_track_log(`appservice_name`,`customer_code`,`datetime`) values('RETAILER LIFTING','$dealer_cust_code','$log_date')";
...
{
	$year_month=date('Y-n',strtotime($year_month));
...
if($dealer_cust_code==""){
$res_data = array("process_status"=>"NO","process_message"=>"Please provide dealer code.","lifting_history_data"=>$lifting_history_data);
}else{
...
$ssdn_qry = "";
if($search_sub_dealer_name!=""){
$ssdn_qry = " and `sub_dealer_rssd_name` like '%$search_sub_dealer_name%' ";
}

$sql3 = "select *,DATE_FORMAT(`date_of_lifting`,'%Y-%c') as `ym_of_lifting` from $lifting_table where `linked_dealer_cust_code`='$dealer_cust_code' and `status`='APPROVED' $ssdn_qry $ym_qry order by `date_of_lifting` desc limit $start_from,$limit";
