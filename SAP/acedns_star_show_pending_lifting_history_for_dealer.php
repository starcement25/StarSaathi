...
$curr_year_month_val = date("Y-m");
$curr_year_month = date("Y-n");
$dealer_cust_code = $_REQUEST["dealer_cust_code"] ? addslashes(trim($_REQUEST["dealer_cust_code"])) : "";
$year_month = $_REQUEST["year_month"] ? addslashes(trim($_REQUEST["year_month"])) : "";
//$year_month=date('Y-n',strtotime($year_month));

$search_sub_dealer_name = $_REQUEST["search_sub_dealer_name"] ? addslashes(trim($_REQUEST["search_sub_dealer_name"])) : "";

if($year_month==""){
 $year_month = $curr_year_month;
 $year_month_val = $curr_year_month_val;	
}
else
{
	$year_month=date('Y-n',strtotime($year_month));
	$year_month_val=date('Y-m',strtotime($year_month));
}
$page_no = $_REQUEST["page_no"] ? trim($_REQUEST["page_no"]) : 1;
if ($page_no == "") {
	$page_no = 1;
}
$limit = 20;
...
if ($dealer_cust_code == "") {
	...
	$ym_qry = "";
	if ($year_month != "") {
		$ym_qry = " having (`date_of_lifting` like '%$year_month%' OR `date_of_lifting` like '%$year_month_val%') ";
	}
	...
	$sql3 = "select * from $lifting_table where `linked_dealer_cust_code`='$dealer_cust_code' and `status`='PENDING'$ssdn_qry$ym_qry order by `date_of_lifting` desc limit $start_from,$limit";
