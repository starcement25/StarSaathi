...
$ssdn_qry = " and `sub_dealer_rssd_name` like '%$search_sub_dealer_name%' ";
	...
	$sql3 = "select *,DATE_FORMAT(`date_of_lifting`,'%Y-%c') as `ym_of_lifting` from $lifting_table where `linked_dealer_cust_code`='$dealer_cust_code' and `status`='REJECTED' $ssdn_qry $ym_qry order by `date_of_lifting` desc limit $start_from,$limit";
