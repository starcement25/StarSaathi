...
include "../cron_page_start.php";
	...
	/*$sqlcustomerSAP="SELECT KUNNR,ALTKN,NAME1,NAME2,NAME3,STRAS,STREET1,STREET2,STREET3,ORT01,ORT02,PSTLZ,TELF2,
					CREDIT_LIMIT,SMTP_ADDR,VKGRP,KDGRP,PAN_NUMBER,MIN_STOCK_QTY,ACCOUNT_NO,CITY2,COUNTER_POT,ZZONE,LZONE,VTEXT,`SEARCH_TERM2`,AUFSD,`REGION`,`ADDITIONAL_DATA2`,`TRANSZONE`,
					`SEARCH_TERM1`,`VWERK`,`NAME4`,`ADD_VAL_FR`,`LATITUDE`,`LONGITUDE`,`DOB`,`ANV_DATE`,`BANK_KEY`,`SUB_ZONE`
					FROM ptblcustomermaster WHERE KDGRP NOT IN('15')  ORDER BY KUNNR,AEDAT ASC,ADDITIONAL_DATA1 ASC";*/
$sqlcustomerSAP="SELECT KUNNR,ALTKN,NAME1,NAME2,NAME3,STRAS,STREET1,STREET2,STREET3,ORT01,ORT02,PSTLZ,TELF2,
					CREDIT_LIMIT,SMTP_ADDR,VKGRP,KDGRP,PAN_NUMBER,MIN_STOCK_QTY,ACCOUNT_NO,CITY2,COUNTER_POT,ZZONE,LZONE,VTEXT,`SEARCH_TERM2`,AUFSD,`REGION`,`ADDITIONAL_DATA2`,`TRANSZONE`,
					`SEARCH_TERM1`,`VWERK`,`NAME4`,`ADD_VAL_FR`,`LATITUDE`,`LONGITUDE`,`DOB`,`ANV_DATE`,`BANK_KEY`,`SUB_ZONE`
					FROM ptblcustomermaster WHERE KDGRP NOT IN('15') AND AEDAT='".$curdate."' ORDER BY KUNNR,AEDAT ASC,ADDITIONAL_DATA1 ASC";
	...
	while($rowcustomerSAP=mysql_fetch_array($rscustomerSAP))
	{
		...
		if(!in_array($dns_customer_code,$dns_customer_code_array))
			...
			${whatsapp_no.$dns_customer_code}=$rowcustomerSAP['TRANSZONE'];
			${sub_dealer_old_code_one.$dns_customer_code}=$rowcustomerSAP['SEARCH_TERM1'];
			${plant.$dns_customer_code}=$rowcustomerSAP['VWERK'];
			${owner_name.$dns_customer_code}=$rowcustomerSAP['NAME4'];
			${appointment_date.$dns_customer_code}=$rowcustomerSAP['ADD_VAL_FR'];
			${lattitude.$dns_customer_code}=$rowcustomerSAP['LATITUDE'];
			${longitude.$dns_customer_code}=$rowcustomerSAP['LONGITUDE'];
			${DOB.$dns_customer_code}=$rowcustomerSAP['DOB'];
			${ANV_DATE.$dns_customer_code}=$rowcustomerSAP['ANV_DATE'];
			${IFSC.$dns_customer_code}=$rowcustomerSAP['BANK_KEY'];
			${SUB_ZONE.$dns_customer_code}=$rowcustomerSAP['SUB_ZONE'];
		}
	foreach($dns_customer_code_array as $dns_customer_code_val){
			...
			$sub_dealer_old_code_one=${sub_dealer_old_code_one.$dns_customer_code_val};
			$plant=${plant.$dns_customer_code_val};
			$owner_name=${owner_name.$dns_customer_code_val};
			$appointment_date=${appointment_date.$dns_customer_code_val};
			$lattitude=${lattitude.$dns_customer_code_val};
			$longitude=${longitude.$dns_customer_code_val};
			$DOB=${DOB.$dns_customer_code_val};
			$ANV_DATE=${ANV_DATE.$dns_customer_code_val};
			$IFSC=${IFSC.$dns_customer_code_val};
			$SUB_ZONE=${SUB_ZONE.$dns_customer_code_val};
			
			if($cust_type=='21' || $cust_type=='22' || $cust_type=='12' || $cust_type=='13')
			{
				$mapped_dealer_ship_to_party_dealer=${sub_dealer_old_code.$dns_customer_code_val};
			}
			else $mapped_dealer_ship_to_party_dealer='';
			
			if($cust_type!='07' && $cust_type!='08'  && $cust_type!='09' && $cust_type!='03' && $cust_type!='04' && $cust_type!='05' && $cust_type!='11' && $cust_type!='21' && $cust_type!='22')
			{
				//$dns_customer_code=${sub_dealer_old_code.$dns_customer_code_val};
				$dns_customer_code=$sub_dealer_old_code_one;
			}
			...
			//echo $dns_customer_code.'-'.$customer_id.'<br/>';
			if(($acedns=='' || $acedns=='NULL') && ($sub_dealer_old_code !='INACTIVE'))
			{
				$acedns='Y';
			}
			else $acedns='N';
			...
			if($cust_type_fetched=='SBS') $cust_type_fetched='Dealer';
			if($cust_type_fetched=='IFS') $cust_type_fetched='Dealer';
			if($cust_type_fetched=='Tender') $cust_type_fetched='Dealer';
		    if($cust_type_fetched=='KAS') $cust_type_fetched='Dealer';
			
			if($cust_type!='07' && $cust_type!='08' && $cust_type!='09' && $cust_type!='03' && $cust_type!='04' && $cust_type!='05' && $cust_type!='11' && $cust_type!='21' && $cust_type!='22' && $cust_type!='12' && $cust_type!='13')
			{
			...
			else if($cust_type=='12' || $cust_type=='13')
			{
			...
			if(substr($customer_id,0,2)=='14' && $cust_type!='03' && $cust_type!='04' && $cust_type!='05' && $cust_type!='11')
			{
			...
			if($countcustomernamechk<1)
			{
				...
				$sql .= " , lattitude='".addslashes($lattitude)."'";
				$sql .= " , longitude='".addslashes($longitude)."'";
				$sql .= " , DOB='".addslashes($DOB)."'";
				$sql .= " , ANV_DATE='".addslashes($ANV_DATE)."'";
				$sql .= " , IFSC='".addslashes($IFSC)."'";
				$sql .= " , sub_zone='".addslashes($sub_zone)."'";
			...
			else
			{
				...
				$whatsapp_no_db=$rowcustomernamechk['whatsapp_no'];
				$plant_db=$rowcustomernamechk['plant'];
				$owner_name_db=$rowcustomernamechk['owner_name'];
				$appointment_date_db=$rowcustomernamechk['appointment_date'];
				$lattitude_db=$rowcustomernamechk['lattitude'];
				$longitude_db=$rowcustomernamechk['longitude'];
				$DOB_db=$rowcustomernamechk['DOB'];
				$ANV_DATE_db=$rowcustomernamechk['ANV_DATE'];
				$IFSC_db=$rowcustomernamechk['IFSC'];
				$sub_zone_db=$rowcustomernamechk['sub_zone'];
				...
				|| $bank_account_number_db!=$bank_account_number || $monthly_potential_db!=$monthly_potential || $zone_db!=$zone || $region_db!=$region || $whatsapp_no_db!=$whatsapp_no || $plant_db!=$plant || $owner_name_db!=$owner_name || $appointment_date_db!=$appointment_date || $lattitude_db!=$lattitude || $longitude_db!=$longitude 
				|| $DOB_db!=$DOB || $ANV_DATE_db!=$ANV_DATE || $IFSC_db!=$IFSC || $sub_zone_db!=$sub_zone) 
				{
					...
					$sqlupdated .= " , whatsapp_no='".addslashes($whatsapp_no)."'";
					$sqlupdated .= " , plant='".addslashes($plant)."'";
					$sqlupdated .= " , owner_name='".addslashes($owner_name)."'";
					$sqlupdated .= " , appointment_date='".addslashes($appointment_date)."'";
					$sqlupdated .= " , lattitude='".addslashes($lattitude)."'";
					$sqlupdated .= " , longitude='".addslashes($longitude)."'";
					$sqlupdated .= " , DOB='".addslashes($DOB)."'";
					$sqlupdated .= " , ANV_DATE='".addslashes($ANV_DATE)."'";
					$sqlupdated .= " , IFSC='".addslashes($IFSC)."'";
					$sqlupdated .= " , sub_zone='".addslashes($sub_zone)."'";
