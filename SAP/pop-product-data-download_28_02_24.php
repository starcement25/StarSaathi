...
$sql_app_track1="insert into $app_service_track_log(`appservice_name`,`customer_code`,`datetime`) values('POP ORDER','$customer_id','$log_date')";
...
if($the_customer_code!=''){
	...
	$sqlbranchcode="SELECT dns_branch_code FROM $branch_master WHERE branch_code IN(SELECT branch_code FROM $customer_master WHERE  dns_customer_code='".$the_customer_code."')";
	...
	$sqlpopproduct="SELECT * FROM $pop_product_master WHERE `status`='Y' AND dns_prod_code IN(SELECT dns_prod_code FROM branch_pop_product  where branch_code='".$dns_branch_code."' and `status`='Y')";
	...
	if($countpopproduct > 0)
	{
		while($rowpopproduct=mysql_fetch_array($rspopproduct))
		{
			...
			$pop_product_date[] = array("dns_prod_code"=>$dns_prod_code,"prod_desc"=>$prod_desc,"prod_image"=>$prod_img_URL.$prod_image,"min_order_qty"=>$min_order_qty,"price_per_piece"=>$price_per_piece,"GST_rate"=>$GST_rate);
