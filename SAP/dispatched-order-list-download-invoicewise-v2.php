	...
	where $T_DOINVOICE.INVNO='".$invoice_no."' and $T_DOINVOICE.APPORDERNO='".$app_orderno."'  and $T_DOINVOICE.APPORDERNO!=''  and $T_DOINVOICE.APPORDERNO IS NOT NULL GROUP BY $T_DOINVOICE.INVNO";
		...
		if ($totall2 > 0) {
			...
			// where $T_APPERPDO_OFFLINE.ERPORDERNO='".$app_orderno."' and $T_APPERPDO_OFFLINE.ERPORDERNO!=''  and $T_APPERPDO_OFFLINE.ERPORDERNO IS NOT NULL GROUP BY $T_APPERPDO_OFFLINE.ERPORDERNO";
			 $sqlall2 = "select $T_DOINVOICE.INVNO,SUM($T_DOINVOICE.INVQTY) AS TOT_INVQTY from $T_DOINVOICE
	where $T_DOINVOICE.INVNO='".$invoice_no."' and $T_DOINVOICE.ERPORDERNO='".$app_orderno."'  and $T_DOINVOICE.APPORDERNO!=''  and $T_DOINVOICE.APPORDERNO IS NOT NULL GROUP BY $T_DOINVOICE.INVNO";
				//mysql_query("SET SESSION sql_mode = 'TRADITIONAL'");
				$resall2 = mysql_query($sqlall2);
				...
				if ($totall2 > 0) {
					while ($row112 = mysql_fetch_assoc($resall2)) {
								...
								$sqlallocationqty = "select SUM(allocation_qty) tot_allocation_qty from $allocation_details where  inv_no='$invoice_no' and APPORDERNO='".$app_orderno."' and delete_at='0' GROUP BY inv_no";
								//echo"<pre>";print_r($sqlallocationqty);die;
								$resallocationqty = mysql_query($sqlallocationqty);
								$totresallocationqty = mysql_num_rows($resallocationqty);
								$rowallocationqty=mysql_fetch_array($resallocationqty);
								...
								$dispatched_invoice_data[] = array("INVNO" => $invoice_no, "inv_qty" => $ch_invqty,"available_allocation_qty" => $available_allocation_qty);
							}
