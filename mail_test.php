...
//$bodyml             = eregi_replace("[\]",'',$bodyml);
$mail->IsSMTP(); // telling the class to use SMTP
//$mail->Host       = "103.87.174.7"; // SMTP server (For gmail "mail.coral.in")
$mail->SMTPDebug  = 3;
$mail->Mailer = 'smtp';
$mail->SMTPAuth   = true;                  // enable SMTP authentication
...
$mail->Port       = 587;                   // set the SMTP port for the GMAIL server (For gmail 465 )
$mail->Username   = "suranjitd@coral.in";
$mail->Password   = "iamsuranjit";

$mail->SetFrom('suranjitd@coral.in', 'SJD');
/*$mail->ClearReplyTos();
$mail->addReplyTo('app.ambica@gmail.com', 'AMBICA');*/
$mail->Subject    = $subject;
$mail->AltBody    = "To view the message, please use an HTML compatible email viewer!"; // optional, comment out and test
$mail->MsgHTML($bodyml);
foreach($to_email_arr as $to_email_arr_val){
...
$bodyml = "Test Message";
$mres = send_the_mail2($to_email,$subject,$bodyml);
echo $mres;
?>