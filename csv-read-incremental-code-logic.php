		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				$countprodcode=mysql_num_rows($rsbranchnamechk);
				...
				$sqlvendor .= "  order_no='".mysql_real_escape_string($order_no)."'";
				$sqlvendor .= "  ,sku_code='".mysql_real_escape_string($prod_code)."'";
				$sqlvendor .= " , qty='".mysql_real_escape_string($qty)."'";
				$sqlvendor .= " , sale_rate='".mysql_real_escape_string($rate)."'";
				mysql_query($sqlvendor) or array_push($error_array,"mysql_error().Internal error in order_details.csv.Please check.");
				  ...
				  //print_r($data);
					...
					if($customer_type=='D')
					{
						...
						if($countrdsnamechk<1)
						{
							...
							mysql_query($sqlrds) or array_push($error_array,"mysql_error().Internal DATA execution problem on distributor table.PLease contact aceDNS admin.");
						}
					}
					//-----------------------------------------------------------End of rds creation----------------------------------------------------
					...
					$rsrdstagcode=mysql_query($sqlrdstagcode);
					$rowrdstagcode=mysql_fetch_array($rsrdstagcode);
					...
					for($cntemp=0;$cntemp< count($emp_code_name_array);$cntemp++)
					{
						...
						$rscustomeremprealtion=mysql_query($sqlcustomeremprealtion);
						$cntcustomeremprealtion=mysql_num_rows($rscustomeremprealtion);
						...
						if($cntcustomeremprealtion <1)
						{
							...
							mysql_query($sqlinsertcustomerrelation) or array_push($error_array,"mysql_error().Internal DATA execution problem on customer relational table.PLease contact aceDNS admin.");
