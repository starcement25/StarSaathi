				...
				{ 
				  ...
				  //print_r($data);
					...
					if($countprodbrandnamechk<1){
						...
						$sqlbrandsubform .= "  product_brand_code='".mysql_real_escape_string($product_brand_name)."'";
						...
						$sqlbrandsubform .= " , product_brand_name='".mysql_real_escape_string($product_brand_name)."'";
					...
					else
					{
						...
						if($product_sub_group_name_existing!=$product_group_name || $vertical_value_existing!=$vertical_value)
						{
							...
							$sqlupdatebrandsubform .= " vertical_value='".mysql_real_escape_string($vertical_value)."'";
							...
							mysql_query($sqlupdatebrandsubform) or die(mysql_error().".Internel error occurrs @row $csv_row_count on Brand SUB Form Master.csv.Please check.");
						}
					}
				}
				 $rec_count++;
				...
				{ 
				  ...
				  //print_r($data);
					...
					else
					{
						...
						mysql_query($sqlupdatevendor) or die(mysql_error().".Internal error occurrs @row $csv_row_count in Broker master.csv.Please check.");
					}
				}
				 $rec_count++;
			}		
				...
				{ 
				  ...
				  //print_r($data);
					...
					$actual_mall=trim($data[1]);
						...
						$sqlupdatebroker .= "  mall_hs_name='".mysql_real_escape_string($actual_mall)."'";
						$sqlupdatebroker .= "  ,download_time=CURRENT_TIMESTAMP()";
						$sqlupdatebroker .= " WHERE mall_hs_name='".mysql_real_escape_string($replacing_mall)."'";
						mysql_query($sqlupdatebroker) or die(mysql_error().".Internal error occurrs @row $csv_row_count in mall master.csv.Please check.");
				}
				 $rec_count++;
			}		
			$successval=1;
			...
			{ 
			  ...
			  //print_r($data);
				...
				$new_branch_code=trim($data[2]);
					...
					$sqlbranch .= "  SET	dns_branch_code='".mysql_real_escape_string($new_branch_code)."'";
					$sqlbranch .= " , download_time=CURRENT_TIMESTAMP()";
		...
		foreach($lines as $line)
		{
			...
			{ 
			  ...
			  //print_r($data);
				...
				while($rowselcustomercode=mysql_fetch_array($rsselcustomercode)){
				...
				$rsempcodename=mysql_query($sqlempcodename);
				$rowempcodename=mysql_fetch_array($rsempcodename);
				...
				$rsdistcode=mysql_query($selectdistcode);
				$rowdistcode=mysql_fetch_array($rsdistcode);
				$distributorcode=$rowdistcode['customer_code'];				
				
					if($cust_type=='R')
					{
						$sqlupdate  = "UPDATE customer_master ";
						$sqlupdate .= "  SET	rds_tag='".mysql_real_escape_string($distributorcode)."'";
						$sqlupdate .= " , download_time=CURRENT_TIMESTAMP()";
