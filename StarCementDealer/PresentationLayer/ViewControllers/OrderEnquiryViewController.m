...
#import "OrderEnquiryViewController.h"
#import "OrderEnquiryCell.h"
#import "WebViewController.h"

@interface OrderEnquiryViewController ()<UITableViewDelegate, UITableViewDataSource>{
...
@implementation OrderEnquiryViewController
...
-(void)designView {
    ...
    [tblViewInvoice registerNib:[UINib nibWithNibName:@"OrderEnquiryCell" bundle:nil] forCellReuseIdentifier:@"cell"];
...
-(void)getInvoices {
    ...
    if (APP_DELEGATE.isServerReachable) {
        ...
        [JsonParser callAPIWithURL:@"https://starsaathi.com/SAP/order_query_data_download.php" parameters:dict completion:^(NSDictionary *dictResponse){
            dispatch_async(dispatch_get_main_queue(), ^(void) {
                [SVProgressHUD dismiss];
                if ([[dictResponse valueForKey:@"process_status"] isEqualToString:@"YES"]) {
                    [arrInvoices removeAllObjects];
                    [arrInvoices addObjectsFromArray:[dictResponse safeValueeForKey:@"order_query_data"]];
...
- (OrderEnquiryCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellIdentifier = @"cell";
    OrderEnquiryCell *cell = [tblViewInvoice dequeueReusableCellWithIdentifier:cellIdentifier forIndexPath:indexPath];
    NSDictionary *dictInvoice = arrInvoices[indexPath.row];
    
    cell.lblProdName.text       = [dictInvoice safeValueeForKey:@"prod_name"];
    cell.lblQtyBags.text        = [dictInvoice safeValueeForKey:@"qty_bags"];
    cell.lblDateAndTime.text    = [dictInvoice safeValueeForKey:@"date_and_time"];
    cell.lblRssdName.text       = [dictInvoice safeValueeForKey:@"rssd_name"];
    cell.lblDateOfLifting.text  = [dictInvoice safeValueeForKey:@"date_of_lifting"];
    cell.lblRemarks.text        = [dictInvoice safeValueeForKey:@"remarks"];
