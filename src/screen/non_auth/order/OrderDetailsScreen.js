...
import { Image, Modal, ScrollView, Text, TextInput, TouchableOpacity, View, StyleSheet, Keyboard } from 'react-native'
...
const uploadGSTDetails = async ({ customerCode, gstNumber, document }) => {
    try {
        ...
        if (document)
            formData.append('upload_doc', { uri: document.uri, type: document.type || 'application/octet-stream', name: document.name || `gst_doc_${Date.now()}` })
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={() => { }} >
        <View style={styles.modalOverlay}>
            ...
            <View style={styles.modalCard}>
                ...
                <Text style={styles.modalMessage}> {selected == 1 ? "Is Additional Place of business the same as per GSTIN certificate?" : cust_type == null ? "Is the Sub-Dealer/RSAR GSTIN registered?" : 'Is the Ship-to-Party RSAR GSTIN registered?'} </Text>
            ...
            <View style={styles.modalCard}>
                ...
                <View style={{ width: '100%', marginTop: moderateScale(16) }}>
                    ...
                    <TouchableOpacity activeOpacity={0.7} style={styles.dropdownContainer} onPress={onPickDocument}>
                        <Text style={[styles.dropdownText, !gstDocument && styles.placeholderText]} numberOfLines={1}> {gstDocument?.name || "Select PDF / JPG / PNG"} </Text>
                    </TouchableOpacity>
                </View>
                <View style={styles.modalButtonRow}>
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={() => { }}>
        <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
                ...
                <Text style={styles.modalMessage}> I confirm that turnover is below the taxable limit as specified under GSTIN, 2017. </Text>
                <View style={styles.modalButtonRow}>
                    <TouchableOpacity activeOpacity={0.7} style={styles.modalButtonSecondary} onPress={onBackPress}>
                        <Text style={styles.modalButtonSecondaryText}>Back</Text>
                    </TouchableOpacity>
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
        <TouchableOpacity activeOpacity={1} style={styles.modalOverlay} onPress={onClose}>
            <TouchableOpacity activeOpacity={1} style={styles.modalCard} onPress={() => { }}>
                ...
                <Text style={styles.modalMessage}> The GSTIN declaration for selected (Ship-to Party) has not yet submitted. Please ask the RSAR to complete the declaration in the App. Order placement will be enabled once the declaration is submitted. </Text>
                <View style={styles.modalButtonRow}>
                    <TouchableOpacity activeOpacity={0.7} style={[styles.modalButtonPrimary, { backgroundColor: primaryColor }]} onPress={onConfirm}>
                        <Text style={styles.modalButtonPrimaryText}>Confirm</Text>
                    </TouchableOpacity>
...
const GSTNotDeclarationForSeflPopup = React.memo(({ visible, primaryColor, onConfirm, onClose }) => (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
        <TouchableOpacity activeOpacity={1} style={styles.modalOverlay} onPress={onClose}>
            <TouchableOpacity activeOpacity={1} style={styles.modalCard} onPress={() => { }}>
                <Text style={styles.modalTitle}>GSTIN Declaration Pending</Text>
                <Text style={styles.modalMessage}> The GSTIN declaration for selected business or godown has not yet submitted. Please complete the GSTIN declaration in the App. Order placement will be enabled once the declaration is submitted. </Text>
                <View style={styles.modalButtonRow}>
                    <TouchableOpacity activeOpacity={0.7} style={[styles.modalButtonPrimary, { backgroundColor: primaryColor }]} onPress={onConfirm}>
                        <Text style={styles.modalButtonPrimaryText}>Confirm</Text>
                    </TouchableOpacity>
...
const OrderDetailsScreen = ({ route, navigation }) => {
    ...
    const [popupStates, setPopupStates] = useState({ shipToSelf: false, destinationAddress: false, dump: false, truck: false, forType: false })
    ...
    const requestForCheckGSTIN = () => {
        ...
        fetch(UrlStorage.BaseUrlList.Saathi.base_url_saathi + "/static_response_v1.php", requestOptions)
            .then((response) => response.json())
            .then((result) => { setIsGSTINRequired(result.process_message) })
            .catch((error) => { })
    }
    ...
    const closeAllPopups = useCallback(() => {
        setPopupStates({ shipToSelf: false, destinationAddress: false, dump: false, truck: false, forType: false })
    }, [])
    ...
    const continueOrderFlow = useCallback(async (item) => {
        ...
        if (UrlStorage.ParameterList.BasicData.user_type == 'broker')
            ...
            await requestForCementTruckList(shipItem?.SAP_code || UrlStorage.ParameterList.BasicData.emp_id)
        if (isSBS) {
            const singleDest = [{ code: item.destination_code, name: item.destination, type: '' }]
            ...
            fetch(UrlStorage.BaseUrlList.Saathi.base_url_saathi + UrlStorage.NonAuthURL.Saathi.OrderURL1.truck_data_list_url, requestOptions)
                ...
                .then((result) => {
                    if (result?.truck_data?.length > 0) {
                        var arr = []
                        result.truck_data.map(item => { arr.push(item.truck_no) })
                        setTruckList(arr)
                    } else
                        setTruckList([])
                })
    ...
    return (
        <SafeView backgroundColor={Colors.white} bar={false} statusbarColor={Colors.main}>
                ...
                <ScrollView ref={scrollViewRef} style={styles.scrollView} showsVerticalScrollIndicator={false} nestedScrollEnabled={false} keyboardShouldPersistTaps="handled" scrollEventThrottle={16} removeClippedSubviews={false} contentContainerStyle={styles.scrollContent} >
                    ...
                    <FormField label="Consignee Name">
                        <View style={styles.inputContainer} pointerEvents="none">
                            <TextInput placeholder='|' value={consigneeName.replaceAll("&amp", '&')} editable={false} placeholderTextColor="#A7A7A7" style={styles.input} />
                        </View>
                    </FormField>
                    ...
                    <FormField label="Destination Address">
                        <TouchableOpacity activeOpacity={0.7} onPress={openDestinationPopup} style={styles.dropdownContainer} >
                            <Text style={[styles.dropdownText, !destinationAddress && styles.placeholderText]} numberOfLines={1} > {destinationAddress || "Select destination address"} </Text>
                        ...
                        <View style={styles.row}>
                            <TouchableOpacity
                                ...
                                <View style={styles.radioContainer}>
                                    ...
                                    <Text ellipsizeMode='tail' numberOfLines={1} style={[styles.radioText, truckList.length > 0 && { width: moderateScale(60) }]} > For {forType && `(${forType})`} </Text>
                                </View>
                            </TouchableOpacity>
                    ...
                    {freight === 2 && dumpDataList.length > 0 && <FormField label="Dump Name">
                        <TouchableOpacity activeOpacity={0.7} onPress={openDumpPopup} style={styles.dropdownContainer} >
                            <Text style={[styles.dropdownText, !dumpName && styles.placeholderText]} numberOfLines={1} > {dumpName || "Select dump"} </Text>
                            <Image source={Icons.DownArrow} style={[styles.dropdownIcon, { tintColor: primaryColor }]} />
                        </TouchableOpacity>
                    </FormField>}
                    {freight === 1 && forType === 'DOT' && truckList.length > 0 && <FormField label="Dealer Truck">
                        <TouchableOpacity activeOpacity={0.7} onPress={openTruckPopup} style={styles.dropdownContainer} >
                            <Text style={[styles.dropdownText, !truckName && styles.placeholderText]} numberOfLines={1} > {truckName || "Select Dealer Truck"} </Text>
                            <Image source={Icons.DownArrow} style={[styles.dropdownIcon, { tintColor: primaryColor }]} />
                        </TouchableOpacity>
                    </FormField>}
                    {freight === 1 && <FormField label="Available Plant">
                        <TouchableOpacity activeOpacity={0.7} style={[styles.dropdownContainer, { backgroundColor: '#edebeb' }]} >
                            <Text style={[styles.dropdownText, (!plant || plant.length === 0) && styles.placeholderText]} numberOfLines={1} > {(plant && plant.length > 0 && plant[0]?.plant_name) ? plant[0].plant_name : "No plant available"} </Text>
                        </TouchableOpacity>
                    </FormField>}
            ...
            <GSTQuestionPopup visible={showGSTQuestionPopup} primaryColor={primaryColor} selected={selected} cust_type={shipItem.cust_type} onYes={handleGSTYes} onNo={handleGSTNo} onBackPress={handleMainBackPress} onYesDealer={handleGSTDeclarationConfirm} />
            ...
            <GSTDeclarationPopup visible={showGSTDeclarationPopup} primaryColor={primaryColor} onConfirm={handleGSTDeclarationConfirm} onBackPress={handleBackPress} />
            <GSTNotDeclarationForRSARPopup visible={showGSTNotDeclarationForRSARPopup} primaryColor={primaryColor} onConfirm={closePopupAndOpenSubDealer} onClose={onClosePopup} />
            <GSTNotDeclarationForSeflPopup visible={showGSTNotDeclarationForDealerPopup} primaryColor={primaryColor} onConfirm={closePopupAndOpenDealer} onClose={onClosePopup} />
