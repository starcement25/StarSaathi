  ...
  function GrowthBadge({ value }) {
    ...
    return (
      <View style={[styles.growthBadge, { backgroundColor: positive ? COLORS.green50 : COLORS.red50 }]}>
        <Text style={[styles.growthText, { color: positive ? COLORS.green800 : COLORS.red800 }]}> {positive ? '+' : ''}{value.toFixed(1)}% </Text>
      </View>
    )
  }
          ...
          {bars.map((bar, i) => (
            <View key={i} style={styles.barColumn}>
              ...
              <View style={styles.barTrack}>
                <View style={[styles.barFill, { height: `${(bar.value / max) * 100}%`, backgroundColor: bar.highlight ? COLORS.purple800 : 'rgba(83,74,183,0.35)', },]} />
              </View>
  ...
  return (
    <SafeView backgroundColor={Colors.white} bar={false} statusbarColor={Colors.main}>
      <View style={{ width: '100%', height: '100%' }}>
        <SBSCommonHeaderView title="Performance Graph SLCT" backPath=" " Filter={true} navigation={navigation} props={props} handlePerformanceFilterOpen={() => setPerformanceCalendarOpen(true)} />
        {loading ? null : <ScrollView style={styles.scroll} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false} >
          ...
          <View style={styles.card}>
            ...
            <View style={styles.currentMonthRow}>
              <View>
                <Text style={styles.bigNumber}> {data?.current_month_qty_mt} <Text style={styles.bigNumberUnit}> MT</Text> </Text>
              </View>
