...
const POPOrderScreen = (props) => {
    ...
    const requestForProductList = async () => {
        ...
        try {
            ...
            if (result.process_status === 'YES')
                setProductList(result.pop_product_date.map((item) => ({ ...item, count: '', })))
        } catch (e) { }
        setLoading(false)
    }
    ...
    const onChangeTextHandler = (text, index) => {
        setProductList((prev) => prev.map((item, i) => i === index ? { ...item, count: text.replace(/[^0-9]/g, '') } : item))
    }

    const incrementHandler = (_, index) => {
        setProductList((prev) => prev.map((item, i) => i === index ? { ...item, count: String(Number(item.count || 0) + 1) } : item))
    }

    const decrementHandler = (_, index) => {
        setProductList((prev) => prev.map((item, i) => i === index ? { ...item, count: Number(item.count) > 0 ? String(Number(item.count) - 1) : '', } : item))
    }
        ...
        return (
            <View style={styles.card}>
                <View style={styles.container}>
                    ...
                    </View> : <Image source={{ uri: item.image }} style={styles.image} />}
    ...
    return (
        <SafeView backgroundColor={Colors.white} bar={false} statusbarColor={Colors.main}>
                ...
                <View style={{ width: "100%", padding: moderateScale(10), gap: moderateScale(10), flex: 1 }}>
                    <View style={{ width: "100%", flex: 1 }}>
                        ...
                        <View style={{ width: "100%", flex: 1, backgroundColor: "#FFFFFF", borderBottomWidth: moderateScale(1), borderLeftWidth: moderateScale(1), borderRightWidth: moderateScale(1), borderColor: "#DCDDDF", padding: moderateScale(10) }}>
                            {appOrder ? (
                                popOrderList?.length > 0 ? <FlatList
                                    data={popOrderList}
                                    key="history"
                                ...
                                /> : <View style={styles.wrapper}>
                                    <View style={styles.iconCircle}>
                                        <Image source={Icons.CementOrder} style={[styles.icon, { tintColor: DataStorage.primaryColorCode }]} />
                                    </View>
                                    <Text style={styles.title}>No Orders Yet</Text>
                                    <Text style={styles.subtitle}>You haven't placed any POP orders.{'\n'}Start by selecting products!</Text>
                                </View>
                            ) : (
                                ...
                                /> : <View style={styles.wrapper}>
                                    <View style={styles.iconCircle}>
                                        <Image source={Icons.Cart} style={[styles.icon, { tintColor: DataStorage.primaryColorCode }]} />
                                    </View>
                                    <Text style={styles.title}>No Products Available</Text>
                                    <Text style={styles.subtitle}>There are no POP products available{'\n'}at the moment. Check back later!</Text>
                                </View>
                            )}
                        </View>
                    </View>
                    {!appOrder && productList?.some(item => Number(item.count) > 0) && <TouchableOpacity activeOpacity={0.95} onPress={() => { checkDataAndGotoCartPage() }}>
            ...
            <MonthYearPicker visible={showPicker} onConfirm={handleChange} onCancel={() => setShowPicker(false)} />
        </SafeView>
    )
}
