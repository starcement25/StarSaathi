  ...
  const Row = ({ label, value, isStatus = false, isChevron = false }) => (
    <View style={cardStyles.row}>
      ...
      <View style={cardStyles.valueWrap}>
        {isStatus ? <View style={[cardStyles.statusBadge, { backgroundColor: statusStyle.bg, borderColor: statusStyle.border }]}>
          <Text style={[cardStyles.statusText, { color: statusStyle.text }]}>{value}</Text>
        </View> : <Text style={cardStyles.value}>{value || '–'}</Text>}
        {isChevron && <Text style={cardStyles.chevron}>›</Text>}
      </View>
    </View>
  )
...
const cardStyles = StyleSheet.create({
  card: { backgroundColor: '#fff', borderRadius: 12, marginHorizontal: 12, marginBottom: 12, paddingHorizontal: 16, paddingVertical: 4, elevation: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3, },
  ...
  label: { fontSize: 13, color: '#666', fontWeight: '400', flex: 1, },
  valueWrap: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', flex: 1.2, },
  value: { fontSize: 13, fontWeight: '700', color: '#111', textAlign: 'right', flexShrink: 1, },
  chevron: { fontSize: 18, color: '#888', marginLeft: 4, fontWeight: '300', },
...
const DeliveryComplainScreen = ({ navigation }) => {
  const defaultFrom = new Date(2026, 4, 9)
  const defaultTo = new Date(2026, 4, 16)
        ...
        <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={styles.contentContainer} >
          <View style={styles.dateSection}>
            ...
            <View style={styles.dateRow}>
              <TouchableOpacity style={styles.dateBox} onPress={() => setShowFromPicker(true)} activeOpacity={0.75} >
                <Image style={styles.calendarIcon} source={Icons.Calender} />
                <Text style={styles.dateText}>{formatDate(fromDate)}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.dateBox} onPress={() => setShowToPicker(true)} activeOpacity={0.75} >
                <Image style={styles.calendarIcon} source={Icons.Calender} />
          ...
          {filteredComplaints.length > 0 ? filteredComplaints.map(item => (
            ...
            <Text style={styles.emptySubtitle}> No complaints found for the selected date range. </Text>
          </View>}
          <View style={{ height: 24 }} />
        </ScrollView>
        {showFromPicker && <DateTimePicker value={fromDate} mode="date" display={Platform.OS === 'ios' ? 'spinner' : 'default'} onChange={handleFromChange} maximumDate={toDate} />}
        {showToPicker && <DateTimePicker value={toDate} mode="date" display={Platform.OS === 'ios' ? 'spinner' : 'default'} onChange={handleToChange} minimumDate={fromDate} />}
      </View>
    </SafeView>
  )
}
...
const styles = StyleSheet.create({
  ...
  contentContainer: { paddingTop: 4, },
  dateSection: { paddingHorizontal: 12, paddingVertical: 14, },
  dateLabel: { fontSize: 13, fontWeight: '700', color: '#111', marginBottom: 10, },
  dateRow: { flexDirection: 'row', gap: 10, },
  dateBox: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#fff', borderRadius: 8, borderWidth: 1, borderColor: '#E0E0E0', paddingHorizontal: 12, paddingVertical: 11, },
  calendarIcon: { height: 18, width: 18, tintColor: Colors.main, },
