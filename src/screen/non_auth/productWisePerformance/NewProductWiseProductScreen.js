    ...
    return (
        <TouchableOpacity onPress={onPress} activeOpacity={0.75} style={{ flexDirection: 'row', alignItems: 'flex-end', marginHorizontal: 5 }} >
            <View style={{ alignItems: 'center', width: barWidth }}>
                <View style={{ height: chartHeight + LABEL_H, justifyContent: 'flex-end' }}>
                    {tgtValue > 0 ? <Animated.View style={{ marginBottom: tgtPx, height: LABEL_H, justifyContent: 'flex-end', position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'center', }}>
                        <Text style={{ width: 30, fontSize: 8, color: '#222', lineHeight: 9, textAlign: 'center' }}> {tgtValue} </Text>
                    </Animated.View> : null}
            ...
            <View style={{ alignItems: 'center', width: barWidth + 10 }}>
                <View style={{ height: chartHeight + LABEL_H, justifyContent: 'flex-end' }}>
                    {achValue > 0 ? <Animated.View style={{ width: '100%', marginLeft: -5, position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'center', marginBottom: achPx, justifyContent: 'center' }}>
                        <Text style={{ fontSize: 8, color: '#222', lineHeight: 9 }}> {achValue} </Text>
                    </Animated.View> : null}
...
const NewProductWiseProductScreen = (props) => {
    ...
    return (
        <SafeView backgroundColor={Colors.white} bar={false} statusbarColor={DataStorage.typeOfUse == 1 ? '#8FC227' : Colors.main}>
            <View style={{ flex: 1 }}>
                <SBSCommonHeaderView title="Product Sales Performance" backPath="" Filter={false} navigation={navigation} props={props} handlePerformanceFilterOpen={() => setPerformanceFilterOpen(true)} />
                                    ...
                                    <View style={{ flexDirection: 'row', height: moderateScale(50), backgroundColor: '#F9F9F9' }}>
                                        {['Product', 'Target\n(MT)', 'Achievement\n(MT)', '% Achievement'].map((title, i) => (
                                            <View key={i} style={{ width: title == 'Product' || title == '% Achievement' ? moderateScale(150) : moderateScale(120), justifyContent: 'center', alignItems: 'center' }}>
                                                <Text style={{ color: '#314158', fontSize: moderateScale(14), textAlign: 'center', fontWeight: '600' }}> {title} </Text>
                                            </View>
                                        ))}
                                    </View>
