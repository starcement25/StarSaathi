...
const AnimatedBar = ({ tgtValue, achValue, maxValue, barWidth, chartHeight, index, onPress, }) => {
  ...
  const tgtPx = tgtAnim.interpolate({ inputRange: [0, 1], outputRange: [0, tgtBarH], })
  const achPx = achAnim.interpolate({ inputRange: [0, 1], outputRange: [0, achBarH], })
  const LABEL_H = 14
    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.75} style={{ flexDirection: 'row', alignItems: 'flex-end', marginHorizontal: 5 }} >
      <View style={{ alignItems: 'center', width: barWidth }}>
        <View style={{ height: chartHeight + LABEL_H, justifyContent: 'flex-end' }}>
          {tgtValue > 0 ? <Animated.View style={{ marginBottom: tgtPx, height: LABEL_H, justifyContent: 'flex-end', position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'center', }}>
            <Text style={{ width: 30, fontSize: 8, color: '#222', lineHeight: 9, textAlign: 'center' }}> {formatVal(tgtValue)} </Text>
          </Animated.View> : null}
      ...
      <View style={{ alignItems: 'center', width: barWidth }}>
        <View style={{ height: chartHeight + LABEL_H, justifyContent: 'flex-end' }}>
          {achValue > 0 ? <Animated.View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'center', marginBottom: achPx, height: LABEL_H, justifyContent: 'flex-end', }}>
            <Text style={{ fontSize: 8, color: '#222', lineHeight: 9 }}> {formatVal(achValue)} </Text>
          </Animated.View> : null}
  ...
  return (
    <View style={{ width: 46, paddingRight: 6, paddingTop: LABEL_H }}>
      <View style={{ height: chartHeight, justifyContent: 'space-between' }}>
        {steps.map((val, i) => (
          <Text key={i} style={{ fontSize: 9, color: 'gray', textAlign: 'right' }}> {formatVal(val)} </Text>
        ))}
      </View>
    </View>
  )
      ...
      return (
        <View key={`lbl-${index}`} style={{ width: groupW + 1, height: labelAreaHeight, marginHorizontal: 5, alignItems: 'center', justifyContent: 'flex-start', paddingTop: 4, overflow: 'visible', }} >
          <View style={{ width: labelAreaHeight, transform: [{ rotate: '90deg' }, { translateX: (labelAreaHeight - groupW) / 2 },], alignItems: 'center', }}>
            <Text numberOfLines={2} style={{ fontSize: 8, color: '#333', fontWeight: '400' }}> {item.label} </Text>
          </View>
        </View>
      )
    })}
...
const ProductWiseProductScreen = (props) => {
  ...
  return (
    <SafeView backgroundColor={Colors.white} bar={false} statusbarColor={Colors.main}>
      <View style={{ flex: 1 }}>
        <SBSCommonHeaderView title={selected} backPath=" " Filter={false} Calendar={true} navigation={navigation} props={props} handlePerformanceFilterOpen={() => setPerformanceFilterOpen(true)} handlePerformanceCalendarOpen={() => setPerformanceCalendarOpen(true)} />
        <View style={{ flex: 1, padding: moderateScale(12) }}>
          <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: LEGEND_H, }}>
            {[{ color: '#33B5E6', label: 'Target' }, { color: '#E41B14', label: 'Achievement' },].map(({ color, label }) => (
          ...
          <View style={{ flex: 1, backgroundColor: '#FFF', borderWidth: 1, borderColor: '#DCDDDF', borderRadius: moderateScale(10), paddingTop: moderateScale(10), paddingBottom: 6, paddingHorizontal: moderateScale(6), }}>
            {chartData.length > 0 ? <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', flex: 1 }}>
                <YAxis maxValue={maxValue} chartHeight={CHART_HEIGHT} noOfSections={NO_OF_SECTIONS} />
                <View style={{ flex: 1 }}>
                  ...
                  <XLabels scrollRef={labelScrollRef} chartData={chartData} barWidth={BAR_WIDTH} labelAreaHeight={LABEL_AREA_H} />
                  {chartData.length > 4 && <Text style={{ fontSize: 9, color: '#ccc', textAlign: 'right', paddingRight: 4, height: SCROLL_HINT_H, lineHeight: SCROLL_HINT_H, }}> ← scroll → </Text>}
                </View>
              </View>
            </View> : <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ color: '#888', fontSize: 14 }}> {loading ? 'Loading...' : 'No data found'} </Text>
            </View>}
          </View>
        </View>
      </View>
      <PerformanceFilterView isVisible={performanceFilterOpen} closeLoginPopup={() => setPerformanceFilterOpen(false)} />
      <PerformanceCalenderView isVisible={performanceCalendarOpen} closeLoginPopup={() => setPerformanceCalendarOpen(false)} onMonthSelect={handleMonthSelect} />
    </SafeView>
  )
}
