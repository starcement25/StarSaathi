...
const OutstandingSummaryScreen = (props) => {
  ...
  const requestForBucket = () => {
    ...
    fetch("https://starsaathi.com/SAP/day_range_list.php", requestOptions)
    ...
    fetch(url, requestOptions)
      ...
      .then((result) => {
        ...
        for (var i = 0; i < bucketList.length; i++) {
          ...
          var bucketObj = {
            label: bucketList[i].end_day != 0 ? bucketList[i].start_day + ' - ' + bucketList[i].end_day + ' Days' : (bucketList[i].start_day + 1) + '+ Days',
            color: '#E74C3C',
            value: 0.0,
            formattedValue: `₹${Number(0.0).toLocaleString('en-IN')}`
          }
          ...
          for (var k = 0; k < result.data.length; k++) {
            ...
            if (day >= start && day <= end) {
              var obj = {
                label: result.data[k].title,
                subLabel: result.data[k].title + " overdue",
              }
              var list = []
              var a = 0
