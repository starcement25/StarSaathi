...
const LedgerScreen = (props) => {
  ...
  const gotoLink = () => {
    ...
    DataStorage.web_link = UrlStorage.BaseUrlList.Saathi.base_url_saathi + '/dashboard/'
  ...
  const requestForLedgerListSBS = async () => {
    ...
    const requestOptions = { method: "GET", redirect: "follow", headers: { "Authorization": UrlStorage.ParameterList.BasicData.user_type == 'broker' ? `SAP_SP` : `SAP_DEALER` + `${UrlStorage.ParameterList.BasicData.emp_id}`, "Content-Type": "application/json", }, }
    var url = ""
    if (UrlStorage.ParameterList.BasicData.user_type == "broker")
  ...
  const downloadLedgerPdf = async () => {
    ...
    try {
      ...
      const res = await RNFetchBlob.config({ fileCache: true, appendExt: 'pdf', }).fetch('GET', url)
      const localPath = res.path()
      props.navigation.navigate('PdfViewScreen', { pdfUrl: localPath, pdfLink: url, page_title: 'Ledger PDF', type: 'local' })
    } catch (error) {
  ...
  const downloadLedgerPdfSBS = async () => {
    ...
    try {
      ...
      const res = await RNFetchBlob.config({ fileCache: true, appendExt: 'pdf', }).fetch('GET', url, { Authorization: UrlStorage.ParameterList.BasicData.user_type == 'broker' ? `SAP_SP${UrlStorage.ParameterList.BasicData.emp_id}` : `SAP_DEALER${UrlStorage.ParameterList.BasicData.emp_id}`, })
      const pdfPath = res.path()
      props?.navigation.navigate('PdfViewScreen', { pdfUrl: pdfPath, page_title: 'Transaction Ledger', type: 'local', pdfLink: url, })
    } catch (error) {
    ...
    <SafeView backgroundColor={Colors.white} bar={false} statusbarColor={Colors.main}>
        ...
        <View style={{ width: "100%", flex: 1 }}>
          ...
          <View style={{ width: "100%", flex: 1 }}>
            {ledgerList?.length > 0 ? <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => { }} activeOpacity={0.95} style={{ paddingHorizontal: moderateScale(10), paddingVertical: moderateScale(7) }}>
      ...
      {showStartDatePicker && <View style={{ width: '100%', height: '100%', position: 'absolute', alignItems: 'center', justifyContent: 'center', backgroundColor: '#00000060' }}>
        <View style={{ backgroundColor: '#FFF', padding: 10, borderRadius: 10 }}>
          <DateTimePicker value={startDate} mode="date" display="default" maximumDate={new Date()} onChange={handleStartDateConfirm} />
        </View>
      </View>}
      {showEndDatePicker && <View style={{ width: '100%', height: '100%', position: 'absolute', alignItems: 'center', justifyContent: 'center', backgroundColor: '#00000060' }}>
        <View style={{ backgroundColor: '#FFF', padding: 10, borderRadius: 10 }}>
          <DateTimePicker value={endDate} mode="date" display="default" maximumDate={new Date()} minimumDate={startDate} onChange={handleEndDateConfirm} />
        </View>
      </View>}
