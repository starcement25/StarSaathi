...
import { View, Text, TextInput, TouchableOpacity, ScrollView, StyleSheet, Modal, FlatList, Platform, KeyboardAvoidingView, } from "react-native"
...
export default function DealerDeclarationScreen({ navigation }) {
    ...
    const loadInitialData = async () => {
        ...
        try {
            const [dealerRes, monthRes] = await Promise.all([
                fetch(UrlStorage.BaseUrlList.Saathi.base_url_saathi + UrlStorage.NonAuthURL.Saathi.OtherURL.api_get_exclusive_dealername + "?customer_id=" + UrlStorage.ParameterList.BasicData.emp_id, { method: "GET" }),
                fetch(UrlStorage.BaseUrlList.Saathi.base_url_saathi + UrlStorage.NonAuthURL.Saathi.OtherURL.api_exclusive_get_month_list + "?customer_id=" + UrlStorage.ParameterList.BasicData.emp_id, { method: "GET" }),
            ])
    ...
    const validateForm = useCallback(() => {
        if (!dealerName) {
            showToast("error", "Error", "Dealer Name cannot be empty")
            return false
        }
        if (!selectedMonth) {
            showToast("error", "Error", "Please select a Month")
            return false
        }
        if (!branch) {
            showToast("error", "Error", "Branch cannot be empty")
            return false
        }
        if (!liftingQty) {
            showToast("error", "Error", "Lifting Quantity cannot be empty")
            return false
        }
        return true
    ...
    const submitDeclaration = useCallback(async () => {
        ...
        setLoading(true)
        const url = UrlStorage.BaseUrlList.Saathi.base_url_saathi + UrlStorage.NonAuthURL.Saathi.OtherURL.api_exclusive_data_save
        const body = {
        ...
        try {
            ...
            if (result?.process_status === "Yes") {
                ...
                showToast("error", result?.process_message ?? "Error", result?.error ?? "Something went wrong. Please try again later")
        } catch (error) {
    ...
    return (
        <SafeView style={styles.container} statusbarColor={Colors.main}>
            ...
            <KeyboardAvoidingView style={styles.flex1} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={Platform.OS === "ios" ? 88 : 0} >
                <ScrollView style={styles.scrollContainer} contentContainerStyle={SCROLL_CONTENT_STYLE} keyboardShouldPersistTaps="handled" keyboardDismissMode="none" removeClippedSubviews={false} >
                    ...
                    <View style={styles.greetingSection}>
                        ...
                        <Text style={styles.descriptionText}> {" "}This is to inform you that I am continuing as an Exclusive dealer of STAR Cement and not lifting cement for any other company.{" "} </Text>
                    </View>
                ...
                <View style={styles.submitButtonContainer}>
                    <TouchableOpacity style={[styles.submitBtn, !screenReady && styles.submitBtnDisabled]} onPress={submitDeclaration} activeOpacity={0.85} disabled={!screenReady} >
            ...
            <Modal visible={cutOffModalVisible} transparent animationType="fade" onRequestClose={() => setCutOffModalVisible(false)} hardwareAccelerated={true} >
                <View style={styles.alertModalOverlay}>
                    <View style={styles.alertModalContainer}>
                        ...
                        <Text style={styles.alertMessage}> {" "}Selected month is not valid. Please choose another to continue.{" "} </Text>
                        <View style={styles.alertButtonRow}>
                            <TouchableOpacity style={styles.alertSecondaryBtn} onPress={handleClose} activeOpacity={0.8} >
                                <Text style={styles.alertSecondaryBtnText}>Close</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={styles.alertPrimaryBtn} onPress={openMonthModal} activeOpacity={0.8} >
...
const styles = StyleSheet.create({
    ...
    input: { height: 44, borderRadius: 8, backgroundColor: "#fff", paddingHorizontal: 14, fontSize: 14, color: "#1a1a1a", borderWidth: 1, borderColor: "#e0e0e0", fontWeight: "500", },
    monthSelectorGroup: { marginBottom: 20, marginTop: 24 },
    monthSelector: { height: 44, borderRadius: 8, backgroundColor: "#fff", paddingHorizontal: 14, borderWidth: 1.5, borderColor: "#e0e0e0", flexDirection: "row", alignItems: "center", justifyContent: "space-between", },
