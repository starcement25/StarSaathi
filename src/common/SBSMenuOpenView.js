...
const MenuItem = ({ icon, label, onPress, accent = false }) => (
  <TouchableOpacity onPress={onPress} activeOpacity={0.65} style={styles.menuItem}>
    {icon && (
      <View style={[styles.menuIconWrap, accent && { backgroundColor: `${BRAND}18` }]}>
        <Image source={icon} style={[styles.menuIcon, { tintColor: accent ? BRAND : TEXT_SECONDARY }]} />
      </View>
    )}
    <Text style={[styles.menuLabel, accent && { color: BRAND, fontWeight: "700" }]}>
      {label}
    </Text>
...
const SBSMenuOpenView = (props) => {
  ...
  return (
    ...
    >
      {/* ── Sidebar shell: full screen height, fixed width ── */}
      <View style={styles.sidebar}>
        ...
        {/* ── Scrollable menu (flex: 1 so it fills remaining space) ── */}
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} bounces={true} >
          ...
          {/* Logout inside scroll so it's always reachable */}
...
const styles = StyleSheet.create({
  // ── KEY FIX: sidebar is now positioned absolutely to fill full screen height ──
  ...
  // ── KEY FIX: flex:1 lets ScrollView fill all space between header and footer ──
  scrollView: { flex: 1 },
