  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => {
        const isSelected = index === selectedIndex + 2; // offset for padding
        return (
          <View style={styles.drumItem}>
            <Text style={[styles.drumText, isSelected && styles.selectedText]}>
              {item}
            </Text>
          </View>
        );
...
const styles = StyleSheet.create({
  ...
  selectionBar: {
    ...
    left: 16,
    right: 16,
    top: ITEM_HEIGHT * 2,         // center row (3rd of 5 visible)
    height: ITEM_HEIGHT,
