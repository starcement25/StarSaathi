...
class TransportModeSubCategoryController extends Controller{
  ...
  public function transportmodesubcategoryincremental(Request $request){
    ...
    if($isverify==1){
      ...
      $sqlquery=$CUTDB->select("SELECT * FROM transport_mode_sub_category WHERE 1 ".$login_condition." ORDER BY transport_mode_sub_cat_name ASC");
      $count=count($sqlquery);
        ...
        $url = url('/api/v1/transportmodesubcategory?nick_name='.$nick_name.'&device_id='.$device_id.'&last_update_time='.$last_update_time.'&incremental_download='.$incremental_download);
