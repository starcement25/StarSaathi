...
class RARejectBIdRateDownloadController extends Controller
{
    ...
    public function rejectbidratedownload(Request $request){
        ...
        $sqlquery=$CUTDB->select("SELECT bid_id,plant_name,prod_code,released_rate,server_indicative_rate,customer_code,qty,bid_rate,
							counter_bid,counter_bid_rate,bid_status,base_rate,app_indicative_rate,GST_percent,GST_value,primary_freight,secondary_freight,depot_cost,branch_code,	incoterms,vertical_value FROM RA_bid_rate_details WHERE SUBSTRiNG(bid_id,3,5)='".$emp_code."' AND bid_status='REJECT' 
