	...
	if($_REQUEST['mode']=='survey_post')
{
	...
	foreach($survey_array_chunk as $key=>$index)
	{
		...
		if($index[13] == '')
		{
			...
			$res_get_max_rowid = mysql_query($sql_get_max_rowid);
			$row_get_max_rowid = mysql_fetch_array($res_get_max_rowid);
		...
		if($flag == 0)
		{
				...
				$res_check_display_order = mysql_query($sql_check_display_order);
				$total_rows = mysql_num_rows($res_check_display_order);
				if($total_rows>0)
				{
					...
					$res_reset_display_order = mysql_query($sql_reset_display_order);
				}
		...
		else
		{
				...
				$select_total_rows = mysql_num_rows($res_select_survey_input);
