...
{
  $sql_zone = "SELECT EM.zone as zone_name FROM employee_master EM, sauda_details SD WHERE substring(SD.sauda_no,3,5)='$row_emp[emp_code]' ".$sauda_booked_condition." AND EM.emp_code=substring(SD.sauda_no,3,5) GROUP BY zone_name ORDER BY zone_name ASC";
  ...
  while($row_zone = mysql_fetch_array($res_zone))	
  {
	  ...
	  while($row_product = mysql_fetch_array($res_product))
	  {
		   ...
		   $sql_quantity_booked = "SELECT sum(SD.convert_qty_two) as mt_booked, sum(SD.qty) as case_booked FROM sauda_details SD, product_master PM, employee_master EM WHERE substring(SD.sauda_no,3,5)='".$row_emp['emp_code']."' AND EM.emp_code=substring(SD.sauda_no,3,5) AND SD.sku_code=PM.prod_code AND PM.$field_name1='$row_product[$field_name1]' AND EM.zone LIKE '%".$zone_name."%' ".$sauda_booked_condition;
...
foreach($zonewise as $branchwiseindex=>$branchwisevalue)
{
	...
	foreach($branchwisevalue as $index=>$val)
	{
		...
		{
		   echo "
				<td align=\"right\" style=\"background:$color; color:blue; cursor:pointer; font-weight:bold;\" onclick=\"show_zonewise_data('$branchwiseindex','$start_date','$end_date','$value','$index');\">".$val."</td>";
