...
ob_start();
	...
	function main(){
		...
		if($current_date_array[1] > 3)
		{
			$date_array=array(($current_date_array[0]+1),$current_date_array[0]);
		}
		else
		{
			$date_array=array(($current_date_array[0]-1),$current_date_array[0]);
		...
		?>
        <center>
        <form method="POST" action="attendance_download_data_modified.php">
        <table width="30%" cellpadding="5px">
        	...
        	<tr class="TDHEAD_SUB">
                ...
                <td>Year:<!--input type="text" value="<?php //echo $current_date_array[0]; ?>" size="6" name="year" readonly /-->
                	<select name="year">
					<?php 
				foreach($date_array as $year_val){
						echo "<option selected>".$year_val."</option>";
				}
				?>
                </select>
                </td>
