...
if($type == 'state'){
	...
	echo "<select name=\"state\" id=\"state\" onchange=\"".$onclick."\">";
	echo "<option value=\"\">Select</option>";
	
	
	$sql_state = "SELECT DISTINCT SUBSTRING_INDEX(state, ',', 1) AS state FROM employee_master WHERE zone IN (".$zone.")".$emp_hierarchy_condition_one." AND state != '' ORDER BY state ASC";
	...
	echo "<option value=\"".$state_string."\">All</option>";
	echo "</select>";
}
