...
if($type == 'sale_access'){
	...
	echo $sql_sale_access = "SELECT DISTINCT sale_access FROM employee_master WHERE branch_code IN (".$branch.")".$emp_hierarchy_condition_one." AND sale_access != '' ORDER BY sale_access ASC";
	$res_sale_access = mysql_query($sql_sale_access);
...
/*--------> HQ Data Populate <--------*/
	...
	$sql_hq = "SELECT DISTINCT hq FROM employee_master WHERE branch_code IN (".$branch.")".$emp_hierarchy_condition_one." AND hq != '' ORDER BY hq ASC";
	$res_hq = mysql_query($sql_hq);
