...
/*--------> Employee Data Populate <--------*/
	...
	//$onclick = "designation_emp(this.value);";
	if($branch != '')
		$branch_condition_one = " AND branch_code IN (".$branch.") ";
	
	echo "<select name=\"employee\" id=\"employee\" onchange=\"".$onclick."\">";
	echo "<option value=\"\">Select</option>";
	
	
	$sql_emp = "SELECT emp_code, emp_name FROM employee_master WHERE sale_access IN (".$sale_access.")".$emp_hierarchy_condition_one.$branch_condition_one." ORDER BY emp_name ASC";
