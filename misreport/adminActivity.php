                ...
                style="display: <?php if($_REQUEST['radio_type']=='today' || $_REQUEST['mode']=='' || $_REQUEST['mode']=='T'){?> ''<?php }else{?>none<?php }?>;height: 150px;overflow-y: scroll;" id="todayAttDisplay">
					...
					<?php 
						...
						while($rowinformation=mysql_fetch_array($resinformation))
						{
							...
							if(sale_rate=='yes')
							{
							$sqltotalorder="SELECT ROUND(SUM(OD.amount),2) AS total_order_received,SUM(OD.qty*OD.sale_rate) AS total_amount FROM order_details OD, location LO,order_header OH,customer_master CM WHERE OD.order_no = LO.trans_id AND LO.trans_id LIKE 'O%' AND LO.emp_code = '".$emp_code."' 
							AND OH.order_no=OD.order_no AND OH.customer_code=CM.customer_code ".$date_condition;
							}
							if(mrp=='yes')
							{
							$sqltotalorder="SELECT ROUND(SUM(OD.amount),2) AS total_order_received,SUM(OD.qty*MRP.mrp) AS total_amount FROM order_details OD, location LO,order_header OH,customer_master CM,mrp MRP WHERE OD.order_no = LO.trans_id AND LO.trans_id LIKE 'O%' 
                ...
                style="display: <?php if($_REQUEST['radio_type']=='yesterday' || $_REQUEST['mode']=='Y'){?> ''<?php }else{?>none<?php }?>;height: 150px;overflow-y: scroll;" id="yesterdayAttDisplay">
					...
					<?php }
						...
						while($rowinfoyesterday=mysql_fetch_array($resinfoyesterday))
						{
							...
							if(mrp=='yes')
							{
							$sqltotalorderyesterday="SELECT ROUND(SUM(OD.amount),2) AS total_order_received,SUM(OD.qty*MRP.mrp) AS total_amount FROM order_details OD, location LO,order_header OH,customer_master CM,mrp MRP WHERE OD.order_no = LO.trans_id AND LO.trans_id LIKE 'O%' 
							AND LO.emp_code = '".$emp_code."' AND OD.sku_code=MRP.product_code AND OH.order_no=OD.order_no AND OH.customer_code=CM.customer_code ".$date_condition;
							}
