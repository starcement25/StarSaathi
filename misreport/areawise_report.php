...
function Popup(data) 
{
	var mywindow = window.open('', 'Areawise Report', 'height=400,width=600');
	mywindow.document.write('<html><head><title>Areawise Report</title>');
...
function exporttocsv(divid)
{
	...
	a.download = 'Areawise Report' + postfix + '.xls';
...
function show_saudawise()
{
	...
	if(type != 'custom')
		GenericAjaxFunction('areawise_data.php?type='+type,'display',0);
	else 
	{
		if(response1 != 0 && response2 != 0)
			GenericAjaxFunction('areawise_data.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'display',0);
	}
}
...
function show_prev_page(){
	...
	window.open('http://acedns.in/acednsproduct/misreport/abdos_report_main.php?type='+type+'&start_date='+start_date+'&end_date='+end_date,'_self');
}
</script>
<?php
}
