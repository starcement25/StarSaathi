...
function post_data()
{
	...
	if($_REQUEST['mode']=='survey_post')
	{
		...
		$res_get_row_id = mysql_query($sql_get_row_id);
		$rows_count = mysql_num_rows($res_get_row_id);
		if($rows_count>0){
			$res_get_row_id = mysql_query($sql_get_row_id);
			$row_get_row_id = mysql_fetch_array($res_get_row_id);
			...
			$res_insert_tableview = mysql_query($sql_insert_tableview);
		}
