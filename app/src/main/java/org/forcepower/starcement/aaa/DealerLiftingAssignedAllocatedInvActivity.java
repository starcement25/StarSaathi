...
import org.forcepower.starcement.fragments.dealer_lifting.allocated.AllocatedDealerLiftingFragment;
...
public final class DealerLiftingAssignedAllocatedInvActivity extends AceDnsParentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            final TextView tvHeaderText =  findViewById(R.id.tvHeaderText);
            ...
            final ImageView ivMonthSelection =  findViewById(R.id.ivMonthSelection);
...
//                final AssignedDealerLiftingFragment frgASSIGNED = new AssignedDealerLiftingFragment().newInstance("");
                final DealerLiftingAssignedInvFragment frgASSIGNED = new DealerLiftingAssignedInvFragment().newInstance("");
                final AllocatedDealerLiftingFragment frgALLOCATED = new AllocatedDealerLiftingFragment().newInstance("");
//                final DealerLiftingAlocattedInvFragment frgALLOCATED = new DealerLiftingAlocattedInvFragment().newInstance("");
                ...
                ivMonthSelection.setOnClickListener(view -> {
                    try {
                        if (HTTPUtils.isConnectionPossible(mContext)) {
                            if (view_pager2.getCurrentItem() == 0) {
                                frgASSIGNED.assignFilter();
                            } else {
                                frgALLOCATED.allocatedFilter();
                            }
                        } else {
