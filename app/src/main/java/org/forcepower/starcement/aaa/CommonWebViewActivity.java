...
public final class CommonWebViewActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try
        {
            ...
            Log.d("TAG", "onCreatewebview_urlwebview_url: "+webview_url);
            if(webview_url.startsWith("https://dealerrewards.myvtd.site/")){
                saveFcmTokenRaw();
            }
            //
            reload(null);
...
//                    loadDialog();
    ...
    public void saveFcmTokenRaw() {
        try {
            Log.d("TAG", "_CALLING saveFcmTokenRaw loader On");

            // JSON body
            JSONObject json = new JSONObject();
            json.put("dealer_id", get_dealer_id(myActivity));
            json.put("fcm_token", get_firebase_token(myActivity));
            json.put("device_id",get_firebase_token(myActivity));
            ...
            String apiUrl = "https://starrewardadmin.myvtd.site/save_fcm_token/";
            print_Log_d("FCM_URL", apiUrl);
            print_Log_d("FCM_RAW_BODY", json.toString());

            // Send POST request with raw JSON
            client.post(myActivity, apiUrl, entity, "application/json", new AsyncHttpResponseHandler() {
                @Override
                public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                    String response = new String(responseBody);
                    try {
                        print_Log_d("FCM_RESPONSE", response);

                        JSONObject jsonResponse = new JSONObject(response);
                        String message = jsonResponse.optString("process_message", "Success");
                        Toast.makeText(myActivity, message, Toast.LENGTH_SHORT).show();

                        if (jsonResponse.optString("message").equalsIgnoreCase("Fcm Token save successfully")) {
                            print_Log_d("FCM_STATUS", "Token saved successfully");
                        } else {
                            print_Log_d("FCM_STATUS", "Failed: " + message);
                        }

                    } catch (Exception e) {
                        e.printStackTrace();
                        print_Log_d("FCM_PARSE_ERROR", e.getMessage());
                    } finally {
                ...
                public void onFailure(int statusCode, Header[] headers, byte[] responseBody, Throwable error) {
                    print_Log_d("FCM_ERROR", "Error: " + error.toString());
                    Toast.makeText(myActivity, "Failed to send FCM token", Toast.LENGTH_SHORT).show();
                }
            });

        } catch (Exception e) {
            e.printStackTrace();
            print_Log_d("FCM_EXCEPTION", e.toString());
        }
    }
