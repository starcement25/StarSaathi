...
public final class ActivityOrderFilter extends AceDnsParentActivity {
	...
	private EditText et_con_address, et_con_name, et_phone_no, et_destination_address,
			et_delivery_remarks, et_plant_name;
	private TextView et_dump_name;
	...
	private LinearLayout ll_dump_details, ll_remarks, ll_plant_name;
	private CharSequence[] items = {"Multiple", "Single"};

	// New: store dialog selections in variables instead of TextViews
	private String forDeliveryPoint = "";
	private String dotDeliveryPoint = "";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			// Core layouts / views
			ll_dump_details = findViewById(R.id.ll_dump_details);
			ll_remarks = findViewById(R.id.ll_remarks);

			// Plant Name layout / edit text
			ll_plant_name = findViewById(R.id.ll_plant_name);
			et_plant_name = findViewById(R.id.et_plant_name);

			if (ll_dump_details != null) {
				ll_dump_details.setVisibility(View.GONE);
			}
			if (ll_remarks != null) {
				ll_remarks.setVisibility(View.GONE);
			}
			if (ll_plant_name != null) {
				ll_plant_name.setVisibility(View.GONE);
			}

			et_destination_address = findViewById(R.id.et_destination_address);
			et_con_name = findViewById(R.id.et_con_name);
			et_phone_no = findViewById(R.id.et_phone_no);
			et_dump_name = findViewById(R.id.et_dump_name);
			et_con_address = findViewById(R.id.et_con_address);
			et_delivery_remarks = findViewById(R.id.et_delivery_remarks);
				...
				public void onClick(View v) {
					try {
						if (HTTPUtils.isConnectionPossible(mContext)) {
							...
							} else if (rb_freight_for.isChecked() && forDeliveryPoint.trim().matches("")) {
								// still enforce FOR Multiple/Single, but from variable, not TextView
...
//							else if(rb_freight_dot.isChecked() && dotDeliveryPoint.trim().matches(""))
//							{
...
//							}
			...
			rb_freight_for = findViewById(R.id.rb_freight_for);
			rb_freight_exw = findViewById(R.id.rb_freight_exw);
			rb_freight_dot = findViewById(R.id.rb_freight_dot);
			...
			rb_freight_for.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// open dialog to choose Multiple/Single
					single_multiple_dialog("FOR");

					rb_freight_exw.setChecked(false);
					rb_freight_dot.setChecked(false);

					if (ll_remarks != null) ll_remarks.setVisibility(View.VISIBLE);
					if (ll_dump_details != null) ll_dump_details.setVisibility(View.GONE);

					// reset plant UI and load new plant name
					ensurePlantViews();
					if (ll_plant_name != null) {
						ll_plant_name.setVisibility(View.VISIBLE);
					}
					if (et_plant_name != null) {
						et_plant_name.setText("");
						et_plant_name.setTag("");
					}

					if (HTTPUtils.isConnectionPossible(mContext)) {
						new GetPlantName_Asynctask().execute();
					} else {
						Toast.makeText(mContext, check_internet_connection, Toast.LENGTH_SHORT).show();
					}
			...
			rb_freight_exw.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// clear any previous FOR/DOT selections
					forDeliveryPoint = "";
					dotDeliveryPoint = "";
					...
					if (ll_remarks != null) ll_remarks.setVisibility(View.GONE);
					if (ll_dump_details != null) ll_dump_details.setVisibility(View.VISIBLE);
			...
			rb_freight_dot.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					// open dialog to choose Multiple/Single for DOT
			...
			radio_ship_self = findViewById(R.id.radio_ship_self);
			radio_ship_sub_dealer = findViewById(R.id.radio_ship_sub_dealer);
			...
			if (branch_code.equalsIgnoreCase("B0042")) {
				final LinearLayout ll_dealer_truck = findViewById(R.id.ll_dealer_truck);
//				ll_dealer_truck.setVisibility(View.VISIBLE);
				final RadioButton rb_truck_yes = findViewById(R.id.rb_truck_yes);
				final RadioButton rb_truck_no = findViewById(R.id.rb_truck_no);
	...
	/** Safely (re)bind plant views if needed */
	private void ensurePlantViews() {
		if (ll_plant_name == null) {
			ll_plant_name = findViewById(R.id.ll_plant_name);
		}
		if (et_plant_name == null) {
		...
		try {
			...
			AlertDG.setItems(items, new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int item) {
					// store selection in variables instead of TextViews
...
//					}
					if (type.equalsIgnoreCase("DOT")) {
						dotDeliveryPoint = items[item].toString();
					} else {
						forDeliveryPoint = items[item].toString();
					}
					dialog.dismiss();
				}
			});
	...
	public void ShowDesti_Dump_Dialog(final ArrayList<DumpMaster> mDumpList) {
		try {
			...
			ImageView btnback = mDumpDialog.findViewById(R.id.back);
			...
			TextView tvDestHeading = mDumpDialog.findViewById(R.id.tvDestHeading);
			tvDestHeading.setText("SELECT DUMP");
			TextView title = mDumpDialog.findViewById(R.id.tvDestHeading);
			title.setText("Please select a dump");
			ListView dialogList = mDumpDialog.findViewById(R.id.list);
			...
			EditText searchText = mDumpDialog.findViewById(R.id.autoCompleteTextView1);
	...
	public void continueOrder() {
		try {
			...
			// Use stored variables instead of TextViews for Delivery_point
			if (Constants.mFreightComponent.equalsIgnoreCase("DOT")) {
				intent.putExtra("Delivery_point", dotDeliveryPoint.trim());
			} else {
				intent.putExtra("Delivery_point", forDeliveryPoint.trim());
			}
	...
	public void Show_Destination_List_Dialog(final ArrayList<DestinationMaster> mDesti_MasterList) {
		try {
			...
			TextView tvDestHeading = mDestinationDialog.findViewById(R.id.tvDestHeading);
	...
	public void showDealerSubDealerList(final String user_type, final ArrayList<DestinationMaster> mDealerSubDealerList) {
		try {
			...
			final EditText searchText = mDestinationDialog
	...
	// New AsyncTask to get Plant Name based on customer_code (selected_SAP_code)
	public final class GetPlantName_Asynctask extends AsyncTaskCoroutine<String, String> {
		private ProgressDialog mStepProgressDialog;
		private String plant_name_local = "";
		private String plant_code_local = "";

		public GetPlantName_Asynctask() {
		}
		...
		public String doInBackground(final String... params) {
			...
			try {
				if (HTTPUtils.isConnectionPossible(mContext)) {
					try {
						final String url = "https://starsaathi.com/SAP/api_get_plant_name.php?customer_code=" + selected_SAP_code;
						print_log_d("plant_name_url ", url);

						final ArrayList<NameValuePair> mHttpParamPairs = new ArrayList<>(0);
						GET_result = HTTPUtils.getDataByHTTP_POST(mContext, url, mHttpParamPairs);
						print_log_d("plant_name_response ", GET_result);

						final JSONObject jo = new JSONObject(GET_result);
						plant_name_local = jo.optString("plant_name", "");
						plant_code_local = jo.optString("plant_code", "");
					} catch (Exception e) {
		...
		public void onPostExecute(String result) {
			...
			try {
				ensurePlantViews();
				if (!"Network Failure".equalsIgnoreCase(result)) {
					if (et_plant_name != null) {
						et_plant_name.setText(plant_name_local != null ? plant_name_local : "");
						et_plant_name.setTag(plant_code_local);
					}
				} else {
					Toast.makeText(mContext, "Unable to fetch plant name. Please try again.", Toast.LENGTH_SHORT).show();
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
