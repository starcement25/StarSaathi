...
import static org.forcepower.starcement.constants.Constants.store_token;
    ...
    public final class TRANS_PostOTP_Asynctask extends AsyncTaskCoroutine<String, String> {
                ...
                if (HTTPUtils.isConnectionPossible(mContext))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            print_log_d("_orur4_O_RE_2 ", Constants.nickName + " "+login_mobile_number+" "+deviceId+" "+dealer_id_input+" "+my_otp);
        ...
        public void storeTokenToServer() {
            ...
            FormBody formBody = new FormBody.Builder()
                    .add("user_id", dealer_id_input)
                    .add("user_type", get_user_type(mContext))
                    .add("token", get_firebase_token(mContext))
                    .build();
            ...
            client.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    // Handle failure
                    runOnUiThread(() -> Toast.makeText(mContext, "API call failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
                }
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    if (response.isSuccessful()) {
                        ...
                        try {
                            ...
                            Log.d("OTPPPP", json.toString());
                            if (json.optString("status").equalsIgnoreCase("success")) {
                                runOnUiThread(() -> Toast.makeText(mContext, "Token stored successfully", Toast.LENGTH_SHORT).show());
                            } else {
                                runOnUiThread(() -> Toast.makeText(mContext, "Failed to store token: " + json.optString("process_message"), Toast.LENGTH_SHORT).show());
                            }
                        } catch (Exception e) {
                            runOnUiThread(() -> Toast.makeText(mContext, "Response parsing error", Toast.LENGTH_SHORT).show());
                        }
                    }
                }
            });
