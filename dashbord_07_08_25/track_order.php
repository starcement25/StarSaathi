...
$sql_on_ord = "select `APPORDERNO`,`STATUS`,`order_date`,`prod_display_name`,`QTY` from $t_apperpdo where `dns_customer_code`='$sswa_selected_dealer_code' and `APPORDERNO`!='' order by `order_date` desc";
...
if($totres_off_ord>0){
	while($row_off_ord = mysql_fetch_assoc($res_off_ord)){
		...
		$order_date_off_ord = $row_off_ord["order_date"] ? trim($row_off_ord["order_date"]) : "";
